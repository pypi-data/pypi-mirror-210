...
"""
    ...
    def integrate_spreading(self, variant = "ww3", wmin = _wmin, wmax = _wmax, n_theta = 72, n_freq = 100, method = "simpson" ) :
        """Return mean directional spread

        Parameters
        ----------
        variant : str, optional
            Variant. The default is "ww3".
    ...
    def _cossin( self , fun, wmin = 0.1, wmax = 2.0, method = "dblquad", n_theta = 101, n_freq = 100):
        ...
        if method == "dblquad" :
            ...
            S_int = [ simps( self.compute(w, t), w ) for t in theta ]
            res = simps( fun(theta) * S_int, theta )
        else :
    ...
    def integrate_mean_direction_simps( self, wmin = _wmin, wmax = _wmax, npts = 101 ):
...
class SeaState2D( _Spectral.SeaState2D, SeaStateABC ):
    ...
    def FromDataFrame( cls, df, ndir = None ):
        ...
        ndir is the number of desired directions in interval [0°, 360°) (including 0° and excluding 360°)
        """
        df_ = df.copy()
        ...
        if not np.isclose(2*np.pi, df.columns, rtol = 1.e-5).any() and not np.isclose(0., df.columns, rtol = 1.e-5).any():
            ...
            df_.loc[:,2*np.pi] = df_.iloc[:,0] # use iloc because small values sucha as 1.e-14 value are not recognized (Key error)

        elif np.isclose(2*np.pi, df.columns, rtol = 1.e-5).any():
            df_.loc[:,0.] = df_.iloc[:,-1]

        df_.sort_index(axis=1, inplace=True)
        if ndir is None: ndir = df_.shape[1]-1
...
if __name__ == "__main__":
    ...
    wave_heading = np.pi / 4
    vessel_azimuth = np.pi
    vessel_azimuth_out = np.pi/4+0.05
    convention = "starspec"

    ss = SeaState([
        _Spectral.Jonswap(hs=1.0, tp=10.0, gamma=3.0, heading=wave_heading, spreading_type=_Spectral.SpreadingType.Cosn, spreading_value=5.),
            ])
