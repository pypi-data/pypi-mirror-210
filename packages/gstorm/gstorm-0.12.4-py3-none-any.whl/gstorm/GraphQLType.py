...
from gstorm.helpers.typing_helpers import attrs_with_hints
...
class GraphQLType(BaseGraphQLType):
    ...
    def get_base_fields(cls):
        list_fields = [
            name for name, hint in attrs_with_hints(cls).items()
    ...
    def get_list_fields(cls):
        list_fields = [
            name
            for name, hint in attrs_with_hints(cls).items()
            if hint == List[GraphQLType]  # if type List[GraphQLType]
        ]
        return list_fields
    ...
    def get_object_fields(cls, typename=None):
        ...
        if typename:
            obj_fields = {
                ...
                if typename in hint.__forward_arg__
            }
    ...
    def get_field_types(cls):
        return {k: v for k, v in attrs_with_hints(cls).items() if not k.startswith('_')}
