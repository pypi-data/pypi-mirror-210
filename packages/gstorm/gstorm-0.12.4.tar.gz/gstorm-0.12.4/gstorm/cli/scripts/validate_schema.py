        ...
        if item['kind'] == 'TYPE':
            ...
            for field in item['fields']:
                ...
                if field['type']['kind'] == 'SCALAR':
                    ...
                    #   })
                elif field['type']['kind'] == 'OBJECT' and field['hasOne']:
                    # first of all, this type exists?
                    if not field['type']['name'] in schema['types']:
                        errors.append({
                            'type': item['name'],
                    ...
                    #   })
                elif field['type']['kind'] == 'OBJECT' and not field['hasOne']:
                    # first of all, this type exists?
                    if not field['type']['name'] in schema['types']:
                        errors.append({
                            'type': item['name'],
            ...
            schema_object_types = {tn2: i2 for tn2,
                                   i2 in schema['types'].items() if i2['kind'] == 'TYPE'}
            referring_types = [
