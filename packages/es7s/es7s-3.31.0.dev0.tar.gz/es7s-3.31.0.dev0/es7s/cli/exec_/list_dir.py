...
class ListDirCommand:
    ...
    def _process_regular_line(self, line: str) -> str:
        ...
        file = File(*splitted)
        columns = file.render(self._custom_columns, self._icon_renderer, self._rwx_map)
...
class _Styles(BaseStyles):
    ...
    OCTAL_PERMS = FrozenStyle(fg=pt.cv.MAGENTA)
...
class File(IFile):
    ...
    def render(
        ...
        if stdout.renderer.is_format_allowed:
            ...
            if self.name.startswith(".") and self.is_dir:
                filefmt += pt.ansi.SeqIndex.DIM.assemble()

        classfmt_resetters = pt.ansi.SeqIndex.BG_COLOR_OFF
        if pt.contains_sgr(filefmt, pt.ansi.SeqIndex.BLACK.params[0]):
            classfmt_resetters += pt.ansi.SeqIndex.COLOR_OFF
        classfmt = filefmt + classfmt_resetters.assemble()

        iconfmt_resetters = pt.ansi.SeqIndex.ITALIC_OFF
    ...
    def _render_perm_chars(self, idx: int, c: str, rwx_map: dict[str, dict[str, pt.FT]]) -> tuple[str, pt.FT]:
        if idx < 3:
    ...
    def _render_name(self, filefmt: str) -> str:
        if self.is_link:
