...
class CliCommand(CliBaseCommand):
    ...
    def format_help(self, ctx: Context, formatter: HelpFormatter) -> None:
        ...
        if stdout:
            stdout = pt.SGR_SEQ_REGEX.sub(
                lambda m: m.group(0)+(seq_color_bg if any(not c or c == '0' or c.startswith('4') for c in m.group(3).split(';')) else ''),
                stdout
            )
            stdout = pt.StringReplacer(re.compile('(^|\n|$)'), fr'\1{seq_color_bg_line}').apply(stdout)
            stdout += f'{pt.SeqIndex.RESET}'
