...
from .utils.utils import feature_normalize
...
class MyGenerator(nn.Module):
    ...
    # def forward(self, target_gray, target_onehot, source_rgb, source_onehot):
        ...
        for i, (target_feature, source_feature) in enumerate(zip(self.net_dict["target_rgb_features"], self.net_dict["source_rgb_features"])):
            self.net_dict["target_rgb_features"][i] = feature_normalize(target_feature)
            self.net_dict["source_rgb_features"][i] = feature_normalize(source_feature)
        ...
        self.net_dict["fake_rgb"] = self.blend_net(blend_input.clone().detach())
