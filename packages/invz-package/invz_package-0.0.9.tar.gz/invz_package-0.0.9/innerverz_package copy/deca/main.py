...
class DECA(nn.Module):
    ...
    def data_preprocess(self, image, lmk=None):
    ...
    def data_postprocess(self, visdict, size=224):
        result_dict = {}
        for key in visdict:
            _,_,h,w = visdict[key].shape
            new_h = size; new_w = int(w*size/h)
            tensor_image = F.interpolate(visdict[key], [new_h, new_w]).detach().cpu().squeeze().permute([1,2,0]).numpy()[:,:,::-1]*255
            _tensor_image = np.minimum(np.maximum(tensor_image, 0), 255).astype(np.uint8)
            result_dict[key] = _tensor_image
        return result_dict
