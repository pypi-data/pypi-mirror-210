...
from lupin_grognard.core.config import COMMIT_DELIMITER, INITIAL_COMMIT, JAMA_REGEX
...
def is_last_description_line_jama_reference(body: List[str]) -> bool:
    return bool(re.match(JAMA_REGEX, body[-1]))
...
def is_gitlab_depth_zero() -> bool:
    return os.getenv("GIT_DEPTH") == "0"


def check_gitlab_depth_zero_defined() -> None:
    """Check if GIT_DEPTH is defined to 0 in GitLab CI"""
    if is_gitlab_ci() and not is_gitlab_depth_zero():
