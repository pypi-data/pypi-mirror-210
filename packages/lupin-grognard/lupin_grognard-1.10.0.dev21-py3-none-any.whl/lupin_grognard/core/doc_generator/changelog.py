...
from lupin_grognard.core.tools.utils import (
    check_gitlab_depth_zero_defined,
    get_current_branch_name,
    info,
    is_last_description_line_jama_reference,
)
...
class Changelog(JinjaGenerator):
    ...
    def _separate_jama_ref(self, commit: Commit) -> Tuple[List[str], str]:
        ...
        """
        if commit.body:
            if is_last_description_line_jama_reference(body=commit.body):
                jama_ref = commit.body[-1]
                body_without_jama_ref = commit.body[:-1]
