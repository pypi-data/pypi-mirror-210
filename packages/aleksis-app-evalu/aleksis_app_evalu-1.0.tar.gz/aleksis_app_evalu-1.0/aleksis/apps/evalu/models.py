...
class EvaluationRegistration(ExtensibleModel):
    ...
    def results_accessible(self):
        return (self.phase.status == "results" or self.finished) and not self.deleted
...
class EvaluationGroup(ExtensibleModel):
    ...
    def get_average_results(self, private_key: RSAPrivateKey) -> List[dict]:
        ...
        results = []
        for item in EvaluationItem.get_agreement_items():
            decrypted_values = [
                r.get_result(private_key) for r in self.results.all() if r.item == item
            ]
    ...
    def get_free_text_results(self, private_key: RSAPrivateKey) -> List[dict]:
        ...
        results = []
        for item in EvaluationItem.get_free_text_items():
            decrypted_values = [
                r.get_result(private_key) for r in self.results.all() if r.item == item
            ]
...
class EvaluationItem(ExtensibleModel):
    ...
    @classmethod
    @cache_memoize(3600)
    def get_agreement_items(cls) -> List["EvaluationItem"]:
        return cls.objects.filter(item_type=QuestionType.AGREEMENT).order_by("part__order", "order")

    @classmethod
    @cache_memoize(3600)
    def get_free_text_items(cls) -> List["EvaluationItem"]:
        return cls.objects.filter(item_type=QuestionType.FREE_TEXT).order_by("part__order", "order")
