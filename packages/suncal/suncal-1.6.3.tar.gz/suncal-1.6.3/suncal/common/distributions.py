...
def get_distribution(name, **kwds):
    ''' Get an instance of the Distribution class.
        ...
        Args:
            ...
            **kwds: Arguments used to set up the distribution
    '''
    if name in _aliases:
        return _aliases[name](name, **kwds)

    if isinstance(name, (stats.rv_continuous, stats.rv_discrete, stats.rv_histogram)):
...
class Distribution:
    ''' Distribution class for handling stats distributions
        ...
        Args:
            name (string): Name of the distribution
            **kwds: Arguments passed to the scipy distribution
    '''
    showshift = False
    dist = stats.norm
    ...
    def helpstr(self):
        ...
        helpstr = '\n'.join(line for line in helpstr if not line.startswith('    %'))
        return helpstr
...
class Dt(Distribution):
    ...
    def update_kwds(self, **kwds):
        ...
        if 'unc' in self.kwds:   # UNC and DF, but no conf or K......
            if 'conf' in self.kwds:
                k = ttable.k_factor(self.kwds['conf'], df)
                std = self.kwds['unc']/k
            else:
                k = self.kwds.get('k', 1)
                std = self.kwds['unc']/k
...
class DTriangular(Distribution):
    ...
    def fit(self, x):
        ''' Find best fitting parameters for the distribution to the sampled x data. '''
        _, loc, scale = self.dist.fit(x, f0=0.5)  # Keep c parameter 0.5 for symmetric triangle
...
class DResolution(Distribution):
    ...
    def helpstr(self):
        return (''' Uncertainty due to digital equipment resolution. The 'a' parameter is\n'''
                ...
                '''Same as a uniform distribution with '''
                '''half-width a/2.''')
