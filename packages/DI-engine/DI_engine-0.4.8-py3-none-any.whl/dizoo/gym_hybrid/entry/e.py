...
from dizoo.gym_hybrid.config.gym_hybrid_hppo_config import gym_hybrid_hppo_config, gym_hybrid_hppo_create_config


def main(main_cfg, create_cfg, seed=22031):
    main_cfg.env.n_evaluator_episode = 1
    cfg = compile_config(main_cfg, seed=seed, auto=True, create_cfg=create_cfg, save_cfg=True)
    cfg.env.replay_path = './'
    cfg.env.evaluator_env_num = 1  # only 1 env for save replay
    cfg.policy.load_path = '/Users/niuyazhe/Downloads/gym-hybrid-hppo-result-1025/gym_hybrid_hppo_seed0_221025_035334/ckpt/ckpt_best.pth.tar'
    # Create main components: env, policy
    env_fn, collector_env_cfg, evaluator_env_cfg = get_vec_env_setting(cfg.env)
    evaluator_env = BaseEnvManager([partial(env_fn, cfg=c) for c in evaluator_env_cfg], cfg.env.manager)
    print(len(evaluator_env_cfg))
...
if __name__ == "__main__":
    ...
    main(gym_hybrid_hppo_config, gym_hybrid_hppo_create_config, seed=0)
