...
from ding.jax_utils import to_raw
...
def inferencer(cfg: EasyDict, agent: "Agent", env: BaseEnvManager) -> Callable:
    """
    Overview:
        The middleware that executes the inference process.
    Arguments:
        - cfg (:obj:`EasyDict`): Config.
        - agent (:obj:`Agent`): The agent to be inferred.
    ...
    def _inference(ctx: "OnlineRLContext"):
        ...
        obs = env.ready_obs.astype(np.float32)
        ctx.obs = obs
        # TODO mask necessary rollout

        inference_output = agent.collect(obs, **ctx.collect_kwargs)
        ctx.action = to_raw(inference_output['action'])
        # TODO(nyz)
        inference_output_list = [
            {k: v[i]
             for k, v in inference_output.items()} for i in range(len(inference_output['action']))
        ]
        ctx.inference_output = {i: v for i, v in zip(env.ready_obs_id, inference_output_list)}

    return _inference


def rolloutor(cfg: EasyDict, agent: "Agent", env: BaseEnvManager, transitions: TransitionList) -> Callable:
    """
    Overview:
        The middleware that executes the transition process in the env.
    Arguments:
        - cfg (:obj:`EasyDict`): Config.
        - agent (:obj:`Agent`): The agent to be used during transition.
    ...
    def _rollout(ctx: "OnlineRLContext"):
        ...
        for i, timestep in enumerate(timesteps):
            transition = agent.process_transition(ctx.obs[i], ctx.inference_output[i], timestep)
            transition = EasyDict(transition)
            transition.collect_train_iter = ctx.train_iter
            transition.env_data_id = env_episode_id[timestep.env_id]
            transitions.append(timestep.env_id, transition)
            if timestep.done:
                agent.reset([timestep.env_id])
