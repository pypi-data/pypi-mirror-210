...
class RiskDecomposition:
    ...
    def get_decomposition_stats(self) -> (pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame):
        ...
        return asset_total_var, asset_sys_variance, asset_idio_variance, factor_vol
    ...
    def weighted_portfolio(self, weights: Union[Dict, pd.DataFrame]):
        ...
        if type(weights) == 'Dict':
            stock_portfolio = pd.DataFrame(weights, index=close_daily.index)
        else:
            stock_portfolio = weights.asfreq('D', method='pad')
        stock_portfolio.index.name = 'date'
        stock_portfolio.columns.name = 'code'
        stock_portfolio = stock_portfolio.loc[self.factor_quad.beta_ts.date.drop_duplicates()]
...
class CompareReports:
    ...
    def _get_portfolio_decompsition(self):
        ...
        portfolio_w = portfolio_w.resample('BM').last()
    ...
    def get_var_rmv_vov(self) -> (pd.DataFrame, pd.DataFrame, pd.DataFrame):
        ...
        for i in range(len(self.decomp_names)):
            ...
            var = (target_ret.abs().quantile(0.99) * np.sqrt(12) / target_vol)

            # 2) Vol targeting (Histogram 2)
            rmv = (target_ret.std() * np.sqrt(12))

            # 3) Volatility of Volatility (Histogram 3)
            vov = np.sqrt(
                ((target_ret.apply(lambda x: x.dropna().rolling(window=12).std()) * np.sqrt(12) - target_vol) ** 2))
            rmv_lst.append(rmv)
            var_lst.append(var)
            vov_lst.append(vov)
    ...
    def get_portfolio_report(self) -> Dict:
        ...
        for i in range(len(protfolio_w)):
            protfolio_w[i].columns = self.decomp_names[i] + ':' + protfolio_w[i].columns
        risk_managed_performance = pd.concat(risk_managed_performance, axis=1)
        risk_managed_performance.columns = pd.Series(self.decomp_names) + ': performance'
...
def test_RiskControl():
    ...
    def test_get_simple_report(model_list: List):
        ...
        my_compare_report.get_reports(report_contents="portfolio", output_file="stock_risk_report7.xlsx")
        my_compare_report.get_reports(report_contents="full", output_file="stock_risk_report.xlsx")
        my_compare_report.get_reports(report_contents="sys_idio", output_file="stock_risk_report1.xlsx")
        my_compare_report.get_reports(report_contents="factor_vol", output_file="stock_risk_report2.xlsx")
        my_compare_report.get_reports(report_contents="volatility", output_file="stock_risk_report3.xlsx")
        my_compare_report.get_reports(report_contents="rmv_vov", output_file="stock_risk_report4.xlsx")
        my_compare_report.get_reports(report_contents="stock_vol", output_file="stock_risk_report5.xlsx")
        my_compare_report.get_reports(report_contents=["rmv_vov", "stock_vol"], output_file="stock_risk_report6.xlsx")
