...
class RoleTreeService(object):
    @staticmethod
    def role_tree(role_id=0, role_key=None):
...
class RoleService:
    role_all_fields = [i.name for i in Role._meta.fields] + ["user_group_id", "permission_id"]
    ...
    def edit_role(params: dict = None):
        ...
        params = format_params_handle(
            param_dict=params,
            filter_filed_list=[
                "id|int", "role_id|int", "role", "role_key", "role_name", "parent_role_id|int", "permission_id|int", "user_group_id|int", "description"
            ],
    ...
    @staticmethod
    def get_role_list(params: dict = None, need_pagination: bool = True, filter_fields: bool = None, **kwargs):
        """
        查询角色列表,
        ...
        sort = sort if sort in ["role_id", "role", "role_name", "user_group_id", "-role_id", "-role", "-role_name", "-user_group_id"] else "-role_id"

        params = format_params_handle(
            param_dict=params,
            filter_filed_list=[
                "role_id|int", "role", "user_group_id|int", "permission_id|int", "parent_role_id|int", "user_id|int",
                "user_group_id_list|list_int", "role_id_list|list_int", "permission_id_list|list_int", "user_id_list|list_int"
            ],
            split_list=["user_group_id_list", "role_id_list", "permission_id_list", "user_id_list"],
            alias_dict={"user_group_id_list": "user_group_id__in", "role_id_list": "role_id__in", "permission_id_list": "permission_id__in", "role": "role_key"}
        )
        ...
        if user_id or user_id_list:
            role_query_set = UserToRole.objects.annotate(
                role_key=F("role__role"),  # 字敦冲突别名搜索
                role_name=F("role__role_name"),
                parent_role_id=F("role__parent_role_id"),
                permission=F("role__permission"),
                permission_id=F("role__permission_id"),
            ...
            role_query_set = Role.objects.annotate(role_id=F("id"), role_key=F("role")).filter(**params)
        ...
        finish_set = filter_result_field(
            ...
            remove_filed_list=["id", "user_group", "permission"],
    ...
    def user_bind_role(user_id, role_id: int = None, role_value: str = None):
        ...
        role_is_set = UserToRole.objects.filter(role_id=role_id).first()
    ...
    def user_role_users(params: dict = None, is_subtree: bool = False, without_user_info: bool = False, **kwargs):
        ...
        # 数据拼接
        user_detail_list = DetailInfoService.get_list_detail(user_id_list=user_id_list, )
    ...
    def user_role_list(user_id):
        ...
        ).values('id', 'role_id', 'user_id', 'role_value', 'role_name', 'permission_id', 'permission_name', 'user_group_value', 'user_group_name'))
