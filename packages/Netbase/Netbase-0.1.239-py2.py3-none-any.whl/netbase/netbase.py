...
def property(node, edge):
	...
	elif edge['pid'] == node.id:
		return (Node(name=edge['subject'], id=edge['sid']), Node(name=edge['object'], id=edge['oid'])) # ignore predicate
		# return (Node(name=edge['subject'], id=edge['sid']), node, Node(name=edge['object'], id=edge['oid'])) # repeat predicate
	else:
...
def main():
	world = net = Netbase()
	for place, pop in world.population:
		if pop.value <=0:
			continue
		print(place, pop.value)
	exit(1)
	super = name(-1)
	print(super)
