...
def generate_stream_select_sql(catalog_entry, columns, stream_name):
    """Generate SQL to extract data froom snowflake"""
    database_name = get_database_name(catalog_entry)
    schema_name = get_schema_name(catalog_entry)
    escaped_db = escape(database_name)
    ...
    select_sql = f'SELECT {",".join(escaped_columns)} FROM {escaped_db}.{escaped_schema}.{stream_name} LIMIT 1000'
...
def sync_query(cursor, catalog_entry, state, select_sql, columns, stream_version, params, replication_method, stream_name=None):
    """..."""
    ...
    if replication_method == 'CDC' and stream_name is not None:
        # Update stream offset to latest position by running a DML query
        cursor.execute(insert_into_stream_sql(catalog_entry, stream_name, columns, rows_saved))
...
def sync_log_based_query(connection, catalog_entry, state, select_sql, columns, stream_version, params, replication_method, stream_name=None):
    ...
    rows_saved = 0
    try:
        connection.cursor().execute(select_sql, params)

        rows = connection.cursor().fetchall()
        LOGGER.info("Rows is %s", rows)
        ...
        if not rows:
            ...
            # Update stream offset to latest position by running a DML query
            connection.cursor().execute(insert_into_stream_sql(catalog_entry, stream_name, columns, 1000))
            connection.commit()
