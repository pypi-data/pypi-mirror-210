...
def connect_nodes(node1, node2, ax, edges_color, edges_width,line_style='solid'):
    (a,b) = get_edge_positions(node1, node2)
    l1 = lines.Line2D(a,b, color=edges_color, linewidth=edges_width,linestyle=line_style)
    ax.add_line(l1)

class Layer():
    def __init__(self, left, num_nodes, node_radius, title=None, spacing_nodes = 1, max_num_nodes_visible=None, color="gray", font_size=36, edges_color="lightGray", edges_width=4,fully_connect=True):
        self.nodes = []
        self.top = 0
        self.title = title
    ...
    def connect_fully(self, layer2, mAx):
        for a in self.nodes:
            for b in layer2.nodes:
                connect_nodes(a,b, mAx, self.edges_color, self.edges_width)
    
    def connect_direct(self, layer2, mAx):
        for a,b in zip(self.nodes,layer2.nodes):
            connect_nodes(a,b, mAx, self.edges_color, self.edges_width,line_style='dotted')
...
class NNV():
    def __init__(self, layers_list, spacing_layer = 60, spacing_nodes=2, align = "middle", max_num_nodes_visible = 4, node_radius = 20, font_size = 18):
        self.layers = []
        left = 0
        self.left = 0
        font_size=int(font_size*(5/len(layers_list)))
        self.spacing_layer = spacing_layer
        max_height = 0
        for l in range(len(layers_list)):
            layer = Layer(left=left, num_nodes=layers_list[l]["units"], node_radius=node_radius, title=layers_list[l]["title"], color=layers_list[l].get("color", "lightGray"), max_num_nodes_visible=max_num_nodes_visible, font_size=font_size, spacing_nodes=spacing_nodes, edges_color=layers_list[l].get("edges_color","lightGray"), edges_width=layers_list[l].get("edges_width", 3),fully_connect=layers_list[l]["fully_connect"])
            self.layers.append(layer)
            if l<len(layers_list)-1 and "spacing_layer" in layers_list[l+1]:
                left = layer.get_right() + layers_list[l+1]["spacing_layer"]
            else:
    ...
    def render(self, save_to_file=None, do_not_show=False):
        ...
        for i in range(len(self.layers) - 1):
            if self.layers[i+1].fully_connect is True:
                self.layers[i].connect_fully(self.layers[i+1], mAx)
            else:
                self.layers[i].connect_direct(self.layers[i+1], mAx)
