            ...
            for region in account.regions:
                for vpc in region.vpcs:
                    if config.configure_spoke_gw_hs:
                        if getattr(vpc, "avtx_cidr") is not None or getattr(vpc, "avtx_cidrs") is None or len(getattr(vpc, "avtx_cidrs")) == 0:
                            errors.append(
                                (account.account_id, region.region, vpc.vpc_id)
                            )
                        elif len(getattr(vpc, "avtx_cidrs")) > len(getattr(vpc, "gw_zones")):
        ...
        if errors:
            ...
            if config.configure_spoke_gw_hs:
                raise ValueError(
                    ...
                    "avtx_cidrs must be used instead of avtx_cidr. "
                    "List of nonconforming VPCs: "
                    f"{error_vpc_str}"
                )
            else:
                raise ValueError(
                    f"'config.configure_spoke_gw_hs' is {config.configure_spoke_gw_hs}, "
                    "avtx_cidr must be used insted of avtx_cidrs. "
                    "List of nonconforming VPCs: "
                    f"{error_vpc_str}"
                )
        if gw_zones_errors:
