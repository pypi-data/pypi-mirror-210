...
cdef dict check_arch_cluster_membership(cnp.uint16_t[:] aSp1IdsView, cnp.uint16_t[:] aSp2IdsView, cnp.ndarray[cnp.uint16_t, ndim=1] gSp1Orthologs, cnp.ndarray[cnp.uint16_t, ndim=1] gSp2Orthologs):
    """
...
cdef inline bint contained_in_same_g_clstr(cnp.uint16_t[:] aSpxIdsView, cnp.uint8_t[:] spxContainedIdxs,  unsigned int spxOrthoCnt, dict graphSpxOrtho2info):
    """
...
cdef inline void write_merged_clstr_to_file(dict clstrDict, str outDir, str sp1Id, str sp2Id):
    ...
    tmpClstr: MergedClstr = MergedClstr(0, np.zeros(0, dtype=np.uint16), np.zeros(0, dtype=np.uint16), np.zeros(0, dtype=np.float32), np.zeros(0, dtype=np.float32))
    cdef size_t i, j # Indexes to be used in for loops
    cdef unsigned int clstrCnt = len(clstrDict)
    cdef cnp.uint16_t[:] sp1Ids
    cdef cnp.uint16_t[:] sp2Ids
    cdef cnp.float32_t[:] sp1Scores
    cdef cnp.float32_t[:] sp2Scores
    cdef unsigned int sp1OrthoCnt
    cdef unsigned int sp2OrthoCnt
    cdef unsigned int clstrSize
    cdef unsigned int clstrId
    ...
    for i in range(clstrCnt):
        ...
        if clstrSize == 2:
            ...
            if tmpScore == 1:
                ofd.write(f"2\t2\t{sp1Id}.{str(sp1Ids[0])} 1\t{sp2Id}.{str(sp2Ids[0])} 1\n")
            else:
                # This is an arch based pair, and the value is the same for both orthologs
                ofd.write(f"2\t2\t{sp1Id}.{str(sp1Ids[0])} {tmpScore:.3f}\t{sp2Id}.{str(sp2Ids[0])} {tmpScore:.3f}\n")
...
cdef inline void write_pairs_to_file(dict clstrDict, str outDir, str sp1Id, str sp2Id, bint mergedClstrs):
    ...
    tmpClstr: MergedClstr = MergedClstr(0, np.zeros(0, dtype=np.uint16), np.zeros(0, dtype=np.uint16), np.zeros(0, dtype=np.float32), np.zeros(0, dtype=np.float32))
    if not mergedClstrs:
        tmpClstr: ArchClstr =  ArchClstr(0, np.zeros(0, dtype=np.uint16), np.zeros(0, dtype=np.uint16), [])

    # Tmp variables
    cdef size_t i, j, w # Indexes to be used in for loops
    cdef unsigned int clstrCnt = len(clstrDict)
    cdef cnp.uint16_t[:] aSp1IdsView
    cdef cnp.uint16_t[:] aSp2IdsView
    cdef unsigned int sp1OrthoCnt
    cdef unsigned int sp2OrthoCnt
    cdef unsigned int clstrId
    ...
    cdef (unsigned int, unsigned int) tmpPair = (0, 0)
    pairSet: set[int, int] = set()
    cdef unsigned int sp1OrthoInt = 0
...
cdef inline list extract_arch_clstr_cosine_vals(bytes rawScoresStr, unsigned int grpLenSp1, unsigned int grpLenSp2):
    """
...
cdef (unsigned int, unsigned int) filter_and_add_arch_ortho2g_clstr(cnp.uint16_t[:] aSp1IdsView, cnp.uint16_t[:] aSp2IdsView, cnp.float32_t[:] aSp1Scores, cnp.float32_t[:] aSp2Scores, cnp.uint8_t[:] aSp1ContainedIdxs, cnp.uint8_t[:] aSp2ContainedIdxs, dict mClstrDict, unsigned int gClstrId, dict archMasterDict, unsigned int sp1Id, unsigned int sp2Id, double covThr, bint allowDomCntMismatch):
    """
    ...
    # Tmp variables
    cdef size_t i, j # Indexes to be used in for loops
    cdef unsigned int sp1OrthoCnt = aSp1IdsView.shape[0]
    cdef unsigned int sp2OrthoCnt = aSp2IdsView.shape[0]
    # If 1 the arch-cluster should be ignored
    cdef bint ignoreClstr = 0
    cdef double sp1ArchCov
    cdef double sp2ArchCov
    cdef unsigned int sp1ArchDomCnt = 0
    cdef unsigned int sp2ArchDomCnt = 0
    ...
    cdef (unsigned int, unsigned int) clusterModOutcome = (0, 0)

    tmpSp1Arch: Arch = Arch(0, 0.0, 0, 0, np.zeros(0, dtype=np.uint16), np.zeros(0, dtype=np.uint16), np.zeros(1, dtype=np.uint16), np.array(["sp1"], dtype=np.str_), np.zeros(0, dtype=np.float32), 1)
    tmpSp2Arch: Arch = Arch(0, 0.0, 0, 0, np.zeros(0, dtype=np.uint16), np.zeros(0, dtype=np.uint16), np.zeros(0, dtype=np.uint16), np.array(["sp2"], dtype=np.str_), np.zeros(0, dtype=np.float32), 1)
    ...
    cdef unsigned int sp1ToRemoveCnt = sum_bint_array(toRemoveSp1Ids)
    cdef unsigned int sp2ToRemoveCnt = sum_bint_array(toRemoveSp2Ids)

    # Number of new orthologs that wil be added to the G cluster
    cdef unsigned int sp1ToAddCnt = sp1OrthoCnt - sum_bint_array(aSp1ContainedIdxs) - sp1ToRemoveCnt
    cdef unsigned int sp2ToAddCnt = sp2OrthoCnt - sum_bint_array(aSp2ContainedIdxs) - sp2ToRemoveCnt
    ...
    cdef cnp.ndarray[cnp.uint16_t, ndim=1] newGSp1Ids
    cdef cnp.ndarray[cnp.uint16_t, ndim=1] newGSp2Ids
    cdef cnp.ndarray[cnp.float32_t, ndim=1] newGSp1Scores
    cdef cnp.ndarray[cnp.float32_t, ndim=1] newGSp2Scores
    cdef unsigned int newGSp1OrthoCnt = 0
    cdef unsigned int newGSp2OrthoCnt = 0
    cdef unsigned int tmpGorthoCnt = 0

    # Tmp views to store the original vectors of the G cluster
    cdef cnp.uint16_t[:] oldGSpxIds
    cdef cnp.float32_t[:] oldGSpxScores
    # To remember at what index to write
    cdef unsigned short wtPosition
    ...
    if sp1ToAddCnt > 0:
        ...
        newGSp1Ids = np.zeros(newGSp1OrthoCnt, dtype=np.uint16)
    ...
    if sp2ToAddCnt > 0:
        ...
        # initiliaze the new vectors
        newGSp2Ids = np.zeros(newGSp2OrthoCnt, dtype=np.uint16)
...
# HACK: when cython will support DataClasses as paramters in cdef functions
# reduce the number of paramaters and directly passing the DataClass objects (e.g., ArchClstr)
cdef (unsigned int, unsigned int) filter_and_add_new_arch_clstr(cnp.uint16_t[:] aSp1IdsView, cnp.uint16_t[:] aSp2IdsView, cnp.float32_t[:] aSp1Scores, cnp.float32_t[:] aSp2Scores, dict mClstrDict, dict archMasterDict, unsigned int sp1Id, unsigned int sp2Id, double covThr):
    """
    ...
    cdef unsigned int sp2OrthoCnt = aSp2IdsView.shape[0]
    cdef unsigned int clstrSize = sp1OrthoCnt + sp2OrthoCnt
    cdef unsigned int newClstrId = len(mClstrDict) + 1
    ...
    # Variables used to create a modified version of the Arch cluster
    # to be added to a graph based cluster
    cdef cnp.ndarray[cnp.uint16_t, ndim=1] newSp1Ids
    cdef cnp.ndarray[cnp.uint16_t, ndim=1] newSp2Ids
    cdef cnp.ndarray[cnp.float32_t, ndim=1] newSp1Scores
    cdef cnp.ndarray[cnp.float32_t, ndim=1] newSp2Scores
    cdef unsigned int sp1ToRemoveCnt = sum_bint_array(toRemoveSp1Ids)
    cdef unsigned int sp2ToRemoveCnt = sum_bint_array(toRemoveSp2Ids)
    cdef unsigned int orthoToRemoveCnt = sp1ToRemoveCnt + sp2ToRemoveCnt

    cdef cnp.ndarray[cnp.float32_t, ndim=1] tmpScArray
    # To remember at what index to write
    cdef unsigned short wtPosition = 0
    ...
    # 1 -> add with modifications (remove only some orthologs)
        ...
        if sp1ToRemoveCnt > 0:
            ...
            newSp1Ids = np.zeros(sp1OrthoCnt - sp1ToRemoveCnt, dtype=np.uint16)
        ...
        if sp2ToRemoveCnt > 0:
            newSp2Ids = np.zeros(sp2OrthoCnt - sp2ToRemoveCnt, dtype=np.uint16)
...
# @cython.boundscheck(False) # Check array bounds
# @cython.profile(False)
# cdef dict load_arch_orto_pckls(str pcklPath):
def load_arch_ortho_pckl(pcklPath:str) -> dict[int, ArchClstr]:
    ...
    cdef unsigned int clstrCnt = len(aClstrDict)
    # print(f"Loaded arch clusters:\t{clstrCnt}")

    tmpArchClstr: ArchClstr = ArchClstr(0, np.zeros(0, dtype=np.uint16), np.zeros(0, dtype=np.uint16), [])
    # cdef cnp.uint16_t[:] gSpxOrthologs
    # cdef cnp.float32_t[:] gSpxScores
    # cdef cnp.ndarray[cnp.uint16_t, ndim=1] newGSpxOrthologs
    # cdef cnp.ndarray[cnp.float32_t, ndim=1] newGSpxScores
    cdef cnp.uint16_t[:] sp1Ids
    # cdef cnp.uint16_t[:] sp2Ids

    # Go through the orhtolog cluster to identify repetitions
    # for i in range(clstrCnt):
...
cdef dict load_arch_orto_tbl(str tblPath):
    ...
    while True:
        ...
        # create ArchClstr objects
        aClstrDict[clstrId] = ArchClstr(clstrSize, np.array(ortoListSp1, dtype=np.uint16), np.array(ortoListSp2, dtype=np.uint16), scoreArrays)
...
cdef object load_graph_orto_tbl(str tblPath):
    """
    ...
    cdef unsigned int clstrId = 0
    cdef unsigned int pairsCnt = 0
    cdef unsigned int tmpProtId
    cdef size_t i # Indexes to be used in for loops
    cdef unsigned gGrpLenSp1, gGrpLenSp2, gClstrSize
    ...
    while True:
        ...
        mClstrDict[clstrId] = MergedClstr(gClstrSize, np.array(orthoListSp1, dtype=np.uint16), np.array(orthoListSp2, dtype=np.uint16), np.array(orthoScoresSp1, dtype=np.float32), np.array(orthoScoresSp2, dtype=np.float32))
...
cdef (unsigned int, unsigned int, unsigned int, unsigned int, unsigned int, unsigned int, unsigned int, unsigned int) integrate_arch_ortho_into_gclstr(dict aClstrDict, dict mClstrDict, dict graphSp1Ortho2info, dict graphSp2Ortho2info, unsigned int sp1Id,  unsigned int sp2Id, dict archMasterDict, double covThr):
    """
    ...
    '''
    cdef bint debug = 0
    debugStr: str = f"""integrate_arch_ortho_into_graph_clusters :: START
    Arch-based clusters:\t{len(aClstrDict)}
    Graph-based clusters:\t{len(mClstrDict)}
    Graph ortho from Sp1:\t{len(graphSp1Ortho2info)}
    Graph ortho from Sp2:\t{len(graphSp2Ortho2info)}
    Sp1 ID:\t{sp1Id}
    Sp2 ID:\t{sp2Id}
    ...
    # Arch based clusters have the following content
     # size=3 -> int
     # sp1Ids = array([4368], dtype=uint16)
     # sp2Ids = array([14856, 17428], dtype=uint16)
     # cosineValues=[array([0.68, 0.74], dtype=float32)])

    # Tmp variables
    cdef size_t i # Indexes to be used in for loops
    # Variables to store info on arch-based clusters
    cdef unsigned int aClstrSize = 0
    cdef unsigned int aClstrId = 0
    cdef unsigned int aClstrCnt = len(aClstrDict)
    cdef unsigned int mClstrCnt = len(mClstrDict)
    cdef unsigned int finalClstrCnt
    ...
    tmpAclstr: ArchClstr = ArchClstr(0, np.zeros(0, dtype=np.uint16), np.zeros(0, dtype=np.uint16), [])

    # Variables to store info on graph-based clusters
    cdef cnp.ndarray[cnp.uint16_t, ndim=1] gSp1Orthologs = np.array(list(graphSp1Ortho2info.keys()), dtype=np.uint16)
    cdef cnp.ndarray[cnp.uint16_t, ndim=1] gSp2Orthologs = np.array(list(graphSp2Ortho2info.keys()), dtype=np.uint16)
    cdef unsigned int gSp1OrthoCnt = gSp1Orthologs.shape[0]
    cdef unsigned int gSp2OrthoCnt = gSp2Orthologs.shape[0]
    ...
    cdef unsigned int newAddedClstrsCnt = 0
    # Case II: Completely contained in G
    cdef unsigned int completelyContainedClstrsCnt = 0
    # Cases I and II mixed:
    #  one part of the cluster is completely new while the other is in G
    cdef unsigned int newAndOldClstrsCnt = 0

    # DEBUG ONLY
    cdef unsigned int addNewCnt = 0
    cdef unsigned int addNewModCnt = 0
    cdef unsigned int rejectNewCnt = 0
    # Case I: (1, 1)
    # cdef unsigned int addOldCnt = 0
    # cdef unsigned int addOldModCnt = 0
    cdef unsigned int rejectContainedCnt = 0
    # Cases I and II: (1, 0); (0, 1)
    cdef unsigned int addNewAndOldCnt = 0
    cdef unsigned int addNewAndOldModCnt = 0
    cdef unsigned int rejectNewAndOldCnt = 0
    # Cases I and II and III mixed: (2, 2), (1, 2), (2, 1), (0, 2), (2, 0)
    cdef unsigned int mixedClstrsCnt = 0
    cdef unsigned int addMixedCnt = 0
    cdef unsigned int addMixedModCnt = 0
    cdef unsigned int addMixedModAllNewCnt = 0 # modification resulted in clstr of all new orthologs
    cdef unsigned int rejectMixedCnt = 0
    ...
    for i in range(aClstrCnt):
        ...
        # (0, 1): all members from sp1 are new, those from sp1 completely contained
    ...
    cdef unsigned int newlyAddedCnt = addNewCnt + addNewModCnt + addMixedModAllNewCnt
    cdef unsigned int gClstrModCnt = addNewAndOldCnt + addMixedModCnt + addMixedCnt
    cdef unsigned int aClstrRejectCnt = rejectNewCnt + rejectContainedCnt + rejectNewAndOldCnt + rejectMixedCnt

    cdef (unsigned int, unsigned int, unsigned int, unsigned int, unsigned int, unsigned int, unsigned int, unsigned int) outTpl = (sp1Id, sp2Id, mClstrCnt, aClstrCnt, finalClstrCnt, newlyAddedCnt, gClstrModCnt, aClstrRejectCnt)
...
# HACK: when cython will support DataClasses as paramters in cdef functions
# reduce the number of paramaters and directly passing the DataClass objects (e.g., ArchClstr)
cdef inline (unsigned int, unsigned int) integrate_contained_and_new_arch_clstr(unsigned int aClstrId, unsigned int aClstrSize, dict aClstrDict, cnp.uint16_t[:] aSp1IdsView, cnp.uint16_t[:] aSp2IdsView, list cosineValsList, dict mClstrDict, dict archMasterDict, unsigned int sp1Id, unsigned int sp2Id, dict graphSp1Ortho2info, dict graphSp2Ortho2info, dict membershipDict, double covThr):
    """
    ...
    cdef unsigned int sp1OrthoCnt = aSp1IdsView.shape[0]
    cdef unsigned int sp2OrthoCnt = aSp2IdsView.shape[0]
    cdef unsigned int gClstrSize = 0
    cdef unsigned int gSpxOrthoCnt
    cdef unsigned int gClstrId = 0
    ...
    cdef (unsigned int, double) tmpGorthoInfo
    ...
    tmpMclstr: MergedClstr = MergedClstr(0, np.array(0, dtype=np.uint16), np.array(0, dtype=np.uint16), np.array(0, dtype=np.float32), np.array(0, dtype=np.float32))
    cdef double sp1ArchCov
    cdef double sp2ArchCov
    cdef unsigned int spxId = 0
    cdef unsigned int sp1ArchDomCnt = 0
    cdef unsigned int sp2ArchDomCnt = 0
    ...
    cdef cnp.uint16_t[:] aSpxNewIdsView
    cdef cnp.uint16_t[:] aSpxContainedIdsView

    tmpSp1Arch: Arch = Arch(0, 0.0, 0, 0, np.zeros(1, dtype=np.uint16), np.zeros(1, dtype=np.uint16), np.zeros(1, dtype=np.uint16), np.array(["sp1"], dtype=np.str_), np.zeros(1, dtype=np.float32), 1)
    tmpSp2Arch: Arch = Arch(0, 0.0, 0, 0, np.zeros(1, dtype=np.uint16), np.zeros(1, dtype=np.uint16), np.zeros(1, dtype=np.uint16), np.array(["sp2"], dtype=np.str_), np.zeros(1, dtype=np.float32), 1)
        ...
        if not ignoreClstr:
            ...
            if tmpGorthoScore < 0.05:
                ...
                if sp1HasNewMembers:
                    ...
                    newGSpxOrthologs = np.zeros(gSpxOrthoCnt + 1, dtype=np.uint16)
...
# HACK: when cython will support DataClasses as paramters in cdef functions
# reduce the number of paramaters and directly passing the DataClass objects (e.g., ArchClstr)
cdef inline (unsigned int, unsigned int) integrate_contained_arch_clstr(unsigned int aClstrId, unsigned int aClstrSize, dict aClstrDict, cnp.uint16_t[:] aSp1IdsView, cnp.uint16_t[:] aSp2IdsView, list cosineValsList, dict archMasterDict, unsigned int sp1Id, unsigned int sp2Id, dict graphSp1Ortho2info, dict graphSp2Ortho2info):
    """
        Given an arch-based cluster with members already in G, integrate it into G if conditions are met.
    """

    cdef (unsigned int, unsigned int) clusterModOutcome = (0, aClstrId)
    ...
    tmpSp1Arch: Arch = Arch(0, 0.0, 0, 0, np.zeros(0, dtype=np.uint16), np.zeros(0, dtype=np.uint16), np.zeros(0, dtype=np.uint16), np.array(["sp1"], dtype=np.str_), np.zeros(0, dtype=np.float32), 1)
    tmpSp2Arch: Arch = Arch(0, 0.0, 0, 0, np.zeros(0, dtype=np.uint16), np.zeros(0, dtype=np.uint16), np.zeros(0, dtype=np.uint16), np.array(["sp2"], dtype=np.str_), np.zeros(0, dtype=np.float32), 1)
...
cdef inline (unsigned int, unsigned int) integrate_mixed_arch_clstr(cnp.uint16_t[:] aSp1IdsView, cnp.uint16_t[:] aSp2IdsView, list cosineValsList, dict mClstrDict, dict archMasterDict, unsigned int sp1Id, unsigned int sp2Id, dict graphSp1Ortho2info, dict graphSp2Ortho2info, dict membershipDict, double covThr):
    """
    ...
    cdef cnp.uint16_t[:] aSpxAllNewIdsView
    cdef cnp.uint16_t[:] aSpxMixedIdsView
...
cdef inline (unsigned int, unsigned int) integrate_mixed_arch_orthologs_into_g_clstr(cnp.uint16_t[:] aSp1IdsView, cnp.uint16_t[:] aSp2IdsView, unsigned int aSp1Mem, unsigned int aSp2Mem, cnp.uint8_t[:] aSp1ContainedIdxs, cnp.uint8_t[:] aSp2ContainedIdxs, list cosineValsList, dict mClstrDict, dict archMasterDict, dict graphSp1Ortho2info, dict graphSp2Ortho2info, unsigned int sp1Id,  unsigned int sp2Id, double covThr):
    '''
    ...
    cdef cnp.ndarray[cnp.uint16_t, ndim=1] aSpxMixedModIds
...
cdef inline (unsigned int, unsigned int) integrate_new_and_mixed_arch_orthologs_into_g_clstr(cnp.uint16_t[:] aSpxAllNewIdsView, cnp.uint16_t[:] aSpxMixedIdsView, cnp.uint8_t[:] aSp1ContainedIdxs, cnp.uint8_t[:] aSp2ContainedIdxs, list cosineValsList, dict mClstrDict, dict archMasterDict, dict graphSpxOrtho2infoAllNew, dict graphSpxOrtho2infoMixed, unsigned int sp1Id,  unsigned int sp2Id, bint sp1HasOnlyNewMembers, double covThr):
    '''
    ...
    cdef unsigned int aSpxAllNewOrthoCnt = aSpxAllNewIdsView.shape[0]
    cdef unsigned int aSpxMixedOrthoCnt = aSpxMixedIdsView.shape[0]
    ...
    cdef unsigned int aSpxContainedCnt = sum_bint_array(aSpxContainedIdxs)
...
# HACK: when cython will support DataClasses as paramters in cdef functions
# reduce the number of paramaters and directly passing the DataClass objects (e.g., ArchClstr)
cdef inline (unsigned int, unsigned int) integrate_new_arch_clstr(unsigned int aClstrId, unsigned int aClstrSize, dict aClstrDict, cnp.uint16_t[:] aSp1IdsView, cnp.uint16_t[:] aSp2IdsView, list cosineValsList, dict mClstrDict, dict archMasterDict, unsigned int sp1Id, unsigned int sp2Id, double covThr):
    """
        Given an arch-based cluster that is not in G, integrate it into G if conditions are met.
    """
    ...
    cdef unsigned int sp1ToRemoveCnt = len(toRemoveSp1Ids)
    cdef unsigned int sp2ToRemoveCnt = len(toRemoveSp2Ids)
    cdef unsigned int orthoToRemoveCnt = sp1ToRemoveCnt + sp2ToRemoveCnt
    cdef cnp.ndarray[cnp.float32_t, ndim=1] tmpScArray
    ...
    # 1 -> add with modifications (remove only some orthologs)
        ...
        #     sys.exit(f"The cluster ID {newClstrId} already exists in the Master Cluster dictionary.")
        # Add the new cluster
        mClstrDict[newClstrId] = MergedClstr(aClstrSize, np.array(newSp1Ids, dtype=np.uint16), np.array(newSp2Ids, dtype=np.uint16), avgCosineVals[0], avgCosineVals[1])
        # print(f"New (modified) Graph-cluster [ID: {newClstrId}]:\n{mClstrDict[newClstrId]}")
...
cdef inline (unsigned int, unsigned int) integrate_new_arch_orthologs_into_g_clstr(unsigned int sp1OrthoCnt, unsigned int sp2OrthoCnt, cnp.uint16_t[:] aSpxNewIdsView,  cnp.uint16_t[:] aSpxContainedIdsView, list cosineValsList, dict mClstrDict, dict archMasterDict, unsigned int spxId, dict graphSpxOrtho2info2Use, dict graphSpxOrtho2info2Mod, bint sp1HasNewMembers, double covThr):
    '''
    ...
    cdef unsigned int arrayIdxSentinel = 0
    cdef unsigned int newOrthologsCnt = aSpxNewIdsView.shape[0]
    cdef unsigned int newOrthoToAddCnt = 0
    cdef unsigned int containedInGorthoCnt = aSpxContainedIdsView.shape[0]
    cdef (unsigned int, double) tmpGorthoInfo
    ...
    cdef double spxArchCov
    tmpSpxArch: Arch = Arch(0, 0.0, 0, 0, np.zeros(0, dtype=np.uint16), np.zeros(0, dtype=np.uint16), np.zeros(0, dtype=np.uint16), np.array(["sp1"], dtype=np.str_), np.zeros(0, dtype=np.float32), 1)
    # Merged cluster to be used for modification
    tmpMclstr: MergedClstr = MergedClstr(0, np.array(0, dtype=np.uint16), np.array(0, dtype=np.uint16), np.array(0, dtype=np.float32), np.array(0, dtype=np.float32))
    # Output tuple from the function
    cdef (unsigned int, unsigned int) clusterModOutcome

    # print(f"Contained in G:\t{containedInGorthoCnt}")

    # Do differetn things depending on the number
    # of orthologs in G and if they are in different clusters
    if containedInGorthoCnt > 1:
        # print(f"Contained in G (many):\t{containedInGorthoCnt}")
        ...
        # The orthologs already contained in G are all in the same cluster
            ...
            if newOrthologsCnt == 1:
                ...
                if spxArchCov < covThr:
                    ...
                    if sp1HasNewMembers:
                        ...
                        # print(np.frombuffer(gSpxOrthologs, dtype=np.uint16))
            ...
            # many-to-many case (multiple orhtolog to add)
                ...
                if newOrthoToAddCnt == 0:
                    ...
                    # inizialize the new arrays
                    newGSpxOrthologs = np.zeros(gSpxOrthoCnt + newOrthoToAddCnt, dtype=np.uint16)
...
cdef inline list process_arch_ortho_subgroup(bytes subgrp):
    ...
    cdef unsigned int grpSize = len(flds)
    ...
    for i in range(grpSize):
        ...
        rawGeneId = flds[i]
        gene = atoi(rawGeneId.split(b".", 1)[1])
        geneIds.append(gene)
...
cdef inline dict remove_contained_in_g(cnp.uint16_t[:] aSpxMixedIdsView, cnp.float32_t[:] aSpxMixedGraphAvgScores, cnp.uint8_t[:] aSpxContainedIdxsView, unsigned int aSpxMixedOrthoCnt, unsigned int aSpxContainedCnt):
    """
    Remove orthologs that are already contained in G clusters.
    """

    # Create the arrays to be returned
    cdef unsigned int outArrayLen = aSpxMixedOrthoCnt - aSpxContainedCnt
    cdef cnp.ndarray[cnp.uint16_t, ndim=1] aSpxModIds = np.zeros(outArrayLen, dtype=np.uint16)
    cdef cnp.ndarray[cnp.float32_t, ndim=1] aSpxModGraphAvgScores = np.zeros(outArrayLen, dtype=np.float32)
    # Counters
    cdef size_t i
    cdef unsigned short modOrthoCnt = 0
    cdef unsigned int tmpOrthoId
...
cdef inline dict remove_repeaded_arch_orthologs(dict aClstrDict, dict sp1Rep2Clstr) except *:
    ...
    # Tmp vars
    # cdef unsigned int pairsCnt = 0
    cdef unsigned int prevClstrCnt = len(aClstrDict)
    cdef unsigned int newClstrCnt = 0
    cdef unsigned int totRepeated = len(sp1Rep2Clstr)
    cdef unsigned int clstrSize
    cdef unsigned int tmpPairsCnt
    cdef unsigned short clstrsToProcess
    cdef unsigned int tmpOrthoId
    cdef unsigned int tmpClstrId
    cdef unsigned int tmpOrthoIdx = 0 # initiliazed to hanled cython warning
    cdef unsigned int tmpSp1ClstrSize
    cdef unsigned int tmpSp2ClstrSize
    ...
    cdef unsigned int rep2keepClstrIdx = 0
    ...
    '''
    cdef cnp.ndarray[cnp.uint16_t, ndim=1] nzRows = np.zeros(0, dtype=np.uint16)
    cdef cnp.ndarray[cnp.uint16_t, ndim=1] nzCols = np.zeros(nnzCnt, dtype=np.uint16)
    cdef cnp.ndarray[cnp.uint16_t, ndim=1] candIdx = np.zeros(0, dtype=np.uint16)
    '''
    # memory views for temporary vectors with scores and ids
    cdef cnp.uint16_t[:] sp1IdsView
    ...
    tmpClstr: ArchClstr = ArchClstr(0, np.zeros(0, dtype=np.uint16), np.zeros(0, dtype=np.uint16), clstrsWithReps)
    ...
    clstrsWithReps.sort()
    clstrsToProcess = len(clstrsWithReps)
    cdef unsigned int removedClstrCnt = 0
    cdef unsigned int removedPairsCnt = 0
    cdef unsigned int repToRemoveCnt = 0
    cdef unsigned int newClstrSize = 0
    cdef unsigned int currentListLen = 0
    ...
    for i in range(clstrsToProcess):
        ...
        # Modify the original cluster
            ...
            setattr(tmpClstr, "sp1Ids", np.array(tmpOrthoIds, dtype=np.uint16))
...
cdef tuple process_graph_ortho_subgroup(bytes subgrp):
    """
...
cpdef inline double avg_array_double(cnp.float32_t[:] valuesToAverage) nogil:
    ...
    cdef unsigned int arrayLen = valuesToAverage.shape[0]
...
@cython.wraparound(False) # Do not chek for negative array indexes
@cython.boundscheck(False)  # Deactivate bounds checking
cpdef inline unsigned int sum_bint_array(cnp.uint8_t[:] valuesToSum) nogil:

    cdef size_t i
    cdef unsigned int result = 0
    cdef unsigned int arrayLen = valuesToSum.shape[0]
