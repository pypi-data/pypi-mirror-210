...
from pylacrosse_ha import (LaCrosse_ha, LaCrosseSensor_ha)
...
class TestLacrosse(object):
    ...
    def test_refresh(self):
        ...
        l = LaCrosse_ha('/dev/ttyTEST', 115200)
    ...
    def test_get_info(self):
        info = LaCrosse_ha._parse_info('[LaCrosseITPlusReader.10.1s (RFM12B f:0 r:17241)]')
        ...
        info = LaCrosse_ha._parse_info('[LaCrosseITPlusReader.10.1s (RFM12B f:0 t:10~3)]')
        eq_(info['rfm1name'], 'RFM12B')
        eq_(info['rfm1frequency'], '0')
...
class TestLaCrosseSensor(object):
    ...
    def test_init(self):
        s = LaCrosseSensor_ha('OK 9 1 1 4 150 66')
        eq_(s.sensorid, 1)
        eq_(s.sensortype, 1)
