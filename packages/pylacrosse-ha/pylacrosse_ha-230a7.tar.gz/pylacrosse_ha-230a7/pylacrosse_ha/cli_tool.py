...
def configure(lacrosse_ha, config, args):
    if args.frequency_rfm1:
        lacrosse_ha.set_frequency(args.frequency_rfm1, 1)
    if args.frequency_rfm2:
        lacrosse_ha.set_frequency(args.frequency_rfm2, 2)

    if args.datarate_rfm1:
        lacrosse_ha.set_datarate(args.datarate_rfm1, 1)
    if args.datarate_rfm2:
        lacrosse_ha.set_datarate(args.datarate_rfm1, 2)

    if args.toggle_mask_rfm1:
        lacrosse_ha.set_toggle_mask(args.toggle_mask_rfm1, 1)
    if args.toggle_mask_rfm2:
        lacrosse_ha.set_toggle_mask(args.toggle_mask_rfm2, 2)

    if args.toggle_interval_rfm1:
        lacrosse_ha.set_toggle_interval(args.toggle_interval_rfm1, 1)
    if args.toggle_interval_rfm2:
        lacrosse_ha.set_toggle_interval(args.toggle_interval_rfm1, 2)


def scan(lacrosse_ha, config, args):
    lacrosse_ha.register_all(scan_callback, user_data=config)
    lacrosse_ha.start_scan()
    while True:
        time.sleep(1)

def get_info(lacrosse_ha, config, args):
    info = lacrosse_ha.get_info()
...
def led(lacrosse_ha, config, args):
    state = args.led_state.lower() == 'on'
    lacrosse_ha.led_config(state)
...
def main(args=None):
    ...
    try:
        ...
        config.readfp(codecs.open(os.path.expanduser(
                '~/.lacrosse_ha/known_sensors.ini'), 'r', 'UTF-8'))
    except IOError:
        config = None

    lacrosse_ha = None
    try:
        lacrosse_ha = pylacrosse_ha.LaCrosse_ha(args.device, 57600)
        lacrosse_ha.open()
        configure(lacrosse_ha, config, args)
        args.func(lacrosse_ha, config, args)

    finally:
        if lacrosse_ha is not None:
            lacrosse_ha.close()
