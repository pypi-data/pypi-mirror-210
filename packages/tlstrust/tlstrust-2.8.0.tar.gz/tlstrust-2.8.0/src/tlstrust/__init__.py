...
from .stores.cloudflare import UNTRUSTED as CLOUDFLARE_UNTRUSTED, PEM_FILES as CLOUDFLARE_PEM_FILES
from .stores.cloudflare_ios import UNTRUSTED as CFSSL_IOS_UNTRUSTED, PEM_FILES as CFSSL_IOS_PEM_FILES
from .stores.cloudflare_osx import UNTRUSTED as CFSSL_OSX_UNTRUSTED, PEM_FILES as CFSSL_OSX_PEM_FILES
from .stores.cloudflare_nss import UNTRUSTED as CFSSL_NSS_UNTRUSTED, PEM_FILES as CFSSL_NSS_PEM_FILES
from .stores.cloudflare_ubuntu import UNTRUSTED as CFSSL_UBUNTU_UNTRUSTED, PEM_FILES as CFSSL_UBUNTU_PEM_FILES
from .stores.cloudflare_windows import UNTRUSTED as CFSSL_WIN_UNTRUSTED, PEM_FILES as CFSSL_WIN_PEM_FILES
...
class TrustStore:
    ...
    def cfssl(self) -> bool:
        untrusted = set(
            CLOUDFLARE_UNTRUSTED
            + CFSSL_IOS_UNTRUSTED
            + CFSSL_OSX_UNTRUSTED
            + CFSSL_NSS_UNTRUSTED
            + CFSSL_UBUNTU_UNTRUSTED
            + CFSSL_WIN_UNTRUSTED
        )
        try:
            return (
                ...
                and isinstance(self.certificate, X509)
                and not self.expired_in_store(SOURCE_CFSSL)
            )
        except FileExistsError:
            return False
    ...
    def exists(self, context_type: int) -> bool:
        ...
        if (
            context_type == SOURCE_CFSSL
            and self.key_identifier in CLOUDFLARE_PEM_FILES.keys()
        ):
            return match_certificate(
                self.key_identifier,
                get_certificate_from_store(self.key_identifier, SOURCE_CFSSL),
            )
        if (
            context_type == PLATFORM_CFSSL_IOS
            and self.key_identifier in CFSSL_IOS_PEM_FILES.keys()
        ):
            return match_certificate(
                self.key_identifier,
                get_certificate_from_store(self.key_identifier, PLATFORM_CFSSL_IOS),
            )
        if (
            context_type == PLATFORM_CFSSL_OSX
            and self.key_identifier in CFSSL_OSX_PEM_FILES.keys()
        ):
            return match_certificate(
                self.key_identifier,
                get_certificate_from_store(self.key_identifier, PLATFORM_CFSSL_OSX),
            )
        if (
            context_type == PLATFORM_CFSSL_NSS
            and self.key_identifier in CFSSL_NSS_PEM_FILES.keys()
        ):
            return match_certificate(
                self.key_identifier,
                get_certificate_from_store(self.key_identifier, PLATFORM_CFSSL_NSS),
            )
        if (
            context_type == PLATFORM_CFSSL_UBUNTU
            and self.key_identifier in CFSSL_UBUNTU_PEM_FILES.keys()
        ):
            return match_certificate(
                self.key_identifier,
                get_certificate_from_store(self.key_identifier, PLATFORM_CFSSL_UBUNTU),
            )
        if (
            context_type == PLATFORM_CFSSL_WINDOWS
            and self.key_identifier in CFSSL_WIN_PEM_FILES.keys()
        ):
            return match_certificate(
                self.key_identifier,
                get_certificate_from_store(self.key_identifier, PLATFORM_CFSSL_WINDOWS),
            )
        if (
    ...
    def check_trust(self, context_type: int = None) -> bool:
        ...
        if context_type == SOURCE_CURL:
            return self.curl
        if context_type == PLATFORM_CFSSL_IOS:
            return self.cfssl
        if context_type == PLATFORM_CFSSL_OSX:
            return self.cfssl
        if context_type == PLATFORM_CFSSL_NSS:
            return self.cfssl
        if context_type == PLATFORM_CFSSL_UBUNTU:
            return self.cfssl
        if context_type == PLATFORM_CFSSL_WINDOWS:
            return self.cfssl
        return self.dart if context_type == SOURCE_DART else self.is_trusted
