...
from .stores.cloudflare import PEM_FILES as CLOUDFLARE_PEM_FILES
from .stores.cloudflare_ios import PEM_FILES as CFSSL_IOS_PEM_FILES
from .stores.cloudflare_osx import PEM_FILES as CFSSL_OSX_PEM_FILES
from .stores.cloudflare_nss import PEM_FILES as CFSSL_NSS_PEM_FILES
from .stores.cloudflare_ubuntu import PEM_FILES as CFSSL_UBUNTU_PEM_FILES
from .stores.cloudflare_windows import PEM_FILES as CFSSL_WIN_PEM_FILES
...
def get_certificate_chain(
    ...
    for method in [
        ...
        try:
            ...
            certificate_chain.extend(iter(conn.get_peer_cert_chain()))
            conn.shutdown()
...
def get_certificate_from_store(aki, context_type: int) -> X509:
    ...
    with contextlib.suppress(KeyError):
        ...
        if context_type == SOURCE_CFSSL:
            certificate = load_certificate(FILETYPE_PEM, CLOUDFLARE_PEM_FILES[aki].encode())
        ...
        if context_type == PLATFORM_CFSSL_IOS:
            certificate = load_certificate(FILETYPE_PEM, CFSSL_IOS_PEM_FILES[aki].encode())
        if context_type == PLATFORM_CFSSL_OSX:
            certificate = load_certificate(FILETYPE_PEM, CFSSL_OSX_PEM_FILES[aki].encode())
        if context_type == PLATFORM_CFSSL_NSS:
            certificate = load_certificate(FILETYPE_PEM, CFSSL_NSS_PEM_FILES[aki].encode())
        if context_type == PLATFORM_CFSSL_WINDOWS:
            certificate = load_certificate(FILETYPE_PEM, CFSSL_WIN_PEM_FILES[aki].encode())
        if context_type == PLATFORM_CFSSL_UBUNTU:
            certificate = load_certificate(FILETYPE_PEM, CFSSL_UBUNTU_PEM_FILES[aki].encode())
...
def do_handshake(conn):
    with contextlib.suppress(SSL.SysCallError):
        conn.do_handshake()
