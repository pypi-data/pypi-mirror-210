...
class TestEnvironment:
    ...
    def __str__(self):
        return '\n'.join(str(s) for s in self._sensors)
    ...
    def _read_configuration_file(self, configuration_file):
        ...
        # used for a crude validity check
        ...
        with open(configuration_file, "r", encoding="utf-8") as csvfile:
            for row in csvfile:
                ...
                if lower_bound <= rowlen <= upper_bound:
                    ...
                    parts = (
                        x if x else None
                        for x, _ in itertools.zip_longest(row, range(upper_bound))
                    )
    ...
    def _report_duplicate_sensor_names(self):
        ...
        if duplicates:
            retval = False
            print(f'duplicate sensor names: {", ".join(x for x in duplicates)}')
    ...
    def read_all_sensors(self):
        ...
        ----------------------------------------------------------------------
        """
        data_for_current_timestamp = {'timestamp': time.time()}
    ...
    def calculate_dew_points(self, data_for_current_timestamp):
        ...
        for sensor in sensor_dew:
            ...
            new_column_title = f'{sensor}_calculated_dew_point'
