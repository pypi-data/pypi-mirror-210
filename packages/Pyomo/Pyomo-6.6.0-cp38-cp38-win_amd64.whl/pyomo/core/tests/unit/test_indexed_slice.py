...
class TestComponentSlices(unittest.TestCase):
    ...
    def test_wildcard_slice(self):
        with self.assertRaisesRegex(
            IndexError,
            'Index .* contains an invalid number of entries for component .*',
        ):
            _slicer = self.m.b[:]
        ...
        self.assertRaisesRegex(
            IndexError,
            'wildcard slice .* can only appear once',
            self.m.b.__getitem__,
            (Ellipsis, Ellipsis),
        )
    ...
    def test_delattr_slices(self):
        ...
        self.assertEqual(
            sum(list(1 if hasattr(x, 'foo') else 0 for x in self.m.b[:, :].c[:, :].x)),
            3 * 3,
        )
    ...
    def test_setitem_slices(self):
        ...
        with self.assertRaisesRegex(
            KeyError,
            "Index 'bogus' is not valid for indexed "
    ...
    def test_str(self):
        ...
        self.assertEqual(
            str(s), "b[...].x[:, 1:2, 1:5:2, ::1, 5, 'a'].component('foo', kwarg=1)"
        )
