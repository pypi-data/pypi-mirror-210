...
from mlflow.spark import _mlflowdbfs_path, _HadoopFileSystem, _should_use_mlflowdbfs, _maybe_save_model
...
# All jars will be downloaded to the current pwd
FLAVOR_NAME = "johnsnowlabs"
SPARK_NLP_VERSION = '4.4.1'
SPARK_NLP_INTERNAL_VERSION = '4.4.1'
SPARK_NLP_JAR = f'spark-nlp-assembly-{SPARK_NLP_VERSION}.jar'
SPARK_NLP_INTERNAL_JAR = f'spark-nlp-jsl-{SPARK_NLP_INTERNAL_VERSION}.jar'
...
def get_default_pip_requirements():
    ...
    """
    # TODO SECRET
    if not 'NLP_SECRET' in os.environ:
        raise Exception('Please set NLP_SECRET environment variable to your secret key before saving or logging your model')
    return [
        'johnsnowlabs_tmp==4.4.6',
        f"https://pypi.johnsnowlabs.com/{os.environ['NLP_SECRET']}/spark-nlp-jsl/spark_nlp_jsl-4.4.2-py3-none-any.whl",
        'pyspark==3.2.1',
        'mlflow-tmp==2.2.17',
        'pandas==1.5.3',
...
def fetch_license_data(
        ...
        nlp_license: Optional[str] = None,
        nlp_secret: Optional[str] = None,
    ...
    license_data: JslSecrets = JslSecrets.build_or_try_find_secrets(
        ...
        local_license_number=None, # TODO HANDLE dont use ?
        remote_license_number=None, # TODO HANDLE dont use ?
        secrets_file=secrets_file,
        hc_license=nlp_license,
        hc_secret=nlp_secret,
        ocr_secret=None,
        ocr_license=None,
        aws_access_key=aws_access_key,
        aws_key_id=aws_key_id,
        store_in_jsl_home=False,
...
def log_model(
    ...
    # artifact repo via Spark. If this fails, we defer to Model.log().
        ...
        return Model.log(
            ...
            flavor=mlflow.johnsnowlabs,  # TODO LOG JSL FLAVOR?!
...
def _save_model_metadata(
    ...
    if conda_env is None:
        if pip_requirements is None:
            # TODO use secret to get pip req
    ...
    pip_requirements = [r for r in pip_requirements if not r.startswith('mlflow==2.3') and not r.startswith('pyspark')]
...
def load_model(model_uri, dfs_tmpdir=None, dst_path=None, **kwargs):
    ...
    if model_uri.startswith("johnsnowlabs:/"):
        # TODO WIP
        # License ---> Add param or env variable
        # We expect the user to start a Sparksession and Auth Environment beforehand properly
        create_sparksession_for_jsl_uri(**kwargs)
        return nlp.load(model_uri.split("johnsnowlabs:/")[1])
...
def get_or_create_sparksession(model_path=None):
    ...
    if spark is None:
        ...
        if model_path:
            ...
            if license_path:
                # TODO LV Serrver endpoints --> Whitelist
                # Todo check if env variable already defined, don't overwrite and use it !
                # cannot put lciense in share, put in env variable
            ...
            spark = nlp.start(nlp=False, spark_nlp=False, jar_paths=jar_paths, json_license_path=license_path,
                              create_jsl_home_if_missing=True)
        else:
            spark = nlp.start()
    return spark
