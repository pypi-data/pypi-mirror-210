...
if True:
    ...
    from alphafed.examples.fed_avg import DATA_OWNER_4_ID
    from alphafed.examples.fed_avg.demos import (DP, FED_IRM, SECURE, SGD,
                                                 VANILLA, get_scheduler,
                                                 get_task_id)

parser = argparse.ArgumentParser(description='Run data owner 4 demo.')
...
logger.info(f'run data owner 4 in {args.mode} mode: {task_id=}')
scheduler._run(id=DATA_OWNER_4_ID, task_id=task_id)
