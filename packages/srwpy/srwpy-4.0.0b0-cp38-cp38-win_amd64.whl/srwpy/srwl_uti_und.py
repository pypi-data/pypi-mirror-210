...
def srwl_uti_und_gen_file_names_for_conv(_ifln, _ofn_core=None, _pref_gap=None, _pref_mode=None, _pref_phase=None, _dict_mode=None, _order_gmp='gmp'):
    """
    Generates list of file names for conversion of magnetic measurements data to SRW format
...
def srwl_uti_und_conv_meas_fld(_ifn):
    ...
    nRows = len(arZ) #OC11012023
    if(nRows < 2): raise Exception("Number of rows in magnetic measurements / simulations data file can't be smaller than 2.")

    for i in range(nRows): #Conversion "mm" -> "m" (both in magn. meas. and Radia sims "mm" are most likely to be used)
        arZ[i] *= 0.001
        
    arBX = array('d', fldDataCols[1])
    arBY = array('d', fldDataCols[2])
    arBZ = array('d', fldDataCols[3]) if(nCols > 3) else array('d', [0]*nRows) #OC11012023
...
if __name__=="__main__":
    ...
    # opt.ifn = "C:\SR_Magnets\CalcSR\ELETTRA\CDI\sim\mag_data\epu"
    # opt.ofn="C:\SR_Magnets\CalcSR\ELETTRA\CDI\sim\mag_data\magn_sim_epu"
    # opt.out_file_name_core = 'epu44_elettra_cdi'
    ...
    #Other call examples (to re-arange Radia simulations data):
    #python srwl_uti_und.py --cnvm --ifn="C:\<some path>\sim\mag_data\epu" --ofn="C:\<some path>\sim\mag_data\magn_sim_epu" --ofnc="epu44_elettra_cdi" --cor --ozc=0 --zk=0 --dbwk=2.0 --prfg=CDI_1EPU_hor_in-phase_g --psfg=.dat --igmp=g
    #python srwl_uti_und.py --cnvm --ifn="C:\SR_Magnets\CalcSR\ELETTRA\CDI\sim\mag_data\phaser" --ofn="C:\SR_Magnets\CalcSR\ELETTRA\CDI\sim\mag_data\magn_sim_phaser" --ofnc="phaser_elettra_cdi" --cor --ozc=0 --zk=0 --dbwk=0.105 --prfg=CDI_phaser_g --psfg=.dat --igmp=g
