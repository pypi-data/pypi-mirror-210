...
class turbine_GPR: 
   ...
   def __init__(self,
      ...
      models_subfolder_path = '/'.join((self.package_path, 'Models'))
      ...
      if model_name==None:
         ...
         try:
            with open(f"{models_subfolder_path}/{model_name}/{model_name}_variables.txt", "r") as file:
               variables = [line.rstrip() for line in file]
               
            with open(f"{models_subfolder_path}/{model_name}/{model_name}_output_variable.txt", "r") as file:
               self.output_key = [line.rstrip() for line in file][0]

            self.variables = variables
            self.fit_dimensions = len(self.variables)
            
            model = joblib.load(f'{models_subfolder_path}/{model_name}/{model_name}.joblib')
   ...
   def get_blade(self,
      ...
      if dimensions in [2,'2D','2d',2.0,'two']:
         ...
         if stack==True:
            fig.tight_layout()
            plt.show()
            fig.savefig('turbine_design/Blade_shapes/blades.pdf')
         else:
            fig1.tight_layout()
            fig2.tight_layout()
            fig1.savefig('turbine_design/Blade_shapes/blades_x_rt.pdf')
            fig2.savefig('turbine_design/Blade_shapes/blades_x_r.pdf')
