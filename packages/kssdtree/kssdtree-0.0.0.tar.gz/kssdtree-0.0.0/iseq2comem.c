...
#define unit_incrs 1000000
    ...
    } else if ((infp = fopen(seqfname, "r")) == NULL) err(errno, "reads2mco():%s", fas_fname);;
...
llong *mt_shortreads2koc(char *seqfname, llong *co, char *pipecmd, int p) {
    ...
    while (!feof(infp)) {
        for (l = 0; l < THREAD_MAX && fgets(tmp, FQ_LEN, infp) && fgets(fq_buff[l], FQ_LEN, infp) &&
                    fgets(tmp, FQ_LEN, infp) && fgets(tmp, FQ_LEN, infp); l++);
            ...
            for (int pos = 0; (ch = fq_buff[t][pos]) != '\n'; pos++) {
                ...
                if (base > TL) {
                    ...
                    llong drtuple = (((unituple & undomask)
                                      + ((unituple & ((1LLU << (half_outctx_len * 2)) - 1))
                            << (TL * 2 - half_outctx_len * 4)))
                            >> (drlevel * 4))
                                    + pfilter;
