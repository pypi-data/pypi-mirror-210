...
int sketch_operate() {
    ...
    if (co_dstat_pan.shuf_id != co_dstat_origin.shuf_id)
        err(errno, "sketcing id not match(%d Vs. %d)", co_dstat_origin.shuf_id, co_dstat_pan.shuf_id);
    fclose(co_stat_fp);
    ...
    for (int c = 0; c < co_dstat_pan.comp_num; c++) {
        ...
        for (int i = 0; i < co_dstat_origin.infile_num; i++) {
            ...
            for (int n = 0; n < fco_pos[i + 1] - fco_pos[i]; n++) {
                if (set_opt.operation == ((dict[cbd_fcode_mem[fco_pos[i] + n] / 64] &
                                           (0x8000000000000000LLU >> (cbd_fcode_mem[fco_pos[i] + n] % 64))) > 0)) {
        ...
        for (int n = 0; n < hashsz; n++) {
            ...
            if (tmphs[hv].taxid == D_TAXID) {
                ...
                if ((tmphs[hv].taxname == NULL && taxname != NULL)
                    ...
                    err(errno, "organize_taxf() abort!: taxid %d has different taxnames in %dth and %dth lines", taxid,
                        tmphs[hv].gids[1], i);
                tmphs[hv].gids[0]++;
...
int combin_subset_pans(char *taxfile) {
    ...
    if (co_dstat_readin.infile_num != subset->gn)
        err(errno, "combin_subset_pans():%s's genome number %d not matches %s's genome number %d", co_dstat_fpath,
            co_dstat_readin.infile_num, taxfile, subset->gn);
    free(co_dstat_fpath);
    mkdir(set_opt.outdir, 0777);
    char tmppath[PATHLEN];
    struct stat s;
