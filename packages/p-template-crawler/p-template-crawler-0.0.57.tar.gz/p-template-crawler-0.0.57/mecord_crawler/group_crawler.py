...
def updateUser(task_id, group_id, url, userConfig):
    ...
    if group_id <= 0:
        ...
        s1 = requests.session()
        s1.keep_alive = False
        res = s1.post(f"https://mecord-beta.2tianxin.com/proxymsg/crawler/create_group", json.dumps(param), verify=False)
...
def processPosts(taskid, group_id):
        ...
        if isinstance(groupCacheConfig[taskid][k], (dict)):
            ...
            if uuid["post"][0]["content_type"] == mediatype2mecord("image"):
                if len(uuid["post"]) > 2:#large than 2 pics use template
                    needTemplate = (random.randint(0,5) == 2)
    ...
    res = s.post("https://mecord-beta.2tianxin.com/proxymsg/crawler/create_post_batch", json.dumps(param), verify=False)
