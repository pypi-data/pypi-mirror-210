...
from disnake.ext.i18n.preprocess import TranslationAgent, Translator
...
class Detector:
    ...
    async def first_language_of(
        ...
        if isinstance(ctx, Message):
            ...
            if ctx._parent and ctx._parent.guild:
                author_id = ctx._parent.author.id
                channel_id = ctx._parent.channel.id
                guild_id = ctx._parent.guild.id
            else:
                if ctx._parent.user:
