...
async def gen_book(config: argparse.Namespace) -> None:
    ...
    if config.output_individual_file:
        gen_multiple_txt(Book(chapters), config)
    else:
        gen_single_txt(Book(chapters), config)
...
def gen_multiple_txt(book: Book, config: argparse.Namespace) -> None:
    ...
    for chapter in book.chapters:
        ...
        filename = Path(chapter.filename).stem + ".txt"
        with open(filename, "w", newline="\n", encoding="utf8") as file:
            file.write(content)


def gen_single_txt(book: Book, config: argparse.Namespace) -> None:
    ...
    with open(config.output, "w", newline="\n", encoding="utf8") as file:
        ...
        file.write(
            _("author:") + f"{config.book_author}{config.paragraph_separator}"
        )
