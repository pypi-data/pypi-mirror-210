...
class Select(TableOperation):
    ...
    label_: StrictStr = Field('select', const=True)
...
class Where(TableOperation):
    ...
    """
    label_: StrictStr = Field('where', const=True)
...
class GroupBy(TableOperation):
    ...
    """
    label_: StrictStr = Field('group_by', const=True)
    ...
    def _aggregate(self, name, *args, **aggs: Column) -> Aggregate:
        ...
        content = self.content.update_node(aggregates=aggs)
...
def _limit_make(parent: TableOperation, limit: int, offset: int) -> Limit:
    ...
    if limit_dtype != DType.Int or limit <= 0:
...
class SetOperation(TableOperation):
    ...
    """
    label_: Optional[StrictStr] = Field('row set op', const=True, alias='label')
...
def dependency_sql(node):
    ...
    def node_filter(t: Node) -> bool:
        return isinstance(t, BaseVarDef)
