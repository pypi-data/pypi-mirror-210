...
from pydantic import root_validator, StrictStr, Field
...
class Column(Node):
    ...
    meta: Union[ColumnMeta, float, int, bool, str, datetime, date, None] = None
    sql: Optional[StrictStr] = None

    # noinspection PyMethodParameters
    @root_validator
    def _compute_val(cls, values):
        if 'fn' in values and 'parents_' in values:
            values['sql'] = values['fn'](*values['parents_'])

            if not isinstance(values['sql'], str):
                raise TypeError(
                    f"Column.fn must be a callable that returns str, but returned {type(values['sql']).__name__} "
                    f"({values['sql']})."
                )
            return values
        return values
    ...
    def __pow__(self, power, modulo=None) -> Column:
        fn = lambda x, y: f'pow({x.sql}, {y.sql})'
