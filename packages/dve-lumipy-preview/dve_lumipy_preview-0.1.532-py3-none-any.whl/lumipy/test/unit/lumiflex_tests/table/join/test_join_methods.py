...
class TestJoinTableMethods(SqlTestCase):
    ...
    def test_join_table_get_param_assignments(self):
        ...
        exp = []
        exp += [t1._add_prefix(c) for c in t1._with_alias('a')._get_param_assignments()]
        exp += [t2._add_prefix(c) for c in t2._with_alias('b')._get_param_assignments()]
        exp += [t3._add_prefix(c) for c in t3._with_alias('c')._get_param_assignments()]
        exp += [t4._add_prefix(c) for c in t4._with_alias('d')._get_param_assignments()]
    ...
    def test_join_table_get_columns(self):
        ...
        exp = []
        exp += [t1._add_prefix(c) for c in t1._with_alias('a').get_columns()]
        exp += [t2._add_prefix(c) for c in t2._with_alias('b').get_columns()]
        exp += [t3._add_prefix(c) for c in t3._with_alias('c').get_columns()]
        exp += [t4._add_prefix(c) for c in t4._with_alias('d').get_columns()]
        self.assertSequenceHashEqual(obs, exp)

        obs = join.get_columns(True)
        exp = []
        exp += [t1._add_prefix(c) for c in t1._with_alias('a').get_columns(True)]
        exp += [t2._add_prefix(c) for c in t2._with_alias('b').get_columns(True)]
        exp += [t3._add_prefix(c) for c in t3._with_alias('c').get_columns(True)]
        exp += [t4._add_prefix(c) for c in t4._with_alias('d').get_columns(True)]
        self.assertSequenceHashEqual(obs, exp)
    ...
    def test_join_table_add_suffix(self):
        ...
        self.assertEqual("[Col0] AS [Col0_a]", join._add_suffix(t1.col0).sql)
        self.assertEqual("[Col0] AS [Col0_b]", join._add_suffix(t2.col0).sql)
        self.assertEqual("[Col0] AS [Col0_c]", join._add_suffix(t3.col0).sql)
        self.assertEqual("[Col0] AS [Col0_d]", join._add_suffix(t4.col0).sql)
    ...
    def test_join_table_left_join_error_with_bad_on_condition(self):
        ...
        self.assertErrorsWithMessage(
            ...
            __root__
              There are columns in the join's on condition that don't belong to any parent table (my.table.one, my.table.two):
                [Col0] (my.table.three)
                [Col0] (my.table.four) (type=value_error)
            """
        )
        ...
        self.assertErrorsWithMessage(
            ...
            1 validation error for Join
            __root__
              There are columns in the join's on condition that don't belong to any parent table (my.table.one, my.table.two, my.table.three):
                [Col0] (my.table.four) (type=value_error)
            """
        )
    ...
    def test_join_table_error_on_duplicate_alias(self):
        ...
        self.assertErrorsWithMessage(
            ...
            1 validation error for Join
            __root__
              The two sides of the join must have different aliases, but were both 'a'. (type=value_error)
            """
        )
        ...
        self.assertErrorsWithMessage(
            ...
            1 validation error for Join
            __root__
              Right table has an alias ('rhs') that clashes with an existing parent table alias ('lhs', 'rhs'). (type=value_error)
            """
        )
    ...
    def test_join_table_getitem(self):
        t1 = self.make_table('my.table.one', n_cols=3, n_params=2)
        t2 = self.make_table('my.table.two', n_cols=2, n_params=3)
