...
class TestWhen(SqlTestCase):
    ...
    def test_when_construction_validation(self):
        ...
        self.assertErrorsWithMessage(
            ...
            "1 validation error for When\n"
            "parents\n"
            "  When node must have one or two parents: (condition) or (condition, then), but received 0. (type=value_error)"
        )
        self.assertErrorsWithMessage(
            ...
            "1 validation error for When\n"
            "parents\n"
            "  When node must have one or two parents: (condition) or (condition, then), but received 3. (type=value_error)"
        )
        self.assertErrorsWithMessage(
            ...
            "1 validation error for When\n"
            "parents\n"
            "  Condition must resolve to a boolean, but was Int. (type=type_error)"
        )
        self.assertErrorsWithMessage(
            ...
            "1 validation error for When\n"
            "parents\n"
            "  The second parent of When must be a Then object, but was int. (type=type_error)"
        )
...
class TestThen(SqlTestCase):
    ...
    def test_then_constructor_validation(self):
        ...
        self.assertErrorsWithMessage(
            ...
            ValueError,
            "1 validation error for Then\nparents\n  Then node must have two parents: (value, then) but received 0. (type=value_error)"
        )
        self.assertErrorsWithMessage(
            lambda: Then(parents=(1, 2, 3)),
            ValueError,
            "1 validation error for Then\nparents\n  Then node must have two parents: (value, then) but received 3. (type=value_error)"
        )
        self.assertErrorsWithMessage(
            lambda: Then(parents=(1, 2)),
            ValueError,
            "1 validation error for Then\nparents\n  Value must be a Column object, but was int. (type=type_error)"
        )
    ...
    def test_then_get_sql_method(self):
        ...
        self.assertSqlEqual("THEN pow([Col1], 2)", then.get_sql())
    ...
    def test_then_get_sql_with_prefix(self):
        ...
        self.assertSqlEqual("THEN pow(AA.[Col1], 2)", then_p.get_sql())
...
class TestCaseColumn(SqlTestCase):
    ...
    def test_case_column_construction_validation(self):
        ...
        self.assertErrorsWithMessage(
            ...
            "__root__\n"
            "  Case must have two parents, but was given 0 (type=value_error)"
        )
        self.assertErrorsWithMessage(
        ...
        self.assertErrorsWithMessage(
            ...
            "1 validation error for CaseColumn\n"
            "parents\n"
            "  Parents must all be Node or a subclass of Node but were (int). (type=type_error)"
        )
        ...
        self.assertErrorsWithMessage(
            ...
            ValueError,
            "1 validation error for CaseColumn\nparents\n  "
            "Parents must all be Node or a subclass of Node but were (int, Then). (type=type_error)"
        )
        ...
        self.assertErrorsWithMessage(
            ...
            ValueError,
            "1 validation error for CaseColumn\nparents\n"
            "  Parents must all be Node or a subclass of Node but were (Column, str). (type=type_error)"
        )
