...
from vid2info.inference.config import TRACK_INFO, TRACK_ID, CLASS_NAME
...
class StateBuffer:
    ...
    def get_first_element_timestamp(self, track_id : int) -> float | None:
        ...
        if self.running_online:
            ...
            raise NotImplementedError("Not yet implemented for the case where the video is not coming from an online stream.")
    ...
    def get_element_state_machine(self, element_info: dict, in_scene_tracks: tuple | list = ()) -> ElementFiniteStateMachine:
        ...
        last_detection = self.get_last_detection_of_element(track_id=element_info[TRACK_INFO][TRACK_ID])
        current_class = element_info[CLASS_NAME]
