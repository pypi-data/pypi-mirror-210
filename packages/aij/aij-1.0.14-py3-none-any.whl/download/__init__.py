...
class VideoPublisher:
    ...
    def __init__(self, host='localhost', queue_name='video_stream'):
        ...
        self.sources = [
            'https://youtu.be/cIzW8Paqvvc?list=PL6XRrncXkMaW5p7muaR2s2IqjouQh4jqS',
            'https://www.youtube.com/watch?v=ZnQdseqFjj0&list=PL6XRrncXkMaW5p7muaR2s2IqjouQh4jqS&index=1&pp=iAQB'
        ]
            ...
            for _source in self.sources:
                ...
                try:
                    with youtube_dl.YoutubeDL(ydl_opts) as ydl:
                        ydl.download([_source])

                        _body = json.dumps(
                            {
                                'source': _source,
                                'target': os.path.join(os.getcwd(), _source.split('/')[-1])
                            }
                        ).encode('utf-8')
                    ...
                    print(
                        f"Could not download the video; {download_exception}"
                    )
...
def main():
    ...
    client.containers.run("rabbitmq:3", detach=True, name='aij-messaging-server', remove = True)
