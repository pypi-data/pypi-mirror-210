...
from .util import (_get_histogram, arg_to_list, dict_val_to_list,
                   set_to_several_objects)
...
class DatasetPool():
    ...
    def __init__(self,
        ...
        for i, ds in enumerate(dsList):
            ...
            with open(cf, errors='ignore') as f:
                ...
                object_parents = [c["parentStructure"] for c in conf["structures"]["list"]]
    ...
    def get_timeseries(self, object_name: str, metric, inplace=False, **kwargs):
        ...
        elif metric.lower() == 'fluo_intensity':
    ...
    def save_selection(self, object_name: str, datasets=None, name: str = None):
        ...
        for ds in datasets:
            saveAndOpenSelection(self[object_name].query('Dataset == @ds'), dsName=ds,
                                 objectClassIdx=self.objects.index(object_name),
                                 selectionName=name,
