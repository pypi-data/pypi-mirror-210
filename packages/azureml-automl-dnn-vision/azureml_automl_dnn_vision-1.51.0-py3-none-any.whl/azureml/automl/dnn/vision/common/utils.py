...
from azureml.automl.core.shared._diagnostics.automl_error_definitions import (
    InsufficientMemory, InsufficientGPUMemory, InsufficientSHMMemory)
...
from azureml.automl.dnn.vision.common.base_model_factory import \
    BaseModelFactory
from azureml.automl.dnn.vision.common.constants import (
    ...
    TrainingLiterals, Warnings, supported_model_layer_info, DistributedLiterals)
...
from azureml.automl.dnn.vision.common.tiling_utils import \
...
def _data_exception_safe_iterator(iterator: Iterator) -> Any:
    while True:
        try:
            ...
            if "DefaultCPUAllocator" in mesg or "is killed by signal: Killed" in mesg or \
                    "exited unexpectedly" in mesg:
                azureml_error = AzureMLError.create(InsufficientMemory)
                raise ResourceException._with_error(azureml_error)
            else:
        ...
        try:
            ...
            if isinstance(e, (AzureMLException, AzureMLServiceException)):
                ...
                if isinstance(e, RuntimeError) and "dataset" not in str_e and "dataloader" not in str_e:
                    ...
                    # If CUDA related error, direct user to file a ticket for us and print installed packages.
                    if ("CUDA error:" in str_e) or ("cuDNN error:" in str_e):
                        error_details = \
                            "Potential temporary hardware failure - please resubmit the run. If that does not solve " \
                            "the problem, please file a support ticket for further investigation."
...
def strtobool(val: str) -> int:
    ...
    if val in TRUE_STRING_VALUES:
        ...
        raise AutoMLVisionDataException(
            "Invalid truth value {}".format(val),
            has_pii=False,
        )
...
def cpu_oom_retry_handler(func: Callable[..., Any]) -> Callable[..., Any]:
    """This decorates a function to retry in case of handle CPU OOM exceptions. This function tries to free occupied
    GPU memory between restarts.
    """
    ...
    def wrapper(*args: Any, **kwargs: Any) -> Any:
        call_count = 0
        retry = False
        while call_count == 0 or retry:
            call_count += 1
            retry = func(*args, **kwargs)
            if retry:
                # Run garbage collection and empy cuda cache to free up gpu memory.
                gc.collect()
                torch.cuda.empty_cache()
    return wrapper


@cpu_oom_retry_handler
def launch_training_with_retries(
        settings: Dict[str, Any],
        ...
        logger: logging.Logger, azureml_run: Run) -> bool:
    """Launch single or distributed training with support for restarts. In case of CPU OOM exceptions,
    it restarts training with updated settings that are expected to reduce CPU memory usage.
    ...
    :param azureml_run: The run object
    :type azureml_run: Run
    :return: Should retry or not
    :rtype: bool
    """
    retry = False
    try:
        distributed_utils.launch_single_or_distributed_training(
            settings=settings, train_worker_fn=train_worker_fn,
            additional_train_worker_fn_args=additional_train_worker_fn_args, logger=logger, azureml_run=azureml_run)
    except (ResourceException, ProcessRaisedException) as ex:
        exception_message = str(ex)
        if AutoMLErrorStrings.INSUFFICIENT_MEMORY in exception_message:
            # CPU OOM
            # Do nothing if multi-node training
            if settings[DistributedLiterals.DISTRIBUTED] and distributed_utils._get_node_count() > 1:
                raise

            msg = "Got a CPU OOM error when training. "
            if settings[DistributedLiterals.DISTRIBUTED]:
                msg += "Restarting training by switching to single process training."
                settings[DistributedLiterals.DISTRIBUTED] = False
                retry = True
            elif settings[SettingsLiterals.NUM_WORKERS] != 0:
                msg += "Restarting training with dataloader num_workers as 0."
                settings[SettingsLiterals.NUM_WORKERS] = 0
                retry = True

            if retry:
                logger.info(msg)
                settings[SettingsLiterals.RESUME_FROM_STATE] = True
            else:
                raise
        else:
            raise
