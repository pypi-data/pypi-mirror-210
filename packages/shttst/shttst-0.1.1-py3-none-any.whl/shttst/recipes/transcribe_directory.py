...
from shttst.files import append_file, destroy_file, get_audio_files, get_files
    ...
    for path in tqdm(audio_list):
        ...
        if audio_class != "fine":
            if remove_bad:
                ...
                append_file(
                    os.path.join(dir_path, "1_bad.txt"), f"{path}|{audio_class}\n"
                )
        else:
...
def transcribe_directory(
    ...
    for path in tqdm(audio_list):
        ...
        if (
            not text
            or chars_per_sec < min_chars_per_sec
            or chars_per_sec > max_chars_per_sec
        ):
            append_file(
                os.path.join(dir_path, "2_bad.txt"), f"{path}|{text}|{duration}\n"
            )
            continue
        ...
        append_file(
            os.path.join(wavs_path, os.path.pardir, "metadata.csv"),
            f"{line.serialize()}",
        )


if __name__ == "__main__":
    meta_files = get_files("/content/yt_dlp", ["csv"], "metadata")
    for meta in meta_files:
        os.remove(meta)
    transcribe_directory("/content/yt_dlp", classify=False)
