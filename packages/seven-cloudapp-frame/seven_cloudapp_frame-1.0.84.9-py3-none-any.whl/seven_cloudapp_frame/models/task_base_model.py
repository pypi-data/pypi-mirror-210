...
@LastEditTime: 2023-05-25 09:29:28
...
class TaskBaseModel(FrameBaseModel):
    ...
    def add_task_count(self, app_id, act_id, module_id, user_id, open_id, task_type, task_info_dict=None, task_sub_type=''):
        """
        ...
        if task_info_dict:
            ...
            task_count_list = task_count_model.get_list("act_id=%s and module_id=%s and user_id=%s and task_type=%s", params=[task_info_dict["act_id"], task_info_dict["module_id"], user_id, task_info_dict['task_type']])
            task_count = self.init_task_count(task_info_dict, task_count_list, task_sub_type)
            task_count_id_md5 = CryptoHelper.md5_encrypt_int(f"{task_info_dict['act_id']}_{task_info_dict['module_id']}_{task_info_dict['task_type']}_{task_sub_type}_{user_id}")
    ...
    def convert_task_node(self, task_id, sort_index, task_type, task_type_name, title, desc='', current=0, total=0, data=[], complete_count=1, current_complete_count=0, reward_value=0, completed_ids=[], get_ways=1, status=0, btn_text=[], route_url=''):
        """
        :description: 合成任务节点
        :param task_id：任务id
        :param sort_index：排序
        ...
        :param total：当前轮领取阀值（如果任务没有（1/2）这样的填0）消费任务表示领取奖励需要消费的金额
    ...
    def convert_gear_task_node(self, act_id, module_id, user_id, task_info_dict, config_json, task_count_list):
        ...
        :last_editors: HuangJianYi
        """
        result = deepcopy(self.convert_task_node(task_info_dict["id"],task_info_dict["sort_index"],task_info_dict["task_type"],SevenHelper.get_enum_key(TaskType, task_info_dict["task_type"]),task_info_dict["task_name"],route_url="receive_reward"))
