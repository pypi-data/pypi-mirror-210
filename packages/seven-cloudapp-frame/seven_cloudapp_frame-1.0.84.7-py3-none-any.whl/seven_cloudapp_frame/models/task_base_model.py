...
@LastEditTime: 2023-05-24 11:21:32
...
class TaskBaseModel(FrameBaseModel):
    ...
    def init_task_count(self, task_info_dict, task_count_list, task_sub_type=""):
        ...
        """
        if task_sub_type:
            task_count = [task_count for task_count in task_count_list if task_info_dict["task_type"] == task_count.task_type and task_count.task_sub_type == task_sub_type]
        else:
    ...
    def add_gear_task_count(self, app_id, act_id, module_id, user_id, open_id, task_type, value, remark=''):
        """
        :description: 添加档位任务累计值，比如累计消费金额，累计抽盒次数
        :param app_id：应用标识
        :param act_id：活动标识
        :param module_id：活动模块标识
    ...
    def conert_task_node(self, task_id, sort_index, task_type, task_type_name, title, desc='', current=0, total=0, data=[], complete_count=1, current_complete_count=0, reward_value=0, completed_ids=[], get_ways=1, status=0, btn_text=[], route_url=''):
        """
        :description: 合成任务节点
        :param task_id：任务id
        :param sort_index：排序
    ...
    def convert_gear_task_node(self, act_id, module_id, user_id, task_info_dict, config_json, task_count_list):
        ...
        :last_editors: HuangJianYi
        """
        result = self.conert_task_node(task_info_dict["id"],
        ...
        for sub_config_json in config_json:
            ...
            sub_result = {}
            sub_result["id"] = sub_config_json["id"] #子任务id
            sub_result["gear"] = sub_config_json["gear"] #档位阀值
            sub_result["reward_name"] = sub_config_json["reward_name"] #奖励名称
            sub_result["reward_value"] = sub_config_json["reward_value"] #奖励数量
            result["data"].append(sub_result)
    ...
    def get_client_task_list_v2(self, app_id, act_id, module_id, user_id, task_types="", daily_repeat_browse=False):
        ...
        for task_info_dict in task_info_list:
            ...
            #有档位任务列表的相关累计任务
                ...
                result_list.append(self.convert_gear_task_node(act_id,module_id,user_id,task_info_dict,config_json,task_count_list))
