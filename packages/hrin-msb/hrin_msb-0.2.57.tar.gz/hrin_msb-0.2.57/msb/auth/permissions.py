...
class Permissions:
	...
	class Authenticated(BasePermission):
		...
		def __get_allowed_role_ids(self) -> list:
			allowed_ids: list = [RoleConst.ADMIN_ROLE_ID] if self.admin_is_allowed_by_default else []
			if isinstance(self.allowed_role_ids, list):
				allowed_ids.extend(self.allowed_role_ids)
			return allowed_ids
