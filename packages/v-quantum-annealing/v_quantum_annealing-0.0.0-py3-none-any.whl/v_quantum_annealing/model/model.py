    ...
    class BinaryQuadraticModel(Base):
        ...
        def __init__(self, *args, **kwargs):
            ...
            Args:
                ...
                offset (float): offset
                vartype (v_quantum_annealing.variable_type.Vartype): vartype
            ...
            self.gpu = gpu
            self.model_type = "v_quantum_annealing.BinaryQuadraticModel"

        def get_cxxvqa_ising_graph(self):
            """Generate cxxvqa Ising graph from the interactions.

            Returns:
                cxxvqa.graph.Dense or cxxvqa.graph.Sparse:
                offset (float): offset of the energy due to qubo->ising transformation
            """

            # if sparse is true, select `cxxvqa.graph.CSRSparse` graph type,
            # else, select `cxxvqa.graph.Dense` graph type.
            if sparse:
                old_vartype = self.vartype
                self.change_vartype("SPIN")

                GraphClass = (
                    cxxvqa.graph.CSRSparse
                    if self.gpu == False
                    else cxxvqa.graph.CSRSparseGPU
                )
                offset = self.offset
                ...
                GraphClass = (
                    cxxvqa.graph.Dense if self.gpu == False else cxxvqa.graph.DenseGPU
                )
...
def BinaryQuadraticModel(linear, quadratic, *args, **kwargs):
    ...
    Args:
        ...
        quadratic (dict): quadratic biases
        offset (float): offset
        vartype (v_quantum_annealing.variable_type.Vartype): vartype ('SPIN' or 'BINARY')
        gpu (bool): if true, this can be used for gpu samplers.
        kwargs:
    Returns:
...
def make_BinaryPolynomialModel(polynomial, index_type=None, tuple_size=0):
    class BinaryPolynomialModel(
        ...
        def __init__(self, *args, **kwargs):
            super().__init__(*args, **kwargs)
            self.model_type = "v_quantum_annealing.BinaryPolynomialModel"

        def get_cxxvqa_ising_graph(self):
            return cxxvqa.graph.Polynomial(self.to_serializable())
