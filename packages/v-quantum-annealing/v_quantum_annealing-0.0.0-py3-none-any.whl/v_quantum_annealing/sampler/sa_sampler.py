...
from v_quantum_annealing.sampler.sampler import BaseSampler
...
class SASampler(BaseSampler):
    ...
    def __init__(
        ...
        self._make_system = {
            "singlespinflip": cxxvqa.system.make_classical_ising,
            "singlespinflippolynomial": cxxvqa.system.make_classical_ising_polynomial,
            "swendsenwang": cxxvqa.system.make_classical_ising,
        }
        self._algorithm = {
            "singlespinflip": cxxvqa.algorithm.Algorithm_SingleSpinFlip_run,
            "singlespinflippolynomial": cxxvqa.algorithm.Algorithm_SingleSpinFlip_run,
            "swendsenwang": cxxvqa.algorithm.Algorithm_SwendsenWang_run,
        }

    def _convert_validation_schedule(self, schedule):
        """Checks if the schedule is valid and returns cxxvqa schedule."""
        if not isinstance(schedule, (list, np.array)):
            raise ValueError("schedule should be list or numpy.array")

        if isinstance(schedule[0], cxxvqa.utility.ClassicalSchedule):
            return schedule
        ...
        # convert to cxxvqa.utility.ClassicalSchedule
        cxxvqa_schedule = []
        for beta, step_length in schedule:
            _schedule = cxxvqa.utility.ClassicalSchedule()
            _schedule.one_mc_step = step_length
            _schedule.updater_parameter.beta = beta
            cxxvqa_schedule.append(_schedule)

        return cxxvqa_schedule
    ...
    def sample(
        ...
        Args:
            bqm (v_quantum_annealing.model.model.BinaryQuadraticModel) binary quadratic model
        ...
        Returns:
            :class:`v_quantum_annealing.sampler.response.Response`: results
        ...
        ising_graph, offset = model.get_cxxvqa_ising_graph()
        ...
        # ------------------------------------------- choose updater
        response = self._cxxvqa_sampling(
    ...
    def _sample_hubo_old(
        ...
        reinitialize_state: Optional[bool] = None,
        seed: Optional[int] = None,
    ) -> "v_quantum_annealing.sampler.response.Response":
        """Sampling from higher order unconstrainted binary optimization.

        Args:
            J (dict): Interactions.
            vartype (str, v_quantum_annealing.VarType): "SPIN" or "BINARY".
        ...
        Examples::
            for Ising case::
                >>> sampler = v_quantum_annealing.SASampler()
                >>> J = {(0,): -1, (0, 1): -1, (0, 1, 2): 1}
        ...
        if initial_state is None:
            if model.vartype == SPIN:
                ...
                def _generate_init_state():
                    return (
                        cxxvqa.graph.Polynomial(model.num_variables).gen_spin(seed)
                        if seed is not None
                        else cxxvqa.graph.Polynomial(model.num_variables).gen_spin()
                    )
                ...
                def _generate_init_state():
                    return (
                        cxxvqa.graph.Polynomial(model.num_variables).gen_binary(seed)
                        if seed is not None
                        else cxxvqa.graph.Polynomial(model.num_variables).gen_binary()
                    )
        ...
        if model.vartype == SPIN:
            if updater is None or updater == "single spin flip":
                sa_system = cxxvqa.system.make_classical_ising_polynomial(
                    _generate_init_state(), model.to_serializable()
                )
                algorithm = cxxvqa.algorithm.Algorithm_SingleSpinFlip_run
            ...
            if updater == "k-local":
                sa_system = cxxvqa.system.make_k_local_polynomial(
                    _generate_init_state(), model.to_serializable()
                )
                algorithm = cxxvqa.algorithm.Algorithm_KLocal_run
...
def geometric_ising_beta_schedule(
    cxxgraph: Union[v_quantum_annealing.cxxvqa.graph.Dense, v_quantum_annealing.cxxvqa.graph.CSRSparse],
    beta_max=None,
    beta_min=None,
    num_sweeps=1000,
):
    ...
    Args:
        cxxgraph (Union[v_quantum_annealing.cxxvqa.graph.Dense, v_quantum_annealing.cxxvqa.graph.CSRSparse]): Ising graph, must be either `Dense` or `CSRSparse`.
    ...
    Returns:
        list of cxxvqa.utility.ClassicalSchedule, list of beta range [max, min]
    """
    ...
    if beta_min is None or beta_max is None:
        ...
        # if `abs_ising_interaction` is empty, set min/max delta_energy to 1 (a
        # trivial case).
    ...
    # set schedule to cxxvqa
    schedule = cxxvqa.utility.make_classical_schedule_list(
        beta_min=beta_min,
        beta_max=beta_max,
