...
class BaseSampler(dimod.Sampler):
    """Base sampler class of python wrapper for cxxvqa simulator."""
    ...
    def _cxxvqa_sampling(
        ...
        """Basic sampling function: for cxxvqa sampling.

        Args:
            model (v_quantum_annealing.BinaryQuadraticModel): model has a information of instaunce (h, J, Q)
            init_generator (callable): return initial state, must have argument structure
            algorithm (callable): system algorithm of cxxvqa
        ...
        def exec_sampling():
            for _ in range(self._params["num_reads"]):
                ...
                # user can use previous result (if re-initilize is set to
                # False)
    ...
    def sample(self, bqm, **parameters):
        ...
        Args:
            bqm (v_quantum_annealing.BinaryQuadraticModel):
