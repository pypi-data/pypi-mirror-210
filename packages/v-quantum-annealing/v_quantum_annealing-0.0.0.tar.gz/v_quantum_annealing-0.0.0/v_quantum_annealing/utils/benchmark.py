...
def solver_benchmark(
    ...
    Args:
        solver (callable): returns v_quantum_annealing.Response, and solver has arguments 'time' and '**args'
        time_list (list):
...
def residual_energy(response, ref_energy):
    ...
    Args:
        response (v_quantum_annealing.Response): response from solver (or sampler).
...
def success_probability(response, solutions, ref_energy=0, measure_with_energy=False):
    """Calculate success probability from v_quantum_annealing.response
...
def se_success_probability(
    ...
    """Calculate success probability's standard error from v_quantum_annealing.response
