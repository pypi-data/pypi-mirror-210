    ...
    class ChimeraModel(make_BinaryQuadraticModel(linear, quadratic)):
        ...
        def validate_chimera(self):
            ...
            for i, j in self.quadratic.keys():
                ...
                adj_list = []
                if z_i >= 4:
                    # part of right side of a Chimera unit cell (in the column
                    # representation).
                    if c_i > 0:
                        adj_list.append(
                    ...
                    # part of left side of a Chimera unit cell (in the column
                    # representation).
                    if r_i > 0:
                        adj_list.append(
        ...
        def get_cxxvqa_ising_graph(self):
            """Get cxxvqa.graph.Chimera type instance

            Args:
                i (int): 1-D index 0~L*L*8-1
                unit_num_L (int): number of chimera grid size
            Returns:
                object (cxxvqa.graph.Chimera)
            """
...
def ChimeraModel(
    ...
    matches a Chimera graph and to convert it to cxxvqa.graph.Chimera.
