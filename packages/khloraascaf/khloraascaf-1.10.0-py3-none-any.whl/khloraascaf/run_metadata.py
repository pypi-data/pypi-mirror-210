...
class MetadataSolution():
    ...
    def from_run_directory(cls, yaml_dir: Path) -> MetadataSolution:
        ...
        return cls(
            ...
            sol_dict[CONTIGS_OF_REGIONS_PREFIX],
            sol_dict[MAP_OF_REGIONS_PREFIX],
        )

    # pylint: disable=too-many-arguments
    def __init__(self, solution_dir: Path,
                 ilp_combi: Iterable[RegionIDT],
                 contigs_of_regions_filename: str,
                 map_of_regions_filename: str):
        """The Initialiser.
        ...
        contigs_of_regions_filename : str
            Contigs of regions filename
        map_of_regions_filename : str
            Map of regions filename
        """
        self.__sol_dir: Path = solution_dir
        self.__ilp_combi: list[RegionIDT] = list(ilp_combi)
        self.__contigs_of_regions_fname: str = contigs_of_regions_filename
        self.__map_of_regions_fname: str = map_of_regions_filename
    ...
    def contigs_of_regions(self) -> Path:
        ...
        Path
            Contigs of regions file path
        """
        return self.__sol_dir / self.__contigs_of_regions_fname
    ...
    def map_of_regions(self) -> Path:
        ...
        Path
            Map of regions file path
        """
        return self.__sol_dir / self.__map_of_regions_fname
                ...
                yaml.dump(
                    {
                        ...
                        CONTIGS_OF_REGIONS_PREFIX:
                        self.__contigs_of_regions_fname,
                        MAP_OF_REGIONS_PREFIX:
                        self.__map_of_regions_fname,
                    },
                    sort_keys=False,
                ),
            )
                    ...
                    [
                        {
                            ...
                            CONTIGS_OF_REGIONS_PREFIX:
                            solution.contigs_of_regions().name,
                            MAP_OF_REGIONS_PREFIX:
                            solution.map_of_regions().name,
...
class MetadataDebug():
    ...
    def __init__(self, run_dir: Path,
                 ...
                 vertices_of_regions_filename: str | None = None,
                 map_of_regions_filename: str | None = None,
                 repeat_fragments_filename: str | None = None):
        """The Initialiser.
        ...
        vertices_of_regions_filename : str | None
            Vertices of regions filename, optional
        map_of_regions_filename : str | None
            Map of regions filename, optional
        repeat_fragments_filename : str | None
            Repeated fragments filename, optional
        """
        ...
        self.__vertices_of_regions_fname: str | None = (
            vertices_of_regions_filename
        )
        self.__map_of_regions_fname: str | None = map_of_regions_filename
        self.__repeat_fragments_fname: str | None = repeat_fragments_filename
    ...
    def vertices_of_regions(self) -> Path:
        ...
        FileNotFoundError
            File does not exist
        """
        if self.__vertices_of_regions_fname is None:
            raise FileNotFoundError()
        return self.__run_dir / self.__vertices_of_regions_fname
    ...
    def map_of_regions(self) -> Path:
        ...
        FileNotFoundError
            File does not exist
        """
        if self.__map_of_regions_fname is None:
            raise FileNotFoundError()
        return self.__run_dir / self.__map_of_regions_fname
    ...
    def repeat_fragments(self) -> Path:
        ...
        FileNotFoundError
            File does not exist
        """
        if self.__repeat_fragments_fname is None:
            raise FileNotFoundError()
        return self.__run_dir / self.__repeat_fragments_fname
    ...
    def write_yaml(self):
        ...
        if self.__ilp_status == LpStatus[LpStatusOptimal]:
            ...
            assert self.__vertices_of_regions_fname is not None
            assert self.__map_of_regions_fname is not None
            dict_debug.update(
                {
                    self.KEY_OPT_VALUE: self.__opt_value,
                    VERTICES_OF_REGIONS_PREFIX:
                    self.__vertices_of_regions_fname,
                    MAP_OF_REGIONS_PREFIX:
                    self.__map_of_regions_fname,
                },
            )
            if self.__ilp_combi[-1] in (IR_REGION_ID, DR_REGION_ID):
                assert self.__repeat_fragments_fname is not None
                dict_debug.update(
                    {
                        FOUND_REPFRAG_PREFIX:
                        self.__repeat_fragments_fname,
                    },
                )
                ...
                dict_debug.update(
                    {
                        ...
                        VERTICES_OF_REGIONS_PREFIX:
                        debug.vertices_of_regions().name,
                        MAP_OF_REGIONS_PREFIX:
                        debug.map_of_regions().name,
                    },
                )
