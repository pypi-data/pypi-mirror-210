...
def __region_scaffolding_debug(instance_name: str, outdir: Path,
    ...
    # ------------------------------------------------------------------------ #
    vertices_of_regions_filename = fmt_vertices_of_regions_filename(
        instance_name, result.ilp_combination(),
    )
    write_vertices_of_regions(result, outdir / vertices_of_regions_filename)

    # ------------------------------------------------------------------------ #
    # The map of regions (debug version)
    # ------------------------------------------------------------------------ #
    map_of_regions_filename = fmt_map_of_regions_filename(
        instance_name, result.ilp_combination(),
    )
    write_map_of_regions(result, outdir / map_of_regions_filename)

    # ------------------------------------------------------------------------ #
    # The repeated fragments
    # ------------------------------------------------------------------------ #
    repeat_fragments_filename = None
    if isinstance(var, (PuLPVarInvFModel, PuLPVarDirFModel)):
        repeat_fragments_filename = fmt_found_repeated_fragments_filename(
            instance_name, result.ilp_combination(),
        )
        write_found_repeated_fragments(var, outdir / repeat_fragments_filename)
    ...
    MetadataAllDebugs(
        ...
        (
            MetadataDebug(
                ...
                map_of_regions_filename,
                (
                    repeat_fragments_filename
                    if repeat_fragments_filename is not None else None
                ),
            ),
        ),
    ).write_yaml(append_yaml=True)
...
def __solution_scaffolding(solution: ScaffoldingResult, outdir_gen: Path,
    ...
    # -------------------------------------------------------------------- #
    contigs_of_regions_filename = fmt_contigs_of_regions_filename(
        instance_name, solution.ilp_combination(),
    )
    write_contigs_of_regions(
        solution, outdir_gen / contigs_of_regions_filename,
    ...
    if not debug:  # already generated if debug is True
        write_map_of_regions(solution, outdir_gen / map_of_regions_filename)
