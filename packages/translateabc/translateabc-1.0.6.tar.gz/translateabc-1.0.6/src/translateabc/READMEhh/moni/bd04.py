...
# skimage包主要用于图像数据的处理，在该实验当中，io模块主要图像数据的读取（imread）和输出（imshow）
# 操作，transform模块主要用于改变图像的大小（resize函数）

# glob包主要用于查找符合特定规则的文件路径名，跟使用windows下的文件搜索差不多，查找文件只用到三个匹配符
# ：”*”, “?”, “[]”。”*”匹配0个或多个字符；”?”匹配单个字符；”[]”匹配指定范围内的字符，
# 如：[0-9]匹配数字。该实验中，glob主要用于返回所有匹配的文件路径列表。它只有一个参数pathname，
...
# tensorflow是目前业界最流行的深度学习框架，在图像，语音，文本，目标检测等领域都有深入的应用。
# 是该实验的核心，主要用于定义占位符，定义变量，创建卷积神经网络模型

# numpy是一个基于python的科学计算包，在该实验中主要用来处理数值运算，包括创建等差数组，生成随机数组，
# 聚合运算等。
...
def read_img(path):
    ...
    # 创建层级列表cate，用于对数据存放目录下面的数据文件夹进行遍历，os.path.isdir用于判断文件是否是目录，
    # 然后对是目录文件的文件进行遍历
    imgs=[]
    # 创建保存图像的空列表
    labels=[]
...
# 利用样本数据data的大小查看数据集的大小num_example
arr=np.arange(num_example)
# 利用numpy中的arange函数生成一个跟样本数据集一样大小的等差数组
np.random.shuffle(arr)
# 利用shuffle函数打乱创建的等差数组的顺序
data=data[arr]
# 利用打乱的数组对样本数据进行打乱
label=label[arr]
# 利用打乱的数组对样本标签同流程进行打乱
...
# 5 create model 
# 创建卷积神经网络模型,该模型是整个实验的核心,原始的模型只有5层隐藏层,包括两个卷积层,两个池化层,一个全连接层,
# 但由于模型的识别效果较差,所以模型做了一系列的调整,
# 直到现在的稳定版本为止,包括11个隐藏层,包括4个卷积层,4个池化层,3个全连接层。
def model(input_tensor, train, regularizer):
    # 定义卷积神经网络模型model，三个变量分别是输入张量input_tensor，用于区分训练过程还是验证过程的train，
    # 正则项regularizer。
    with tf.variable_scope('layer1-conv1'):
        ...
        # 定义变量偏置：bias，名称bias，[32]代表当前层的深度；initializer代表偏置的初始化，
        # 用函数tf.constant_initializer将其初始化为0，也可以初始化为tf.zeros_initializer或者
        # tf.ones_initializer
        conv1 = tf.nn.conv2d(input_tensor, conv1_weights, strides=[1, 1, 1, 1], padding='SAME')   
        # 上面为定义卷积层：input_tensor为当前层的节点矩阵；
        # conv1_weights代表卷积层的权重；strides为不同方向上面的步长；
        # padding标识填充，有两种方式，SAME表示用0填充，“VALID”表示不填充。
        relu1 = tf.nn.sigmoid(tf.nn.bias_add(conv1, conv1_biases))
        # 定义激活函数：利用bias_add给每个节点都加上偏置项，然后利用relu函数去线性化

    with tf.name_scope("layer2-pool1"):
        #定义一个：layer2-pool1（用于op）
        # 池化层可以优先缩小矩阵的尺寸，从而减小最后全连接层当中的参数；池化层既可以加快计算速度，也可以防止过拟合。
        pool1 = tf.nn.max_pool(relu1, ksize = [1,2,2,1],strides=[1,2,2,1],padding="VALID") 
        # ksize代表pool窗口的尺寸，首尾两个数必须是1，ksize最常用[1,2,2,1]和[1,3,3,1]；
        # strides代表filter的步长，首尾两个数必须为1；padding代表填充方式；
...
# 定义正则项：这里主要用L2正则项，用于防止过拟合，提升模型的泛化能力；
...
logits_eval = tf.multiply(logits,b,'logits_eval')      # 常数与矩阵相乘

#6.loss,optimizer and accuracy 
#loss和acc：
loss=tf.nn.sparse_softmax_cross_entropy_with_logits(logits=logits, labels=y_)
# 定义损失函数：计算预测标签logits和原始标签y_之间的sparse交叉熵
