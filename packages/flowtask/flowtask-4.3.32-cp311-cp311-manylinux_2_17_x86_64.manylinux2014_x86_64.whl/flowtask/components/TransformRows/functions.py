...
def to_time(df, field: str, replace_nulls: bool = False, value: str = '00:00:00'):
    try:
...
def drop_column(df: pandas.DataFrame, field=''):
    return df.drop([field], axis=1)


def add_days(df: pandas.DataFrame, field='', column='', days=1):
    df[field] = df[column] + pandas.DateOffset(days=days)
    return df


def substract_days(df: pandas.DataFrame, field='', column='', days=1):
    df[field] = df[column] - pandas.DateOffset(days=days)
    return df


def rename_column(df: pandas.DataFrame, field='', rename=''):
    return df.rename(columns={field: rename})


def regex_match(df: pandas.DataFrame, field='', column='', regex=''):
    df[field] = df[column].str.extract(regex)
    return df


def zerofill(df: pandas.DataFrame, field='', num_zeros=1):
    """ Fill with zeroes an string until the length """
    df[field] = df[field].astype(str).str.zfill(num_zeros)
    return df


def pad(df: pandas.DataFrame, field='', num_chars=4, side='left', char='0'):
...
def concat(df: pandas.DataFrame, field='', columns=[], separator=' '):
    try:
        if columns and isinstance(columns, list):
...
def split_array(df: pandas.DataFrame, field='', column='', separator=' ', trim=False):
    try:
        col = column if column else field
...
def split(df: pandas.DataFrame, field: str = '', column: str = '', separator: str = ' ', index: int = 0, is_integer: bool = False) -> pandas.DataFrame:
    """
    This function takes a pandas DataFrame and splits the values in a specified column into multiple values based on a specified separator,
    and stores the resulting split values in a new column in the same DataFrame.

    :param df: pandas DataFrame to be modified.
    :param field: Optional, name of the new column to store the split values. Defaults to an empty string ('') which will store the split values in a column with the same name as the original column, but with the index of the split value appended.
    :param column: Name of the column in the df DataFrame to be split.
    :param separator: Optional, the character or string used as the separator to split the values in the specified column. Defaults to a space (' ').
    :param index: Optional, the index of the split value to be extracted and stored in the new column. Defaults to 0 which will store the first split value.
    :param is_integer: Optional, a boolean flag that determines whether the split value should be converted to an integer data type. Defaults to False.
    :return: Modified pandas DataFrame with the split values stored in a new column.
    """
    try:
        # Split the values in the specified column using the provided separator and store the resulting split values in a new column
        if not field:
...
def slice(df: pandas.DataFrame, field='', column='', start=0, end=1, is_integer: bool = False):
...
def case(df: pandas.DataFrame, field='', column='', condition: Any = None, match=None, notmatch=None):
    """Case.
...
def divide(df: pandas.DataFrame, field='', divisor=100):
...
def nullif(df: pandas.DataFrame, field='', chars=[]):
...
def capitalize(df: pandas.DataFrame, field=''):
    df[field] = df[field].str.title()
    return df


def to_round(df, field='', ndecimals: int = 2):
    try:
...
def uppercase(df: pandas.DataFrame, field='', from_column: str = None):
...
def lowercase(df: pandas.DataFrame, field='', from_column: str = None):
    if from_column is not None:
        column = from_column
    else:
        column = field
...
def both_strip(df: pandas.DataFrame, field='', character=' '):
    df[field] = df[field].str.strip(character)
    return df


def trim(df: pandas.DataFrame, field='', characters=' ', remove_empty: bool = False):
...
def right_strip(df: pandas.DataFrame, field='', character=' '):
    df[field] = df[field].str.rstrip(character)
    return df


def replace_regex(df: pandas.DataFrame, field='', to_replace='', value=''):
    try:
...
def ereplace(df: pandas.DataFrame, field='', columns=[], newvalue=''):
    col1 = columns[0]
    col2 = columns[1]
...
def to_string(
    df: pandas.DataFrame, field: str, remove_nan: bool = False) -> pandas.DataFrame:
...
def convert_to_array(df: pandas.DataFrame, field='', separator=',', remove_empty: bool = False):
    df[field] = df[field].astype(str)
...
def convert_to_datetime(df: pandas.DataFrame, field='', remove_nat=False):
    try:
        df[field] = pandas.to_datetime(
            df[field], errors='coerce', infer_datetime_format=True)
...
def convert_to_time(df: pandas.DataFrame, field='', format="%H:%M:%S", not_null=False):
    df[field] = df[field].where(df[field].notnull(), None)
    df[field] = pandas.to_datetime(
...
def column_to_date(df: pandas.DataFrame, field='', column='', format="%Y-%m-%d"):
    if not column:
        column = field
...
def datetime_format(df: pandas.DataFrame, field='', column='', format="%Y-%m-%d"):
    try:
        if not column:
            column = field
...
def column_to_integer(df: pandas.DataFrame, field='', column=''):
    df[field] = pandas.to_numeric(df[column], errors='coerce')
    df[field] = df[field].astype('Int64')
    return df


def convert_to_date(df: pandas.DataFrame, field='', format="%Y-%m-%d", remove_nat=False):
    df[field] = pandas.to_datetime(df[field], format=format, errors='coerce')
    df[field] = df[field].astype('datetime64[ns]')
...
def string_to_date(df: pandas.DataFrame, field='', column='', format="%Y-%m-%d"):
    df[field] = pandas.to_datetime(df[column], format=format, errors='coerce')
...
def num_formatter(n):
    if type(n) == str:
        return f"-{n.rstrip('-').lstrip('(').rstrip(')')}" if n.endswith('-') or n.startswith('(') else n.replace(',', '.')
    else:
        return n


def convert_to_numeric(df, field: str, remove_nan: bool = True, fix_negatives: bool = False):
...
def convert_to_integer(df: pandas.DataFrame, field='', not_null=False, fix_negatives: bool = False):
    try:
        if fix_negatives is True:
            # mask = df[field].str.endswith('-')
            # df.loc[mask, field] = '-' + df.loc[mask, field].str[:-1]
...
def convert_to_boolean(df: pandas.DataFrame, field='', boolDict={'True': True, 'False': False}, nan: bool = False):
    if field not in df.columns:
        # column doesn't exists
        df = df.assign(field=nan)
    try:
        df[field] = df[field].astype(str).replace(boolDict)
...
def from_currency(df, field='', symbol='$', remove_nan=True):
...
def to_percentile(df, field='', symbol='%', divisor=None, remove_nan=True):
    df[field] = df[field].replace(
        '[\\{},) ]'.format(symbol), '', regex=True
    ).replace(
        '[(]', '-', regex=True
...
def split_cols(df, field='', separator=',', columns=[]):
    if isinstance(columns, list):
        df[columns] = df[field].str.split(separator, expand=True)
    return df


def startofweek(df, field='', column=''):
    df[field] = df[column] - \
...
def extract(df, field='', column='', to_date=None, value='day'):
    if not column:
        column = field
...
def to_numeric(df, field: str, remove_alpha: bool = True, to_integer: bool = False):
    """
    Converts a column of a pandas DataFrame to numeric values, optionally removing non-numeric characters and converting to integer.

    Args:
    - df (pandas.DataFrame): The DataFrame containing the column to be converted.
    - field (str): The name of the column to be converted.
    - remove_alpha (bool, optional): If True, removes non-numeric characters from the column before converting. Default is True.
    - to_integer (bool, optional): If True, converts the column to integer dtype after converting to numeric. Default is False.
