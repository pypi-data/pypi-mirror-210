...
from flowtask.conf import TEMPLATE_DIR
...
def getTemplateHandler(newdir: str = None):
    if newdir:
        ...
        return TemplateHandler(directory=TEMPLATE_DIR)
...
class TemplateHandler(object):
    ...
    def __init__(self, directory: Path, **kwargs):
        ...
        if not self.path.exists():
            raise RuntimeError(
                f'DI: template directory {directory} does not exist'
            )
        if 'config' in kwargs:
        ...
        try:
            ...
            self._strparser = Environment(loader=BaseLoader)
