...
class WarehouseOnBoardingTest(TestCase):
    ...
    @patch.object(WarehouseOnboardingService, "onboard")
    def test_airflow_flow(self, onboard_mock):
        expected_options = {
            **{"connectionType": "airflow", "warehouseType": "etl"},
            **_SAMPLE_BASE_OPTIONS,
        }

        self._service.onboard_airflow(**_SAMPLE_BASE_OPTIONS)
        onboard_mock.assert_called_once_with(
            validation_query=TEST_AIRFLOW_CRED_MUTATION,
            validation_response="testAirflowCredentialsV2",
