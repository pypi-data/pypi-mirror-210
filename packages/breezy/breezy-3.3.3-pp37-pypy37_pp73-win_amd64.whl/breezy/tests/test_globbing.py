...
class TestGlobster(TestCase):
    ...
    def test_asterisk(self):
        self.assertMatch([
            ...
            ('*bar',
             ['bar', 'foobar', 'foo\\nbar', 'foo.bar', 'foo/bar',
              'foo/foobar', 'foo/f.bar', '.bar', 'foo/.bar'],
             []),
            ])
    ...
    def test_double_asterisk(self):
        self.assertMatch([
            ...
            ('**/bar',
             ['bar', 'foo/bar'],
             ['foobar', 'foo.bar', 'foo/foobar', 'foo/f.bar',
              '.bar', 'foo/.bar']),
    ...
    def test_leading_dot_slash(self):
        self.assertMatch([
            ('./foo',
             ['foo'],
             ['\u8336/foo', 'barfoo', 'x/y/foo']),
            ('./f*',
             ['foo'],
             ['foo/bar', 'foo/.bar', 'x/foo/y']),
            ])
    ...
    def test_trailing_slash(self):
        self.assertMatch([
            ...
            ('.\\foo\\',
             ['foo'],
             ['foo/', '\u8336/foo', 'barfoo', 'x/y/foo']),
            ])

    def test_leading_asterisk_dot(self):
        self.assertMatch([
            ('*.x',
             ['foo/bar/baz.x', '\u8336/Q.x', 'foo.y.x', '.foo.x',
              'bar/.foo.x', '.x', ],
             ['foo.x.y']),
            ('foo/*.bar',
             ['foo/b.bar', 'foo/a.b.bar', 'foo/.bar'],
             ['foo/bar']),
            ('*.~*',
             ['foo.py.~1~', '.foo.py.~1~'],
             []),
...
class TestNormalizePattern(TestCase):
    ...
    def test_mixed_slashes(self):
        ...
        self.assertEqual(
            '/foo/bar', normalize_pattern('\\/\\foo//\\///bar/\\\\/'))
