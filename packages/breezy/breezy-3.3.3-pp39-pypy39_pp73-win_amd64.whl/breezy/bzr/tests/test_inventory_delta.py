...
"""
    ...
    def test_parse_delete(self):
        ...
        delta = parse_result[4]
        self.assertEqual(
            ('old-file', None, b'deleted-id', None), delta[-1])
...
class TestSerialization(TestCase):
    ...
    def test_tree_reference_disabled(self):
        ...
        non_root = new_inv.make_entry(
            'tree-reference', 'foo', root.file_id, b'id')
        non_root.revision = b'changed'
    ...
    def test_to_inventory_torture(self):
        ...
        delta = [
            ...
            # an old root:
            ('', 'old-root', b'TREE_ROOT',
                make_entry('directory', 'subdir-now', b'new-root-id',
                           b'TREE_ROOT', revision=b'moved-root')),
            # a file that followed the root move
            ('under-old-root', 'old-root/under-old-root', b'moved-id',
                make_entry('file', 'under-old-root', b'TREE_ROOT',
            ...
            ('ref', 'ref', b'ref-id',
                make_entry('tree-reference', 'ref', b'new-root-id', b'ref-id',
                           reference_revision=b'tree-reference-id',
                           revision=b'new-rev')),
            # a symlink now in a deep dir
            ('dir/link', 'old-root/dir/link', b'link-id',
                make_entry('symlink', 'link', b'deep-id', b'link-id',
                           symlink_target='target', revision=b'new-rev')),
            # a deep dir
            ('dir', 'old-root/dir', b'deep-id',
                make_entry('directory', 'dir', b'TREE_ROOT', b'deep-id',
                           revision=b'new-rev')),
            # a file with an exec bit set
            (None, 'configure', b'exec-id',
