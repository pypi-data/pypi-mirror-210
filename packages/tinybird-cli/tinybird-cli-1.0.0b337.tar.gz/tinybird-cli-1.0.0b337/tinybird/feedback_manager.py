...
class FeedbackManager:
    ...
    error_setting_copy_node = error_message("Failed setting node to be used to copy {node} on pipe {pipe}: {error}")
    ...
    error_not_allowed_in_main_branch = error_message("Command disabled for 'production' Environment")
    ...
    error_switching_to_main = error_message("** Unable to switch to production Workspace. Need to authenticate again, use 'tb auth")
...
Ready? """)
    ...
    info_conection_already_exists = info_message("Connection {name} already exists. Use 'tb connection ls' to list your connections")
...
**    $ tb datasource rm {datasource}""")
    ...
    info_copy_from_main_job_url = info_message("** Copy from production Workspace to '{datasource_name}' job url {url}")
...
""")
    ...
    success_workspace_data_branch = success_message("** Partitions from production Workspace have been attached to the Environment")
