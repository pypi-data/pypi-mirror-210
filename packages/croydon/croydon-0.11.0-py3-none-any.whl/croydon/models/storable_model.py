...
from ..types import TModel, KwArgs
...
class StorableModel(BaseModel):
    ...
    def aggregate(cls,
                  ...
                  query: Optional[Dict[str, Any]] = None,
                  **kwargs: KwArgs) -> AsyncIOMotorCursor:
        if not query:
            query = {}
    ...
    async def find_ids(cls,
                       ...
                       **kwargs: KwArgs) -> List[ObjectId]:
        objs = await cls.find_projected(query, **kwargs).to_list(max_count)
        return [obj["_id"] for obj in objs]

    @classmethod
    async def find_one(cls: Type[TModel], query: Dict[str, Any], **kwargs: KwArgs) -> Optional[TModel]:
    ...
    async def _cache_get(cls: Type[TModel],
        ...
        if await ctx.cache_l1.has(cache_key):
            data = await ctx.cache_l1.get(cache_key)
            td = time() - t1
            ctx.log.debug(
                "%s L1 hit %s %.3f secs", ctx.cache_l1.__class__.__name__, cache_key, td
            )
            return ctor(data)

        if await ctx.cache_l2.has(cache_key):
            data = await ctx.cache_l2.get(cache_key)
            await ctx.cache_l1.set(cache_key, data)
            td = time() - t1
            ctx.log.debug(
        ...
        if obj:
            ...
            await ctx.cache_l2.set(cache_key, data)
            await ctx.cache_l1.set(cache_key, data)
    ...
    async def invalidate(self, **kwargs: KwArgs) -> None:
    ...
    async def _invalidate(cache_key: str) -> None:
        if await ctx.cache_l1.delete(cache_key):
            ctx.log.debug("%s delete %s", ctx.cache_l1.NAME, cache_key)
        if await ctx.cache_l2.delete(cache_key):
            ctx.log.debug("%s delete %s", ctx.cache_l2.NAME, cache_key)
