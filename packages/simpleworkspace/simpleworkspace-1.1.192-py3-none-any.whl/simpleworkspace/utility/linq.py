...
class LINQ(Iterator[_T]):
    ...
    def Select(self, selector: Callable[[_T], Any]):
        """
    ...
    def Batch(self, chunkSize: int):
        ...
        >>> LINQ([1, 2, 3, 4, 5]).Batch(2).ToList()
        [[1,2], [3,4], [5]]
        """
        from more_itertools import batched
        return LINQ(batched(self, chunkSize))
    
    def BatchEvenly(self, totalChunks:int):
        """
        Batches the elements of the iterable evenly into the fixed number of chunks.
        Keep in mind that returned number of chunks might be less than totalChunks if the iterable has less items

        :param totalChunks: The total number of chunks to create.

        Example usage:
        
        >>> LINQ([1, 2, 3, 4, 5]).BatchEvenly(2).ToList()
        [[1,3,5], [2,4]]
        """
