...
class ThreePhaseSwitchBox:
    ...
    def setMux(self,phase):
        ...
        logging.debug(os.path.basename(__file__) + ": Sending ModBus Request: " + str(request))
        tsock.send(request)
        while True:
            try:
                response = tsock.recv(1024)
                logging.debug(os.path.basename(__file__) + ": Received ModBus Response: " + response)
            except:
                   break
        tsock.close()
        ...
        # delay to wait for the relay to physcially switch and settle
        sleep(1.0)
    ...
    # returns a list of [ALIAS,IP] lists, where NAME=ET-2260
    ...
    def discover():
        ...
        for response in responses:
            ...
            if strdict["NAME"] == "ET-2260":
                # create a list entry from the alias and ip address
                devices.append([strdict['ALIAS'],strdict['IP']])
