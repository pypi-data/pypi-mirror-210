...
class OTPmeJob(object):
    ...
    def start_job(self):
        ...
        if self.start_process:
            multiprocessing.atfork(quiet=True, exit_on_signal=True)
        ...
        ## Inform clusterd about new bunch (from this job) of objects to sync.
        #if config.daemon_mode:
        #    bunch_handler = ClusterJournalBunchHandler(self.uuid)
        #    bunch_handler.finish()
        #    #try:
        #    #    comm_queue = multiprocessing.InterProcessQueue("clusterd-object-sync")
        #    #except Exception as e:
        #    #    msg = "Failed to init interprocess queue: %s" % e
        #    #    self.logger.critical(msg)
        #    #comm_handler = comm_queue.get_handler("api")
        #    #comm_handler.send(recipient="clusterd-conn-handler", command=self.uuid)
