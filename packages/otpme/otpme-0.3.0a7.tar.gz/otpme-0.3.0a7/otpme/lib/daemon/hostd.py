...
class HostDaemon(OTPmeDaemon):
    ...
    def __init__(self, *args, **kwargs):
        self.replay_transactions_child = None
    ...
    def get_sync_connection(self, realm=None, site=None,
        ...
        if node:
            ...
            if config.site_master_node or force_site_address:
    ...
    def get_unsync_sites(self, timeout=None):
        ...
        if not config.site_master_node:
            return
    ...
    def replay_transactions(self):
        """ Replay leftover transactions. """
        if self.replay_transactions_child:
            if self.replay_transactions_child.is_alive():
                return
        transactions = backend.get_file_transactions()
        transactions += backend.get_object_transactions()
        if not transactions:
            return
        # Create child process.
        child = multiprocessing.start_process(name=self.name,
                            target=self._replay_transactions,
                            join=True)
        self.replay_transactions_child = child

    def _replay_transactions(self):
        """ Replay leftover transactions. """
        # Set proctitle for new child process.
        self.set_proctitle(proctitle="Replay transactions")
        # Handle multiprocessing stuff.
        multiprocessing.atfork(exit_on_signal=True)
        # Replay transactions.
    ...
    def _run(self, **kwargs):
        ...
        while True:
            try:
                ...
                # Make sure there are no transactions leftover (e.g. after
                # daemon crash).
                if (now - last_transaction_check) >= transaction_check_interval:
