...
class OTPmeSyncP1(OTPmeServer1):
    ...
    def get_site(self, realm, site):
        ...
        # Get master node object config.
        site_master = backend.get_object(object_type="node", uuid=site.master)
        if not site_master:
            return
        object_config = site_master.get_sync_config(peer=self.peer)
        node_config = json.encode(object_config, encoding="hex")

        # Add master node units.
        unit_uuid = site_master.unit_uuid
        while unit_uuid:
        ...
        site_objects.append([site_master.oid.full_oid, node_config])
    ...
    def _process(self, command, command_args):
        ...
        # Make sure orphan transactions are applied before starting a new sync.
        ...
        if self.peer.type == "node" \
        and not config.realm_master_node \
        and not config.site_master_node:
