...
class OTPmeJoinP1(OTPmeServer1):
    ...
    def join_realm(self, site, host_type, host_name,
        ...
        if host.site_uuid != config.site_uuid:
            ...
            # Check if the joining node is the site master.
            if host.uuid != s.master:
    ...
    def finish_join(self, host, host_public_key, callback,
        ...
        if host.type == "node":
            ...
            for node_name in online_nodes:
                ...
                if clusterd_conn:
                    try:
                        clusterd_conn.write(host.oid.full_oid,
                                            host.object_config.copy(),
                                            host.last_modified,
                                            host.last_used)
                    except Exception as e:
    ...
    def _process(self, command, command_args):
        ...
        if command == "join":
            ...
            if host and host.type == "node":
                if self.request_site.master \
                and self.request_site.name != config.site:
                    site_master = backend.get_object(object_type="node",
                                                    uuid=self.request_site.master)
                    if site_master and host.uuid != site_master.uuid:
                        message = (_("Site master node already exists: %s")
                                    % site_master.fqdn)
                        status = False
                        self.logger.warning(message)
                        return self.build_response(status, message)
