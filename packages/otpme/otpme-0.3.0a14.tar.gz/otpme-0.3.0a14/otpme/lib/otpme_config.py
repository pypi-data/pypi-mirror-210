...
class OTPmeConfig(object):
    ...
    @property
    def site_master(self):
        from otpme.lib import backend
        master_node = backend.get_object(object_type="node",
                                        uuid=self.site_master_uuid)
        if not master_node:
            return
        return master_node.name

    @property
    def site_master_uuid(self):
        from otpme.lib import backend
        return_attributes = ['master']
        result = backend.search(object_type="site",
                                attribute="name",
                                value=self.site,
                                return_attributes=return_attributes)
        if not result:
            msg = "Unable to get site master: %s" % self.site
            raise OTPmeException(msg)
        site_master = result[0]
        return site_master

    @property
    def site_master_address(self):
        from otpme.lib import backend
        master_node = backend.get_object(object_type="node",
                                        uuid=self.site_master_uuid)
        if not master_node:
            return
        return master_node.fqdn

    @property
    def site_master_node(self):
        if not self.uuid:
            return
        if not self.site_uuid:
            return
        if not self.site_master_uuid:
            return
        if not self.realm_master_uuid:
            return
        if self.uuid == self.site_master_uuid:
            return True
        return False
