...
class Site(OTPmeObject):
    ...
    @object_lock()
    def master_failover(self, new_master_node, run_policies=True,
        callback=default_callback, _caller="API", **kwargs):
        """ Set new master node for our site. """
        node = Node(name=new_master_node, realm=self.realm, site=self.name)
        if not node.exists():
            msg = (_("Unknown node: %s") % new_master_node)
            return callback.error(msg)

        if self.master == node.uuid:
            msg = (_("Node '%s' is already the master node.") % node.name)
            return callback.error(msg, exception=AlreadyMaster)
        ...
        if run_policies:
            try:
                self.run_policies("modify",
                                callback=callback,
                                _caller=_caller)
                self.run_policies("master_failover",
                                callback=callback,
                                _caller=_caller)
            except Exception as e:
                return callback.error()

        self.master = node.uuid
        self.update_index("master", self.master)
    ...
    def show_config(self, callback=default_callback, **kwargs):
        ...
        if self.master:
            ...
            or self.verify_acl("master_failover"):
                node = backend.get_object(object_type="node", uuid=self.master)
                if node:
