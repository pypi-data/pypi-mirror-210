...
def register():
    ...
    multiprocessing.register_shared_list("radius_reload_queue")
...
def cluster_radius_reload():
    ...
    try:
        multiprocessing.radius_reload_queue.append(reload_time)
    except ValueError:
        pass
    multiprocessing.cluster_event.set()

def cluster_sync_object(object_uuid, object_id, action,
    new_object_id=None, object_config=None):
    if config.one_node_setup:
        return
...
class ClusterData(object):
    def __init__(self, action, uuid, object_id, object_config=None,
        last_modified=None, new_object_id=None):
        self.action = action
        ...
        while True:
            ...
            for reload_time in multiprocessing.radius_reload_queue:
                if reload_time > time.time():
                    continue
                try:
                    multiprocessing.radius_reload_queue.remove(reload_time)
                except ValueError:
                    pass
