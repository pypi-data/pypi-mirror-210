...
def get_master_node(instance=False):
    ...
    if not result:
        msg = "Unknown site: %s" % config.site
        raise OTPmeException(msg)
    own_site = result[0]
    result = backend.search(object_type="node",
                            attribute="uuid",
                            value=own_site.master,
                            realm=config.realm,
                            return_type="name")
    if not result:
        msg = "Unknown master node: %s" % own_site.master
        raise OTPmeException(msg)
    master_node_name = result[0]
    if not instance:
