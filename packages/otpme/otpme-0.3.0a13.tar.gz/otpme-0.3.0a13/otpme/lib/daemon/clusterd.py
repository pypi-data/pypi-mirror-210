...
DEL_JOURNAL_NAME = "del_journal"
DEL_JOURNAL_DIR = os.path.join(config.spool_dir, DEL_JOURNAL_NAME)
...
def register():
    ...
    multiprocessing.register_shared_list("cluster_writes")
    multiprocessing.register_shared_list("cluster_renames")
    multiprocessing.register_shared_dict("cluster_objects")
    ...
    register_del_journal()

def register_del_journal():
    """ Directory to store cluster del journal. """
    config.register_config_var("del_journal_dir", str, DEL_JOURNAL_DIR)
    backend.register_data_dir(name=DEL_JOURNAL_NAME,
                            path=DEL_JOURNAL_DIR,
                            drop=True,
                            perms=0o770)
...
def cluster_sync_object(object_uuid, object_id, action, object_config=None,
    new_object_id=None, last_modified=None, checksum=None):
    if config.one_node_setup:
        return
    if multiprocessing.cluster_event is None:
        return
    if action == "delete":
        del_journal_entry = DelJournalEntry(object_uuid, object_id=object_id)
        del_journal_entry.add()
        try:
            multiprocessing.cluster_writes.remove(object_uuid)
        except ValueError:
            pass
        try:
            multiprocessing.cluster_renames.remove(object_uuid)
        except ValueError:
            pass
        multiprocessing.cluster_event.set()
        return
    if action == "rename":
        cluster_data = ClusterData(action=action,
                                    uuid=object_uuid,
                                    checksum=checksum,
                                    object_config=object_config,
                                    object_id=object_id.full_oid,
                                    new_object_id=new_object_id.full_oid)
        multiprocessing.cluster_renames.append(cluster_data)
    if action == "write":
        cluster_data = ClusterData(action=action,
                                    uuid=object_uuid,
                                    object_id=object_id.full_oid,
                                    ...
                                    checksum=checksum)
        multiprocessing.cluster_objects[cluster_data.time] = object_config
        multiprocessing.cluster_writes.append(cluster_data)
        #try:
        #    print("WWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWW", object_id)
        #    import pprint
        #    pprint.pprint(object_config['INDEX'])
...
class ClusterData(object):
    def __init__(self, action, uuid, object_id, last_modified=None,
        new_object_id=None, checksum=None):
        self.action = action
...
class DelJournalEntry(object):
    def __init__(self, object_uuid, object_id=None, timestamp=None):
        if timestamp is None:
            timestamp = time.time()
        self.object_id = object_id
        self.object_uuid = object_uuid
        self.timestamp = timestamp
        entry_id = "%s-%s" % (timestamp, object_uuid)
        self.entry_dir = os.path.join(config.del_journal_dir, entry_id)
    ...
    def delete(self):
        msg = ("Deleting del journal entry: %s" % self.object_id)
...
class ClusterDaemon(OTPmeDaemon):
    ...
    def get_event_name(self, node_name):
        client_event_name = "/clusterd-comm-%s" % node_name
    ...
    def start_initial_sync(self, node_name):
        ...
        while True:
            ...
            if node.name not in self.active_nodes:
                break
    ...
    def start_interprocess_comm(self):
        ...
        while True:
            ...
            for node_name in self.active_nodes:
                client_event_name = self.get_event_name(node_name)
                cluster_client_event = multiprocessing.Event(client_event_name)
                cluster_client_event.set()

    def do_master_node_election(self):
        """ Do master node election. """
        node_fails = {}
        # Set vote for this node. This is also done in clusterd protocol.
        try:
            node_vote = multiprocessing.node_votes[self.host_name]
        except:
            node_vote = random.random()
            current_master_node = stuff.get_master_node()
            if self.host_name == current_master_node:
                node_vote += 1
        ...
        while True:
            ...
            for x_node in self.active_nodes:
    ...
    def handle_node_connections(self):
        ...
        for node_name in all_nodes:
            ...
            self.online_nodes.remove(node_name)
        ...
        for node_name in all_nodes:
            ...
            self.active_nodes.append(node.name)
            self.online_nodes.append(node.name)
    ...
    def _start_cluster_communication(self):
        ...
        config.cluster_vote_participation = False

        get_master_node = False
        master_node = stuff.get_master_node()
        if master_node == self.host_name:
            get_master_node = True
        ...
        if get_master_node:
            try:
                clusterd_conn = connections.get("clusterd", timeout=5)
            except Exception as e:
                msg = "Failed to get cluster master connection: %s" % e
                self.logger.warning(msg)
                clusterd_conn = None
            if clusterd_conn:
                clusterd_conn.start_master_failover()
                try:
                    current_master_node = clusterd_conn.get_master_node()
                except UnknownMasterNode:
                    current_master_node = None
                if current_master_node is not None:
                    self.do_master_node_sync(current_master_node)
                node_vote = clusterd_conn.get_node_vote()
                own_node_vote = node_vote + 1 + (node_vote / 100)
                multiprocessing.node_votes[self.host_name] = own_node_vote
    ...
    def _start_cluster_connection(self, node_name):
        ...
        clusterd_conn = None
        while True:
            # Stop process if node is not active anymore.
            if node_name not in self.active_nodes:
                break
            # Wait for object cluster event.
            if not start_over:
                cluster_client_event.wait(timeout=3)
                cluster_client_event.clear()
            ...
            if do_init_sync:
                ...
                #multiprocessing.cluster_writes.clear()
                #multiprocessing.cluster_renames.clear()
            ...
            if not config.cluster_quorum:
                #msg = "Waiting for cluster quorum..."
                #self.logger.info(msg)
                start_over = True
                time.sleep(1)
                continue
            ...
            for sync_time in multiprocessing.nsscache_sync_queue:
                ...
                # Send sync request.
                try:
                    clusterd_conn.do_nsscache_sync()
                except Exception as e:
                    start_over = True
            ...
            for reload_time in multiprocessing.radius_reload_queue:
                ...
                try:
                    clusterd_conn.do_radius_reload()
                except Exception as e:
                    start_over = True
                ...
                self.logger.info(msg)
            # Handle cluster write requests.
            sync_objects = set(list(multiprocessing.cluster_writes)
                            + list(multiprocessing.cluster_renames))
            x_sort = lambda x: x.time
            sync_objects_sorted = sorted(sync_objects, key=x_sort)
            for object_data in sync_objects_sorted:
                ...
                checksum_already_written = False
                if object_checksum in node_checksums:
                    checksum_already_written = True
                else:
                    node_checksums.append(object_checksum)
                if len(node_checksums) >= 1024:
                    node_checksums = node_checksums[512:]

                # Get object ID.
                x_oid = oid.get(object_id)

                try:
                    #sync_node_list = multiprocessing.sync_nodes[object_uuid]
                    sync_node_list = multiprocessing.sync_nodes[object_data.time]
                except:
                    sync_node_list = []
                if node_name not in sync_node_list:
                    # Write object to peer.
                    if action == "write" and not checksum_already_written:
                        try:
                            object_config = multiprocessing.cluster_objects[object_data.time]
                        except KeyError:
                            continue
                        last_modified = object_data.last_modified
                        try:
                            last_used = backend.get_last_used(x_oid.realm,
                                                            x_oid.site,
                                                            x_oid.object_type,
                                                            object_uuid)
                        except Exception as e:
                            msg = "Failed to get last used time: %s" % x_oid
                            self.logger.warning(msg)
                            continue
                        #msg = ("Sending write request to node: %s: %s"
                        #        % (node_name, object_id))
                        #self.logger.debug(msg)
                        #try:
                        #    print("wwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwww", object_id)
                        ...
                        #    pass
                        try:
                            write_status = clusterd_conn.write(object_id,
                                                            object_config,
                                                            last_modified,
                                                            last_used)
                        except Exception as e:
                        ...
                        if write_status == "done":
                            msg = ("Written object to node: %s: %s"
                                    % (node_name, object_id))
                            self.logger.debug(msg)
                    # Write object to peer.
                    if action == "rename":
                        new_object_id = object_data.new_object_id
                        #msg = ("Sending rename request to node: %s: %s: %s"
                        #        % (node_name, object_id, new_object_id))
                        #self.logger.debug(msg)
                        try:
                            rename_status = clusterd_conn.rename(object_id,
                                                                new_object_id)
                        except Exception as e:
                            start_over = True
                        ...
                        if rename_status == "done":
                            msg = ("Renamed object on node: %s: %s: %s"
                                    % (node_name, object_id, new_object_id))
                            self.logger.debug(msg)
                    # Re-read node list after object was sent.
                    try:
                        #sync_node_list = multiprocessing.sync_nodes[object_uuid]
                        sync_node_list = multiprocessing.sync_nodes[object_data.time]
                    except:
                        sync_node_list = []
                    sync_node_list.append(node_name)
                    #multiprocessing.sync_nodes[object_uuid] = sync_node_list
                    multiprocessing.sync_nodes[object_data.time] = sync_node_list
                ...
                for x_node_name in nodes:
                    if x_node_name == self.host_name:
                        continue
                    try:
                        #sync_node_list = multiprocessing.sync_nodes[object_uuid]
                        sync_node_list = multiprocessing.sync_nodes[object_data.time]
                        ...
                        break
                    if x_node_name in sync_node_list:
                        continue
                    if x_node_name not in self.online_nodes:
                        continue
                    #if x_node_name not in list(multiprocessing.init_sync_done):
                    #    continue
                    all_nodes_in_sync = False
                    break
                if not all_nodes_in_sync:
                    continue
                try:
                    multiprocessing.cluster_writes.remove(object_data)
                except ValueError:
                    pass
                try:
                    multiprocessing.cluster_renames.remove(object_data)
                except ValueError:
                    pass
                try:
                    object_config = multiprocessing.cluster_objects.pop(object_data.time)
                except KeyError:
                    pass

            for object_time in list(multiprocessing.sync_nodes):
                found_write = False
                for x_data in multiprocessing.cluster_writes:
                    if x_data.time != object_time:
                        continue
                    found_write = True
                    break
                if not found_write:
                    for x_data in multiprocessing.cluster_renames:
                        if x_data.time != object_time:
                            continue
                        found_write = True
                        break
                if found_write:
                    continue
                try:
                    multiprocessing.sync_nodes.pop(object_time)
                except KeyError:
                    pass

            # Handle del journal.
            for journal_file in glob.glob(config.del_journal_dir + "/*"):
                journal_file_name = os.path.basename(journal_file)
                timestamp = journal_file_name.split("-")[0]
                object_uuid = "-".join(journal_file_name.split("-")[1:])
                del_journal_entry = DelJournalEntry(object_uuid, timestamp=timestamp)
                try:
                    del_journal_entry.load()
                except FileNotFoundError:
                    continue
                if node_name not in del_journal_entry.get_nodes():
                    object_id = del_journal_entry.object_id
                    try:
                        object_id = oid.get(object_id)
                    except Exception as e:
                        msg = "Failed to get object ID: %s: %s" % (object_id, e)
                        self.logger.warning(msg)
                        continue
                    # Delete object on peer.
                    #msg = ("Sending delete request to node: %s: %s"
                    #        % (node_name, object_id))
                    #self.logger.debug(msg)
                    try:
                        del_status = clusterd_conn.delete(object_uuid, object_id)
                    except Exception as e:
                        start_over = True
                    ...
                    msg = ("Deleted object on node: %s: %s"
                            % (node_name, object_id))
                    self.logger.debug(msg)
                    del_journal_entry.add_node(node_name)
                # Get all nodes.
                nodes = backend.search(object_type="node",
                                        attribute="uuid",
                                        value="*",
                ...
                for x_node_name in nodes:
                    if x_node_name == self.host_name:
                        continue
                    if x_node_name in del_journal_entry.get_nodes():
                        continue
                    all_nodes_in_sync = False
                    break
                if not all_nodes_in_sync:
                    continue
                del_journal_entry.delete()
    ...
    def _run(self, **kwargs):
        ...
        self.active_nodes = multiprocessing.get_list()
        self.online_nodes = multiprocessing.get_list()
        ...
        multiprocessing.cluster_writes.clear()
        multiprocessing.cluster_objects.clear()
        multiprocessing.cluster_renames.clear()
        multiprocessing.sync_nodes.clear()
