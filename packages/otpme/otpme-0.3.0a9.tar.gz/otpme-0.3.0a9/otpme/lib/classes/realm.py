...
class Realm(OTPmeObject):
    ...
    def init(self, realm_master, master_address, ca_key_len=None, ca_valid=None,
        ...
        master_node = backend.get_object(object_type="node",
                                    uuid=master_site.master)

        # Update host key files.
        host.update_data(host_cert=master_node.cert,
                        host_key=host_key,
        ...
        default_policies = config.get_default_policies(master_node.type)
        for policy_name in default_policies:
            result = backend.search(attribute="name",
                                    value=policy_name,
                                    object_type="policy",
            ...
            if not result:
                ...
                continue
            policy = result[0]
            if policy.uuid in master_node.policies:
                continue
            master_node.add_policy(policy.name, verify_acls=False)
