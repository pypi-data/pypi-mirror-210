...
class CommandHandler(object):
    ...
    def do_sync(self, sync_type="objects", resync=False, offline=False,
        ignore_changed_objects=False, max_tries=config.hostd_sync_retry_count,
    ...
    def renew_auth_key(self):
        ...
        if config.realm_master_node or config.site_master_node:
            config.use_api = True
    ...
    def join_realm(self, host_type, realm=None, site=None,
        domain=None, jotp=None, unit=None, no_daemon_start=False,
        host_key_len=None, site_key_len=None, trust_site_cert=False,
    ...
    def handle_cluster_status(self):
        ...
        for node in result:
            ...
            try:
                ...
                self.logger.warning(msg)
                try:
                    node_status[node.name]['status'] = "Down"
                except:
                    node_status[node.name] = {'status':"Down"}
                continue
            try:
        ...
        for x_node in node_status:
            ...
            if x_node_master == "Down":
                x_status_line = "Node: %s (Down)" % x_node
            else:
                x_status_line = ("Node: %s (Quorum: %s) (Master: %s)"
                                % (x_node, x_node_quorum, x_node_master))
