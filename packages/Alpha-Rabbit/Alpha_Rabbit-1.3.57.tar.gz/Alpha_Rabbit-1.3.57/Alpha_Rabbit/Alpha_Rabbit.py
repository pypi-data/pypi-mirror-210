...
class single_signal_test(object):
    ...
    def efficient_judge(self,qreturn,fc):
        ...
        if p_top_halfmean<=0.05 and p_lsret<=0.05:
            print(fc+'有用;头部10组平均收益一阶差分p值{},多空收益p值{}'.format(p_top_halfmean,p_lsret))
            return True
        # 且两个乘起来能跟原来的匹配；且另一个不能太差
            ...
            print(fc+'勉强有用;头部10组平均收益一阶差分p值{},整体平均收益一阶差分p值{},多空收益p值{}'.format(p_top_halfmean,p_groupmean,p_lsret))
            return True            
        return False
    ...
    def short_judge(self,qreturn,fc):
        ...
        if p_top5>0.1 and p_bottom5<=0.05 and (abs(groupmean.iloc[-1])<abs(groupmean.iloc[0])):
...
class multi_factor_test(object):
    ...
    def multif_barra_norm(self,allfactors,quantiles,Bft):
    ...
    def batch_factors_test(self,weight_method,allfactors,Price,quantiles,days,savedir,index_price = None,demean = False):
        ...
        for fc in list(allfactors):
            print(fc)
            res_df = sst.factor_prepare(allfactors,fc,quantiles)
            sst.factor_ret_test_sheet(weight_method,index_price,fc,res_df,Price,days,savedir[0],demean)
