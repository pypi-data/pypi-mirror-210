...
def _delvewheel_init_patch_1_3_7():
    ...
    if sys.version_info[:2] >= (3, 8) and not is_conda_cpython or sys.version_info[:2] >= (3, 10):
        ...
        load_order_filepath = os.path.join(libs_dir, '.load-order-jpegtran_cffi_wheels-0.5.3.dev0')
        if os.path.isfile(load_order_filepath):
            with open(os.path.join(libs_dir, '.load-order-jpegtran_cffi_wheels-0.5.3.dev0')) as file:
