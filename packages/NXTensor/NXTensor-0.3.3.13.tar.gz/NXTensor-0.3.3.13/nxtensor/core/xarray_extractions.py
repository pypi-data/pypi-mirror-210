...
def extract_square_region(dataset: xr.Dataset, variable_netcdf_attr_name: str, formatted_date: str,
    ...
    if x_size != y_size:
        raise ExtractionError(f'x_size and y_size must be the same (square extraction) but got x_size = {x_size} and' +
                              f' y_size = {y_size}')
    if x_size % 2 == 1.:
    ...
    with dask.config.set(scheduler=dask_scheduler):
        ...
        if lon_min < lon_west_edge or lon_max >= lon_east_edge:
            ...
            result = np.concatenate((left_data, right_data), axis=1)
        else:
