...
class Label2Num:
    def __init__(self):
        ...
        model_path = pkg_resources.resource_filename("KorDigits", "results/save_model/wo_sp_kt_model.pt")
    ...
    def _label_3(self, tagging):
        ...
        output = output.replace(".", "점", 1).replace("~", "에서 ").replace("-", " ").replace(":", "대")
        return output
    ...
    def _label_5(self, tagging):
        ...
        output = output.replace(".", '점', 1).replace("~", "에서 ").replace("+", "플러스").replace("-", " ").replace(":", "대")
        return output
    ...
    def _label_7(self, tagging):
        ...
        if wave_match:
            ...
            for c in zip(start, end):
                num_words = self.number_to_kor2(c)
...
if __name__ == '__main__':
    # data = pd.read_csv("data/num_test.txt", delimiter="\t", header=None)
    label2num = Label2Num()
    print(label2num.generate("금요일 19시 30분부터 시작된 수원 삼성과 인천 유나이티드 경기에서 1-2로 인천이 우승했다."))
