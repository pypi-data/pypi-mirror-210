...
def parse_args():  # pylint: disable=missing-function-docstring
    ...
    parser.add_argument('--work-dir', default='./work-dir', help='Directory to save output files.')
    parser.add_argument('--device', help='device used for conversion', default='cpu')
    parser.add_argument(
        '--log-level',
        help='set log level',
        default='INFO',
        choices=list(logging._nameToLevel.keys()),  # pylint: disable=W0212
    )
...
def export():  # pylint: disable=missing-function-docstring
    args = parse_args()
    ...
    if partition_cfgs is not None:
        ...
        for partition_cfg in partition_cfgs:  # type: ignore
