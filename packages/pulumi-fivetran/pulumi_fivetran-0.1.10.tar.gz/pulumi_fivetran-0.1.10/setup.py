...
class InstallPluginCommand(install):
    def run(self):
        install.run(self)
        try:
            check_call(['pulumi', 'plugin', 'install', 'resource', 'fivetran', PLUGIN_VERSION, '--server', 'https://github.com/benesch/pulumi-fivetran/releases/download/v0.1.10/'])
            ...
            if error.errno == errno.ENOENT:
                ...
                `pulumi plugin install resource fivetran {PLUGIN_VERSION}`
                """)
            else:
                raise
...
setup(name='pulumi_fivetran',
      ...
      url='https://github.com/benesch/pulumi-fivetran',
      project_urls={
