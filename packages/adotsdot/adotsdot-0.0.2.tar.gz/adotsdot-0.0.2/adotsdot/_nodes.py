...
def _diff_dict(prev, curr, schema, depth, is_list=False):
    ...
    for k in both:
        if is_list:
            ...
            new_schema = schema.get(k, default)

        child_node = diff(prev[k], curr[k], schema=new_schema, depth=depth)
...
def _diff_list(primitive, prev, curr, schema, depth):
    ...
    if isinstance(primitive, _SUPPORTED_PRIMITIVES):
        ...
        node = DictDiffNode(depth)
        inverted_prev = _invert_list(prev, schema=schema, key='_id')
        inverted_curr = _invert_list(curr, schema=schema, key='_id')
        node = _diff_dict(
...
def _invert_list(l, schema, key):
    """Invert a list on a unique key."""
    try:
        unique_id = schema[key]
    except (KeyError, TypeError):
        raise ValueError("No schema passed to be able to distinguish list of dictionaries %s" % str(l))

    return {e[unique_id]: e for e in l}
