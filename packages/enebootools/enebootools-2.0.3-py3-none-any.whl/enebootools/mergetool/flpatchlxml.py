...
class XMLFormatParser(object):
    ...
    def validate(self):
        for elem in self.format.xpath("assert/*"):
            ...
            if type(result) is object:
                result = bool(result is None)
            if not result:
                self.iface.debug(
                    "Fallo al validar la regla %s" % etree.tostring(elem, encoding="unicode")
                )
                return False
            else:
    ...
    def _evaluate(self, elem, from_elem=None):
        ...
        def sxpath(text, from_elem, **kwargs):
            try:
                ...
                self.iface.exception(
                    "EvaluateError", "Error evaluando  %s  %s" % (text, repr(kwargs))
                )
                return None
        ...
        try:
            if elem.tag == "empty":
                ...
                for subelem in elem:
                    value = self._evaluate(subelem, from_elem)
                    if not isinstance(subelem.tag, str):
                        continue
                    args.append(value)
                try:
                    ...
                    self.iface.error(
                        "Error ejecutando format2: %s %% %s" % (repr(format_string), repr(args))
                    )
                    raise
                return retval
                ...
                self.iface.warn(
                    "Descartando tag de evaluación desconocido: %s:%s" % (elem.tag, text)
                )
        except Exception as e:
    ...
    def load_entity(self, entity_name, context_info, search_xpath, context_items=[], root=None):
        ...
        for element in search:
            ...
            if parent is not None:
                if parent.tag == "context-information":
                    continue
            # if element.xpath("context-information"): continue
            if element.get("ctx-info-created") is not None:
                continue
    ...
    def load_entities(self, root=None, tag=None):
        ...
        for entity in self.entities:
            ...
            if tag:
                ...
                if tag not in self.entities_tags[entity_name] and tag != entity_name:
                    continue
            ...
            for search_xpath in entity.xpath("search/xpath"):
                ret = self.load_entity(
                    entity_name,
                    self.entities_ctx[entity_name] + self.default_ctx,
                    search_xpath.text.strip(),
                    self.context_items,
                    root,
                )
                if not ret:
    ...
    def clean_ctxid(self):
        ...
        if reindent_items:
            ...
            for element in self.root.iter():
                ...
                subelement = None
                for sub in element:
                    if sub.tail and parent_text is None:
                        parent_text = sub.tail
                    if len(sub) == 0:
                        continue
                    if sub.text is None:
                        continue
            ...
            if depth == -1:
                ...
                if parent_level is None:
                    ...
                    if grandparent is not None:
                        grandparent_level = grandparent.text

                    if grandparent_level:
                        increment = parent_level.replace(grandparent_level, "")
                    else:
                        increment = "    "
    ...
    def add_context_id(self, root=None):
        ...
        for element in root:
            if element.tag == "context-information":
                continue
...
class XMLDiffer(object):
    def __init__(
        self, iface, format, style, file_base, file_final=None, file_patch=None, recover=False
    ):
        self.iface = iface
        self.namespaces = {
            "xsl": "http://www.w3.org/1999/XSL/Transform",
        ...
        if file_patch:
            rbt = False  # Remove Blank Text
        else:
            rbt = True
        self.xbase = XMLFormatParser(
            self.iface, self.format, self.style, file_base, rbt, recover=recover
        )

        if not self.xbase.validate():
            self.iface.error(
                "El fichero base no es válido para el formato %s" % (self.format.get("name"))
            )
            raise ValueError
        if not self.xbase.load_entities():
            self.iface.error(
                "Error al cargar entidades del formato %s (fichero base)"
                % (self.format.get("name"))
            )
            raise ValueError

        self.xfinal = XMLFormatParser(
            self.iface, self.format, self.style, file_final, rbt, recover=recover
        )

        if not self.xfinal.validate():
            self.iface.error(
                "El fichero final no es válido para el formato %s" % (self.format.get("name"))
            )
            raise ValueError
        if not self.xfinal.load_entities():
            self.iface.error(
                "Error al cargar entidades del formato %s (fichero final)"
                % (self.format.get("name"))
            )
            raise ValueError
        if file_patch:
            parser = etree.XMLParser(
                ns_clean=True, remove_blank_text=True, recover=True, encoding=self.xbase.encoding
            )
    ...
    def final_output(self):
        if self.xfinal.root is not None:
            ...
            if isinstance(doc, etree._Element):
                return doctype + _xf(
                    doc, xml_declaration=False, cstring=True, encoding=self.xfinal.encoding
                )
            else:
    ...
    def compare_elems(self, base_elem, final_elem):
        ...
        for k, v in sorted(base.items()):
            ...
            if k[0] == "@":
                tag = "attribute"
                kwargs["name"] = k[1:]
                if kwargs["name"].startswith("ctx-info-"):
                    continue
            op = "update"
            if v is None:
                op = "create"
    ...
    def _get_ctxinfo_old(self, elem):
        ...
        if len(ctx) == 0:
            ...
            ctx = ";".join(["%s=%s" % (k, v) for k, v in sorted(dict(ctx[0].attrib).items())])
        return ctx
    ...
    def compare_subelems(self, base_elem, final_elem):
        base = [
            self.xbase.apply_one_id(subelem)
            for subelem in base_elem
            if subelem.tag != "context-information"
        ]
        final = [
            self.xfinal.apply_one_id(subelem)
            for subelem in final_elem
            if subelem.tag != "context-information"
        ]
    ...
    def resolve_select(self, element, select):
        ...
        while path:
            ...
            if newelement is None:
                return element, "/".join([p0] + path)
            elif newelement.get("ctx-id") != p0:
                self.iface.info2(
                    "Seleccionamos %s cuando buscabamos %s" % (newelement.get("ctx-id"), p0)
                )
            element = newelement
    ...
    def resolve_select2(self, element, select):
        ...
        if select.startswith("text()"):
            select = "text()"
        elif select.startswith("#text"):
            select = "text()"
        elif select == ".":
            select = "."
        tend = time.time()
    ...
    def _do_patch_action(self, action):
        ...
        if select is None:
            self.iface.warn(
                "Falta atributo 'select' en " + etree.tostring(action, encoding="unicode")
            )
            return
        ...
        if actionname == "update" and select == "text()":
            ...
            close_matches = difflib.get_close_matches(searching, alternatives, 4, 0.4)
            self.iface.info(
                "No se encontró el elemento %s entre %s y no se eliminó nada"
            ...
            self.iface.warn(
                "Accion no aplicada -> %s\t%s\t%s" % (actionname, element.get("ctx-id"), select)
            )
...
def diff_lxml(iface, base, final):
    ...
    format = config_tree.xpath("/etc/formats/format[@name=$format_name]", format_name=format_name)[
        0
    ]
    ...
    if len(styles) > 1:
        iface.warn(
            "Había más de un estilo con el nombre %s y hemos cargado el primero."
            % (repr(style_name))
        )
    ...
    try:
        ...
        try:
            ...
            file_final = open(final, "rb")
            xmldiff = XMLDiffer(
                iface, format, style, file_base=file_base, file_final=file_final, recover=True
            )
        except etree.XMLSyntaxError as e:
...
def patch_lxml(iface, patch, base):
    ...
    xmldiff = XMLDiffer(
        iface, format, style, file_base=file_base, file_final=file_final, file_patch=file_patch
    )
    ...
    if tdeltams > 300:
        ...
        iface.info2(
            "-- Time %.2f ms , %.2f ms sname , %.2f ms evaluating"
            % (
                tdeltams,
                xmldiff.xbase.time_sname * 1000 + xmldiff.xfinal.time_sname * 1000,
                xmldiff.xbase.time_evaluate * 1000 + xmldiff.xfinal.time_evaluate * 1000,
            )
        )
