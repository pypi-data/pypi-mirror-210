...
class MySQLStatementBuilder:
    ...
    def select(self, table: str, columns: List[str]):
        ...
        self.query += stmnt.create_select_statement(table, columns)
        return self

    def count(self, table: str):
        self.query += stmnt.create_count_statement(table)
        self.default_fetchtype = FetchType.FETCH_ONE
        return self

    def update(self, table: str, set_statement, values):
        self.query += stmnt.create_update_statement(table, set_statement)
        self.values.extend(values)
        return self

    def delete(self, table: str):
        self.query += stmnt.create_delete_statement(table)
        return self

    def order_by(self, columns: List[str], order: stmnt.Sort = None):
        self.query += stmnt.create_order_by_statement(columns, order)
        return self

    def offset(self, offset_count: int):
        self.query += stmnt.create_offset_statement(offset_count)
        return self

    def limit(self, limit_count: int):
        self.query += stmnt.create_limit_statement(limit_count)
        return self

    def inner_join(self, target_table, join_statement):
        self.query += stmnt.create_inner_join_statement(target_table, join_statement)
        return self
    ...
    def where(self, condition, condition_values: List[Any]):
        ...
        self.query += stmnt.create_prepared_where_statement(condition)
    ...
    def execute(self,
        ...
        if fetch_type is FetchType.FETCH_NONE and dictionary is True:
            Cnf.logger.error('FetchType cannot be None if dictionary is set to True.')
            raise InvalidStatementConfiguration('FetchType cannot be None if dictionary is set to True.')

        if no_logs is False:
            Cnf.logger.debug(f'Executing query "{self.query}" with values "{self.values}". fetch_type={fetch_type}')
    ...
    def execute_procedure(self, procedure: str, args: List) -> List[List[Any]]:
        ...
        """
        Cnf.logger.debug(f'executing stored procedure "{procedure}" with arguments {args}')
