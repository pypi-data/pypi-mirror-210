...
@click.command()
@click.argument("url")
@click.option("--baseline", default=DEFAULT_BASELINE_PATH, help="baseline file (yaml).")
def main(url, baseline):
    ...
    for line in b"".join(output).decode().split("\n"):
        if "Start" in line:
            ...
            click.echo(f" {'Cipher Suite Name (IANA/RFC)':41} Evaluation")
            ...
            if line.strip() == "-":
                ...
                try:
                    click.echo(
                        f" {cipher_suite:<41} "
                        + click.style(**baseline_suites[current_protocol][cipher_suite])
                    )
