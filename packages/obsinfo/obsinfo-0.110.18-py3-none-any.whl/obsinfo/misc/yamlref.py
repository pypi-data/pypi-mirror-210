...
class JsonRef(LazyProxy):
    ...
    def replace_refs(cls, obj, _recursive=False, **kwargs):
        ...
        ``kwargs`` include:
            ...
            jsonschema: Flag to turn on `JSON Schema mode <http://json-schema.org/latest/json-schema-core.html#anchor25>`_,
        ...
        try:
            if not isinstance(obj["$ref"], basestring):  # It's a $ref but not a string
                raise TypeError  # Dirty trick to skip the else: clause
        except (TypeError, LookupError):
            pass
        else:
