...
class Command(BaseCommand):
    ...
    def update_device_sni_status_data(self, device):
        ...
        try:
            ...
            self.stdout.write(self.task_error(text=f"Get data for serial number {device['sr_no']}", changed=False))
    ...
    def update_device_sni_summary_data(self, device):
        ...
        coverage_status = "Covered by Cisco SNTC" if covered else "Not covered by Cisco SNTC"
        ...
        try:
            if not device["service_contract_number"]:
                self.stdout.write(f"{device['sr_no']} has no service_contract_number")
            else:
        ...
        try:
            if not device["service_line_descr"]:
                self.stdout.write(f"{device['sr_no']} has no service_line_descr")
            else:
        ...
        try:
            if not device["warranty_type"]:
                self.stdout.write(f"{device['sr_no']} has no warranty_type")
            else:
        ...
        try:
            if not device["warranty_end_date"]:
                self.stdout.write(f"{device['sr_no']} has no warranty_end_date")
            else:
        ...
        try:
            if not device["covered_product_line_end_date"]:
                self.stdout.write(f"{device['sr_no']} has no covered_product_line_end_date")
            else:
            ...
            self.stdout.write(f"{device['sr_no']} has no coverage_end_date")
    ...
    def update_device_type_eox_data(self, pid, eox_data):
        ...
        try:
            ...
            if "Incorrect PID:" in eox_data["EOXRecord"][0]["EOXError"]["ErrorDescription"]:
                ...
                self.stdout.write(f"{pid} has no EoX support information announced")
            else:
        ...
        # Do nothing when JSON field does not exist
            ...
            self.stdout.write(f"{pid} has EoX support information announced")
        ...
        try:
            ...
            if not eox_data["EOXRecord"][0]["EOXExternalAnnouncementDate"]["value"]:
                self.stdout.write(f"{pid} has no eox_announcement_date")
            else:
        ...
        try:
            ...
            if not eox_data["EOXRecord"][0]["EndOfSaleDate"]["value"]:
                self.stdout.write(f"{pid} has no end_of_sale_date")
            else:
        ...
        try:
            if not eox_data["EOXRecord"][0]["EndOfSWMaintenanceReleases"]["value"]:
                self.stdout.write(f"{pid} has no end_of_sw_maintenance_releases")
            else:
        ...
        try:
            if not eox_data["EOXRecord"][0]["EndOfSecurityVulSupportDate"]["value"]:
                self.stdout.write(f"{pid} has no end_of_security_vul_support_date")
            else:
                ...
                self.stdout.write(f"{pid} end_of_security_vul_support_date: {end_of_security_vul_support_date}")
        ...
        try:
            if not eox_data["EOXRecord"][0]["EndOfRoutineFailureAnalysisDate"]["value"]:
                self.stdout.write(f"{pid} has no end_of_routine_failure_analysis_date")
            else:
                ...
                self.stdout.write(f"{pid} end_of_routine_failure_analysis_date: {end_of_routine_failure_analysis_date}")
        ...
        try:
            if not eox_data["EOXRecord"][0]["EndOfServiceContractRenewal"]["value"]:
                self.stdout.write(f"{pid} has no end_of_service_contract_renewal")
            else:
                ...
                self.stdout.write(f"{pid} end_of_service_contract_renewal: {end_of_service_contract_renewal}")
        ...
        try:
            if not eox_data["EOXRecord"][0]["LastDateOfSupport"]["value"]:
                self.stdout.write(f"{pid} has no last_date_of_support")
            else:
        ...
        try:
            if not eox_data["EOXRecord"][0]["EndOfSvcAttachDate"]["value"]:
                self.stdout.write(f"{pid} has no end_of_svc_attach_date")
            else:
    ...
    def get_product_ids(self, manufacturer):
        ...
        self.stdout.write(self.task_name(text="Get all PIDs"))
    ...
    def get_serial_numbers(self, manufacturer):
        ...
        self.stdout.write(self.task_name(text="Get all serial numbers"))
            ...
            try:
                ...
                for device in d:
                    ...
                    if not device.serial:
                        self.stdout.write(self.task_error(text=f"Get serial number for {device}", changed=False))
    ...
    def handle(self, *args, **kwargs):
        ...
        self.stdout.write(self.task_name(text="Get data for all PIDs"))

        for pid in product_ids:
            url = (f"{base_url}/supporttools/eox/rest/5/EOXByProductID/1/{pid}?responseencoding=json")
            api_call_response = requests.get(url=url, headers=api_call_headers, verify=False, timeout=self.REQUESTS_TIMEOUT)
        ...
        self.stdout.write(self.task_name(text="Get data for all serial numbers"))
