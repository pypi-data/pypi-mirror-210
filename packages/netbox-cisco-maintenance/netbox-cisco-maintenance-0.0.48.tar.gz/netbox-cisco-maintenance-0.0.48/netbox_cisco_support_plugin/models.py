...
class CiscoSupport(ChangeLoggedModel):
    ...
    coverage_status = models.TextField(help_text="Cisco support coverage status", blank=True, null=True)
...
class CiscoDeviceSoftware(ChangeLoggedModel):
    objects = RestrictedQuerySet.as_manager()

    device_software = models.OneToOneField(to="dcim.Device", on_delete=models.CASCADE)

    def __str__(self):
        return "%s Support" % self.device_software
    
    recommended_release = models.TextField(
        help_text="The recommended release from the Cisco support API",
        blank=True,
        null=True,
    )

    desired_release = models.TextField(
        help_text="The desired release which is defined as the current standard for the environment",
        blank=True,
        null=True,
    )

    current_release = models.TextField(
        help_text="The current installed release on the device",
        blank=True,
        null=True,
    )
