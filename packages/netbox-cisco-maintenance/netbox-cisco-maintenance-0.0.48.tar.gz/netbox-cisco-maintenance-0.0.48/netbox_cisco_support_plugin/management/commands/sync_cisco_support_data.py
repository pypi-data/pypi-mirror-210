...
from netbox_cisco_support_plugin.models import CiscoDeviceTypeSupport, CiscoSupport, CiscoDeviceSoftware
...
class Command(BaseCommand):
    ...
    def update_device_ss_data(self, pid, ss_data):
        try:
        ...
        # Check if CiscoDeviceSoftware record already exists
        try:
            ds = CiscoDeviceSoftware.objects.get(device_type=dt)
        # If not, create a new one for this Device Type
        except CiscoDeviceSoftware.DoesNotExist:
            ds = CiscoDeviceSoftware(device_type=dt)
    ...
    def get_device_types(self, manufacturer):
        ...
        try:
            ...
            self.stdout.write(self.task_error(text=task, changed=False))
            self.stdout.write(f"Manufacturer {m} - No Device Types")
    ...
    def handle(self, *args, **kwargs):
        ...
        # Step 1: Get all PIDs for all device types and get the recommended software release
        ...
        product_ids = [pid for pid in product_ids if not any(pid.startswith(prefix) for prefix in rm_prefixes)]
        # Remove pids if the match the condition for endswith
        rm_suffixes = ["AXI-E", "AXI-A"]
        product_ids = [pid for pid in product_ids if not any(pid.endswith(suffix) for suffix in rm_suffixes)]
        ...
        chg_suffixes = ["-A", "-E"]
        product_ids = [pid[:-2] if any(pid.endswith(suffix) for suffix in chg_suffixes) else pid for pid in product_ids]
        # Remove duplicated and empty pids in the final pid_list
        product_ids = [pid for pid in list(set(product_ids)) if pid]
        ...
        for pid in product_ids:
            ...
            if api_call_response.status_code == 200:
                ...
                self.update_device_ss_data(pid, data)
            else:
                # Show an error
        ...
        # Step 2: Get all PIDs for all Device Types of that particular manufacturer
        ...
        # Step 3: Get all Serial Numbers for all Devices of that particular manufacturer
