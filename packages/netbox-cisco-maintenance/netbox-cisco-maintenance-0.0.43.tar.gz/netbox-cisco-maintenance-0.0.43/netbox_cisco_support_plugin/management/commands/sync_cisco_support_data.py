            ...
            try:
                ...
                for device in d:
                    ...
                    if not device.serial:
                        self.stdout.write(self.task_error(text=f"Get serial number {device}", changed=False))
                        self.stdout.write(f"Found device {device} WITHOUT Serial Number - SKIPPING")
                        continue

                    self.stdout.write(self.task_info(text=f"Get serial number {device}", changed=False))
                    self.stdout.write(f"Found device {device} with Serial Number {device.serial}")

                    serial_numbers.append(device.serial)
            except Device.DoesNotExist:
                self.stdout.write(self.task_error(text=f"Get serial number {dt}", changed=False))
