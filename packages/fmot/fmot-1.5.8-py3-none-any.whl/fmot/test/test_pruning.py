...
class TestPruning:
    ...
    def test_compute(self):
        ...
        expected_output = torch.tensor([[0, 1, 1],
                                        [0, 1, 1],
                                        [0, 1, 1],
        ...
        # Check 2
        tensor = torch.tensor([[-5, 4, 4, 6, -7],
                               [5, -4, -4, 6, -7],
                               [5, -4, -4, 6, -7],
                               [5, -4, -4, 6, -7]], dtype=torch.float64)
    ...
    def test_conv1d_pruning(self):
        ...
        new_weight = torch.zeros(model.conv.weight.shape) + 1
        new_weight[0, 0, 0] = 0.
        model.conv.weight = nn.Parameter(new_weight)
        _pencil_conv1d_pruning(model.conv, 'weight', 1 / 8, 8)
        for i in range(out_channels):
            assert (model.conv.weight[i, 0, 0] == 0)
        ...
        new_weight[0, 0, 0] = 0.
        model.conv.weight = nn.Parameter(new_weight)
        _pencil_conv1d_pruning(model.conv, 'weight', 0.33, 8)
        for i in range(out_channels):
            assert (model.conv.weight[i, 0, 0] == 0)
