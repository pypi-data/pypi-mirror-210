...
class UTM(nn.Module):
    ...
    @eval_method
    def get_quantized_model(self, B_quant=8, N_quant=10, bw_conf='double', 
        interpolate=True):
    ...
    def get_mixed_precision_model(self, eta=0.3, B_quant=8, N_quant=10):
        ...
        cmodel = self.get_quantized_model(B_quant, N_quant, bw_conf='double', interpolate=True)
        optimize_mixed_precision(cmodel, objective, eta, inputs, targets)
        return cmodel

    @eval_method
    def get_fqir(self, B_quant=8, N_quant=10, bw_conf='double',
                 interpolate=True, eta=0.3):
        ...
        if bw_conf == 'mixed':
            ...
            cmodel = self.get_quantized_model(B_quant, N_quant, bw_conf,
                interpolate)
        return cmodel.trace()
    ...
    def get_quantization_nrmse(self, B_test=8, B_quant=8, N_quant=10, bw_conf='double', 
        ...
        y0, y1 = [torch.cat(list(map(torch.flatten, x))).detach().numpy() for x in [y0, y1]]
    ...
    def _test_fqir_runtime(model, graph, x, remove_batchdim=0, offby=0):
        ...
        for yy_torch, yy_fqir in zip(y_torch, y_fqir):
            ...
            assert np.max(diff) <= offby, ('Diff detected greater than allowed'
                f' off-by of {offby} in {n_off}/{diff.size} elements. max error: {np.max(diff)}')
            if np.max(diff) > 0:
                warnings.warn(f'Had nonzero FQIR-vs-QAT diff. Was {np.max(diff)}, within the set integer tolerance {offby}.')

    @torch.no_grad()
    def test_fqir_runtime(self, B_test=8, B_quant=8, N_quant=10,
                          bw_conf='double', interpolate=True, remove_batchdim=0):
        """Test that the fqir runtime outputs matches the quantized qat outputs"""
        qmodel = self.get_quantized_model(B_quant=B_quant, N_quant=N_quant, 
...
class TestLibrary:
    ...
    def __repr__(self):
        ...
        rep += '\n\t' + '\n\t'.join([f'{k}:  {str(v)}' for k, v in self.test_sets.items()])
        return rep
