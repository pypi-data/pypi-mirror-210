...
class ResNetBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, dilation,
        repeats=1, residual=True, scale=True, bias=False, batchnorm=True):
        super().__init__()
        self.layers = nn.ModuleList()
        i = 0
        for __ in range(repeats-1):
            self.layers.append(DSTemporalConv(
...
class DSTCResNet(torch.nn.Module):
    ...
    """
    def __init__(self, input_size, channel_sizes, kernel_sizes, dilations, 
        repeats, residuals, num_labels, scale=True, bias=False):
        super().__init__()
        num_blocks = len(channel_sizes)
        assert all([len(p) == num_blocks for p in [kernel_sizes, dilations, repeats,
                residuals]])
