...
class Mean(nn.Module):
    ...
    def _from_float(cls, parent, bw_conf, interpolate,
        ...
        return cls(dim=parent.dim, keepdim=parent.keepdim, 
            bitwidth=bw_conf.activations, biased=parent.biased)
...
class DepthWiseConvSummer(nn.Module):
    def __init__(self, in_channels, kernel_size, lin_list, bias, bitwidth):
        ...
        self.weight_quant = torch.nn.ModuleList(quantizers.ParameterQuantizer(bitwidth)
                                for _ in range(len(lin_list)))
        ...
        if self.bias:
            ...
            self.bias_quant = torch.nn.ModuleList(quantizers.ParameterQuantizer(bitwidth)
                                for i in range(len(lin_list))
                                if (i % self.kernel_size) == 0)
        ...
        for i, weight in enumerate(self.weight_list):
            if i % self.kernel_size == 0:
                ...
                if self.bias:
                    ...
                    y = self.add(y, self.bias_quant[bias_index](self.bias_list[bias_index]))
            else:
                y = self.add(y, self.mul(x_list[i % self.kernel_size], self.weight_quant[i](weight)))
    ...
    def _from_float(cls, parent, bw_conf, interpolate,
        observer=quantizers.DEFAULT_OBSERVERS['default'], **observer_kwargs):
        return cls(in_channels=parent.in_channels,
...
class LogMM(nn.Module):
    def __init__(self, act_bw, lut_bw, observer=quantizers.DEFAULT_OBSERVERS['default']):
        super().__init__()
        self.mm_big = Matmul(act_bw, observer=observer)
        self.mm_small = NoShiftMM(act_bw)
        self.lut_big = AddIdentityTLUT(torch.log, lut_bw, act_bw,
            observer=observer)
        self.lut_small = AddIdentityTLUT(torch.log, lut_bw, act_bw,
            observer=observer)
...
class LogEpsMM(nn.Module):
    def __init__(self, epsilon, act_bw, lut_bw, observer=quantizers.DEFAULT_OBSERVERS['default']):
        super().__init__()
        self.mm_big = Matmul(act_bw, observer=observer)
        self.mm_small = NoShiftMM(act_bw)
        self.lut_big = AddIdentityTLUT(torch.log, lut_bw, act_bw,
    ...
    def _from_float(cls, parent, bw_conf, interpolate, 
        observer=quantizers.DEFAULT_OBSERVERS['default'], **observer_kwargs):
        observer = partial(observer, **observer_kwargs)
        return cls(parent.epsilon, bw_conf.activations, bw_conf.lut, 
            observer=observer)
