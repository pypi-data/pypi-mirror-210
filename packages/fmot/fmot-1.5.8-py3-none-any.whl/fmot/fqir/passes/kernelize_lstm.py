...
def get_weights(arith: GraphProto, node: NodeProto, layer_idx: int,
    ...
    for name in ['weight_ih', 'weight_hh', 'bias_ih', 'bias_hh']:
        ...
        if weight is not None:
            ...
            if weight.ndim >= 2:
                ...
                dtype = 'fqint16'
            proto = TensorProto(f'{name}_l{layer_idx}', dtype, w_int.shape, 
                value=w_int, quanta=int(layer_consts[f'{name}_quanta']))
...
def get_simple_vvadd(name: str, dtype: str, length: int, quanta: int, 
    x: TensorProto, y: TensorProto, output: TensorProto=None) -> Tuple[TensorProto, NodeProto]:
    ...
    node = NodeProto(name, registry_v1['vvadd'], 
        inputs={'x': x, 'y': y},
        outputs=[z],
        constants={
            'shamt_x': s_x, 
...
def get_chunk(name: str, chunks: int, x: TensorProto) -> Tuple[List[TensorProto], NodeProto]:
    length_in = x.shape[-1]
    l_per = length_in // chunks
    outputs = [
        TensorProto(name=f'{name}_{i}', dtype=x.dtype, shape=[l_per], quanta=x.quanta) 
        for i in range(chunks)]
    node = NodeProto(name, registry_v1['chunk'],
        inputs={'x': x},
        outputs=outputs,
        constants={'chunks': chunks, 'dim': -1})
    return outputs, node

def get_nonlin(name: str, b_addr: int, b_act: int, act_dtype: str, lut_dtype: str, q_addr: int, 
    q_lut: int, table, x: TensorProto, output: TensorProto=None, interpolate=False) -> Tuple[TensorProto, List[NodeProto]]:

    if not interpolate:
        return _get_nonlin(name, b_addr, act_dtype, lut_dtype, q_addr, q_lut,
            table, x, output)
    else:
        return _get_nonlin_interpolate(name, b_addr, b_act, act_dtype, lut_dtype,
            q_addr, q_lut, table, x, output)

def _get_nonlin(name: str, b_addr: int, act_dtype: str, lut_dtype: str, q_addr: int, 
    q_lut: int, table, x: TensorProto, output: TensorProto=None) -> Tuple[TensorProto, List[NodeProto]]:

    shape = x.shape
    shamt = x.quanta - q_addr
    
    if output is None:
        lut_output = TensorProto(f'{name}_out', dtype=act_dtype, quanta=q_lut, shape=shape)
    else:
        lut_output = output

    addr = TensorProto(f'{name}_addr', lut_dtype, shape=x.shape, quanta=q_addr)
    addr_shift = NodeProto(f'{name}_addr_shift', registry_v1['shift'],
        inputs={'x': x},
        outputs=[addr],
...
def _get_nonlin_interpolate(name: str, b_addr: int, b_act: int, act_dtype: str, lut_dtype: str, q_addr: int, 
    q_lut: int, table, x: TensorProto, output: TensorProto=None) -> Tuple[TensorProto, List[NodeProto]]:
    ...
    def add_shape_print(x, msg: str):
        nodes.append(NodeProto('print', registry_v1['print'],
            inputs={'x': x},
            outputs=[],
    ...
    nodes += [NodeProto(f'{name}_floor_shift', registry_v1['shift'],
        inputs={'x': x},
        outputs=[floor],
        constants={'shamt': shamt, 'bw': b_addr})]
    
    ceil = TensorProto(f'{name}_ceil', lut_dtype, shape=shape, quanta=q_addr)
    nodes += [NodeProto(f'{name}_ceil_viadd', registry_v1['viadd'],
        inputs={'x': floor},
        outputs=[ceil],
        constants={"y": 1, "shamt_x": 0, "shamt_y": 0, "shamt_bwred": 0, "bw": b_addr, 
            "bw_x": b_addr, "bw_y": b_addr})]
    
    # CONCATENATE THEM (TO AVOID CALLING MULTIPLE LUTS)
    cat_out = TensorProto(f'{name}_addrs', lut_dtype, shape=shape2, quanta=q_lut)
    nodes += [NodeProto(f'{name}_addr_cat', registry_v1['cat'],
    ...
    floor_up = TensorProto(f'{name}_floor_up', dtype=act_dtype, shape=shape, quanta=x.quanta)
    nodes += [NodeProto(f'{name}_floor_upshift', registry_v1['shift'],
        inputs={'x': floor},
        outputs=[floor_up],
    ...
    nodes += [NodeProto(f'{name}_rem_sub', registry_v1['vvsub'],
        inputs={'x': x, 'y': floor_up},
        outputs=[mux],
        constants={"shamt_x": 0, "shamt_y": 0, "shamt_bwred": 0, 
            "bw": b_act, "bw_x": b_act, "bw_y": b_act})]

    # MULTIPLY THE TWO EVALUATIONS BY THE MUX
    mix_floor = TensorProto(f'{name}_mixfloor', dtype=act_dtype, shape=shape, quanta=q_lut)
    nodes += [NodeProto(f'{name}_mixmul_floor', registry_v1['vvmul'],
        inputs={'x': y_floor, 'y': mux},
        outputs=[mix_floor],
        constants={'shamt_bwred': shamt, 'bw': b_act})]
    
    mix_ceil = TensorProto(f'{name}_mixceil', dtype=act_dtype, shape=shape, quanta=q_lut)
    nodes += [NodeProto(f'{name}_mixmul_ceil', registry_v1['vvmul'],
    ...
    nodes += [NodeProto(f'{name}_subout', registry_v1['vvsub'],
    ...
    if output is None:
        output = TensorProto(f'{name}_output', dtype=act_dtype, shape=shape, quanta=q_lut)
    nodes += [NodeProto(f'{name}_add_out', registry_v1['vvadd'],
...
def get_mul(name: str, quanta: int, dtype: str, x: TensorProto, y: TensorProto,
    output: TensorProto=None, debug=False) -> Tuple[TensorProto, NodeProto]:
...
def lstm_kernelization(arith: GraphProto, init: GraphProto, node: NodeProto,
    node_i: int):
    ...
    for idx in range(num_layers):
        ...
        # INIT HIDDEN STATE
        h_0 = TensorProto(f'lstm_h0_l{idx}', act_dtype, [hidden_size],
            quanta=layer_conf['q_unity'])
        c_0 = TensorProto(f'lstm_c0_l{idx}', act_dtype, [hidden_size],
            quanta=layer_conf['q_c'])

        h_initializer = NodeProto(f'h_init_l{idx}', optype=registry_v1['zeros'],
            inputs={}, outputs=[h_0], constants={'shape': [hidden_size]})
        init.add_node(h_initializer)
        c_initializer = NodeProto(f'c_init_l{idx}', optype=registry_v1['zeros'],
            inputs={}, outputs=[c_0], constants={'shape': [hidden_size]})
        ...
        if not REARRANGE_GATES:
            ...
            x_sigmoid = TensorProto(f'x_sigmoid_l{idx}', dtype=act_dtype, 
                shape=[3*hidden_size], quanta=q_matmul)
            x_tanh = TensorProto(f'x_tanh_l{idx}', dtype=act_dtype, 
                shape=[hidden_size], quanta=q_matmul)
            split = NodeProto(f'gate_split_l{idx}', registry_v1['split'],
                inputs={'x': y_mm},
            ...
            (i, f, o), chunk = get_chunk(
                f'chunk_ifo_l{idx}', x=ifo, chunks=3)
            kern_nodes.append(chunk)
        ...
        # c_next = i*g + f*c_prev
        ig, ig_prod = get_mul(f'ig_prod_l{idx}', 
            quanta=layer_conf['q_c'],
            dtype=act_dtype,
            x=i, y=g)
        kern_nodes.append(ig_prod)

        fc, fc_prod = get_mul(f'fc_prod_l{idx}',
            quanta=layer_conf['q_c'],
            dtype=act_dtype,
            x=f, y=c_0)
        ...
        c_next, c_sum = get_simple_vvadd(
            f'c_sum_l{idx}', dtype=act_dtype, length=hidden_size,
            quanta=layer_conf['q_c'],
            x=fc, y=ig, output=z
        )
        ...
        h_next, h_gate = get_mul(
            ...
            quanta=layer_conf['q_unity'],
            dtype=act_dtype,
            x=h_next, y=o, output=z
        )
        kern_nodes.append(h_gate)

        # ASSIGN: h->h_next, c->c_next
        kern_nodes.append(NodeProto(
            f'assign_h_l{idx}', registry_v1['assign'],
            inputs={'y': h_0, 'x': h_next},
            outputs=[], constants={}
        ))
        kern_nodes.append(NodeProto(
            f'assign_c_l{idx}', registry_v1['assign'],
    ...
    kern_nodes.append(NodeProto(
        f'copy_l{idx}', registry_v1['copy'],
        inputs={'x': h_next}, outputs=[h_final]))
