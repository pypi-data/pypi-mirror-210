...
def plot_slice(X, Y, Z, vmin=None, vmax=None, skew_angle=90, ax=None,
              xlim=None, ylim=None,
    ...
    """
    This function plots a 2D heatmap of a given dataset (Z) with the possibility of applying logarithmic or symmetrical logarithmic scales to the colorbar. The plot can be sheared to create a trapezoidal view with a defined skew angle. Tick labels are displayed on all sides of the plot. The function takes the following parameters:
    ...
    `vmin`: The minimum value to plot in the dataset. If not provided, the minimum of the dataset will be used.
    
    `vmax`: The maximum value to plot in the dataset. If not provided, the maximum of the dataset will be used.
    ...
    `xticks`: The major tick interval for the x-axis. If not provided, the function will use a default minor tick interval of 1.
    
    `yticks`: The major tick interval for the y-axis. If not provided, the function will use a default minor tick interval of 1.
    ...
    `title`: A string containing the title for the plot. If not provided, a default title will be used.
    ...
    for i in range(0,len(ax.xaxis.get_ticklines())):
        ...
        if i%2:
            ...
            # This first method shifts the top ticks horizontally to match the skew angle. This does not look good in all cases.
            # line.set_transform(Affine2D().translate((ymax-ymin)*np.sin(skew_angle*np.pi/180),0) + line.get_transform())
            # This second method skews the tick marks in place and can sometimes lead to them being misaligned.
