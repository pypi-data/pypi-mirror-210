...
class Dataset(_TorchDataset):
    ...
    def __init__(self, config: Sequence, features: Sequence, data: Sequence, transform: Optional[Callable] = None) -> None:
        """
        Args:
...
class CacheDataset(Dataset):
    ...
    def __init__(
        ...
        if not isinstance(transform, Compose):
            transform = Compose(transform)
        super().__init__(config=config, features=features, data=data, transform=transform)
