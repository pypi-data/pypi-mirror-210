...
class TrojJobHandler:
    ...
    def list_stress_test_jobs(
        ...
        while wait:
            ...
            for job in jobs:
                if job["state"] in TrojJobStatuses.dead_statuses:
                    stat_res.append(True)
                else:
                    stat_res.append(False)
                if pretty:
                    print_out += "{:<39} {:<12} {:<45} {:<35} {:<20}".format(
        ...
        return self.client.list_stress_test_jobs(
            project_name=project_name,
            dataset_name=dataset_name,
            test_run_name=test_run_name,
            status=status,
...
class TrojJobStatuses:
    ...
    ERROR = "Error"
    COMPLETED = "Completed"
    dead_statuses = [UNKNOWN, FAILED, COMPLETED, ERROR]
