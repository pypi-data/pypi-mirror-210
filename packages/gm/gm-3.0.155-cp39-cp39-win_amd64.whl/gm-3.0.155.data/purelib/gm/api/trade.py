...
from gm.csdk.c_sdk import py_gmi_place_order, py_gmi_get_unfinished_orders, py_gmi_get_orders, \
    ...
    py_gmi_get_algo_orders, py_gmi_get_child_orders, py_gmi_smart_reorder, py_gmi_smart_reorder_cancel, \
    py_gmi_get_entrustable_volume_by_symbol_pb
from gm.enum import OrderQualifier_Unknown, OrderDuration_Unknown, OrderSide_Buy, OrderSide_Sell, OrderType_Limit, OrderType_Market
from gm.model.storage import context
from gm.pb.account_pb2 import Order, OrderBusiness_OPTION_BUY_OPEN, OrderBusiness_OPTION_COVERED_BUY_CLOSE, OrderBusiness_OPTION_COVERED_SELL_OPEN, OrderBusiness_OPTION_EXERCISE, OrderBusiness_OPTION_SELL_OPEN, Orders, OrderStyle_Volume, OrderStyle_Value, OrderStyle_Percent, \
...
from gm.pb.trade_pb2 import GetUnfinishedOrdersReq, GetOrdersReq, CancelAllOrdersReq, CloseAllPositionsReq, \
    GetExecrptsReq
from gm.pb.trade_query_service_pb2 import GetEntrustableVolumeBySymbolReq, GetEntrustableVolumeBySymbolRsp
...
def get_execution_reports():
    ...
    for account in context.accounts.values():
        ...
        if result:
            res = ExecRpts()
            res.ParseFromString(result)
            reports.extend([protobuf_to_dict(
                res_order, including_default_value_fields=True) for res_order in res.data])
    ...
    if algo_name is None or not isinstance(algo_param, dict) or len(algo_param) == 0:
        ...
        try:
            ...
            if lower_algo_name == 'ats-smart':
                ...
                algo_param['start_time'] = '%d' % time.mktime(time.strptime(
                    today + algo_param['start_time'], "%Y-%m-%d %H:%M:%S"))
                algo_param['end_time'] = '%d' % time.mktime(time.strptime(
                    today + algo_param['end_time'], "%Y-%m-%d %H:%M:%S"))
                algo_param['end_time_referred'] = '%d' % time.mktime(time.strptime(
                ...
                # 兼容处理
                start_time = algo_param.get('time_start', None) if algo_param.get(
                    'start_time', None) is None else algo_param.get('start_time', None)
                end_time = algo_param.get('time_end', None) if algo_param.get(
                    'end_time', None) is None else algo_param.get('end_time', None)

                time_start = '%d' % time.mktime(time.strptime(
                    today + start_time, "%Y-%m-%d %H:%M:%S"))
                time_end = '%d' % time.mktime(time.strptime(
                    today + end_time, "%Y-%m-%d %H:%M:%S"))
...
# ============ 算法接口 ==================
def algo_smart_reorder(symbol, price, volume, side, position_effect, repeat_n, max_price_offset, time_out, time_wait, account=""):
    ...
    order_type = OrderType_Limit
    value = py_gmi_smart_reorder(symbol, price, volume, side, order_type,
                                 position_effect, repeat_n, max_price_offset, time_out, time_wait, account)
    if value >= 1000000:
