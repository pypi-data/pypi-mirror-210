...
from gm.constant import DATA_TYPE_TICK, SCHEDULE_INFO, DATA_TYPE_L2TRANSACTION
from gm.csdk.c_sdk import (
    ...
    c_status_fail, py_gmi_set_mfp, py_gmi_user_timer, py_gmi_user_timer_stop,
...
def subscribe(symbols, frequency='1d', count=1, wait_group=False, wait_group_timeout='10s', unsubscribe_previous=False):
    # type: (GmSymbols, Text, int, bool, Text, bool) -> None
    """
...
def current(symbols, fields=''):
    ...
    for tick in ticks.data:
        tick = protobuf_to_dict(tick, including_default_value_fields=False)
