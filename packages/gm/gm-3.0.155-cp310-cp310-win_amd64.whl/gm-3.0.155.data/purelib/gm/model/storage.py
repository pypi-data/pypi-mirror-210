...
class Context(object):
    ...
    def data(symbol, frequency, count=1, fields='', bar_df=True):
        ...
        if frequency == DATA_TYPE_TICK:
            return Context._get_tick_data(symbol, frequency, count, fields)
        else:
            return Context._get_bar_data(symbol, frequency, count, fields)

    @staticmethod
    def _get_tick_data(symbol, frequency, count, fields):
        ...
        if count > len(symbol_tick_queue):
            ...
            comb_tick_datas = list(symbol_tick_queue) + tick_datas_trans
            # 排除掉重复的数据
            tick_key_set = set()
            new_tick_datas = list()
        ...
        # 按要求取数据
        d = [v.data for idx, v in enumerate(symbol_tick_queue) if idx < count]
        d.reverse()
        ...
        for bar_data_item in d:
            ...
            for f in filter_fields:
                item[f] = bar_data_item.get(f)
            dd.append(item)
        return pd.DataFrame(dd)

    @staticmethod
    def _get_bar_data(symbol, frequency, count, fields):
        ...
        if count > len(symbol_bar_queue):
            ...
            try:
                # 回测模式 15:15:01 后才会更新日线bar, 如果时间在这之前通过 history_n 补的数据应该往前一天
                if frequency == "1d" and query_data_end_time.time() < datetime.time(15, 15, 1):
                    delta_count += 1
                    remove_tail_bar = True
                else:
                    remove_tail_bar = False

                bar_datas = history_n(symbol=symbol, frequency=frequency, count=delta_count,
                                  end_time=query_data_end_time, adjust=context.adjust_mode,
                                  adjust_end_time=adjust_end_time)
                if remove_tail_bar:
                    bar_datas = bar_datas[:-1]
            except:
                bar_datas = []
        ...
        # 按要求取数据
        d = [v for idx, v in enumerate(symbol_bar_queue) if idx < count]
        d.reverse()
        if len(fields) == 0:
            td = [{
                'symbol': item.symbol,
