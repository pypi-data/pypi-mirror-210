...
from gm.constant import CALLBACK_TYPE_TICK, CALLBACK_TYPE_BAR, \
    ...
    CALLBACK_TYPE_INIT, CALLBACK_TYPE_L2TRANSACTION, CALLBACK_TYPE_ALGOORDER, CALLBACK_TYPE_L2ORDER, \
    CALLBACK_TYPE_L2ORDER_QUEUE, CALLBACK_TYPE_USER_TIMER
from gm.csdk.c_sdk import BarLikeDict2, TickLikeDict2, gmi_now_plus, py_gmi_use_dsproxy
...
from gm.model import DictLikeExecRpt, DictLikeOrder, DictLikeIndicator, DictLikeParameter, DictLikeAccountStatus, \
    DictLikeConnectionStatus, DictLikeError, DictLikeL2Transaction, DictLikeAlgoOrder, DictLikeL2Order, \
...
from gm.pb.data_pb2 import Tick, Bar, L2Transaction, L2Order, L2OrderQueue
...
from gm.utils import gmsdklogger, beijing_tzinfo, protobuf_timestamp2bj_datetime, round_float
...
def tick_callback_new(data):
    ...
    for q in tick.quotes:  # type: Quote
        quotes.append({
            ...
            'ask_v': q.ask_v,
            'bid_q': {'total_orders': q.bid_q.total_orders, 'queue_volumes': q.bid_q.queue_volumes},
            'ask_q': {'total_orders': q.ask_q.total_orders, 'queue_volumes': q.ask_q.queue_volumes},
        })

    if len(quotes) < 10:
        for _ in range(len(quotes), 10):
            zero_val = {
                'bid_p': 0, 'bid_v': 0, 'ask_p': 0, 'ask_v': 0,
...
def tick_callback(data):
    ...
    if symbol not in context.tick_data_cache:
        ...
        context.tick_data_cache[symbol] = collections.deque(
            maxlen=context.max_tick_data_count)
...
bars_in_waitgroup_backtest = collections.defaultdict(
    list)  # type: Dict[Text, List[BarLikeDict2]]
...
def bar_callback(data):
    ...
    if not barwaitgroupinfo.is_symbol_in(symbol):
        gmsdklogger.debug(
            'wait_group = True, 但是股票不在waitgroup的列表里时, 直接发了, symbol=%s, frequency=%s', symbol, frequency)
        context.on_bar_fun(context, [bar, ])
    ...
    if context.mode == MODE_BACKTEST:  # 处理回测模式下, wait_group = True
        ...
        if pre_eob_in_backtest is None:
            context._temporary_now = datetime.datetime.fromtimestamp(
                gmi_now_plus()).replace(tzinfo=beijing_tzinfo)
    ...
    if context.mode == MODE_LIVE:  # 处理实时模式下, wait_group = True
        ...
        if len(barwaitgroupinfo) == len(eob_bar_dict[eob]):
            gmsdklogger.debug(
                "实时模式下, wait_group的bar都到齐了, 触发on_bar. eob=%s", eob)
            context.bar_data_cache[k].appendleft(bar)
...
def l2order_queue_callback(data):
    if context.on_l2order_queue_fun is not None:
        ...
        l2order_queue = protobuf_to_dict(
            l2order_queue, including_default_value_fields=True, dcls=DictLikeL2OrderQueue)
        ...
        for frequency, eob_tick_dict in six.iteritems(bars_in_waitgroup_live):
            ...
            if barwaitgroupinfo is not None:
                ...
                for eob_time in list(six.iterkeys(eob_tick_dict)):
                    ...
                    delta_second = cur_now_s - \
                        first_bar['receive_local_time']  # type: float
