...
def xla_call_module(args, version, module, Sout, Tout, dim_args_spec=[], platforms=[], function_list=[], name=None):
  ...
  Args:
    ...
    platforms: An optional list of `strings`. Defaults to `[]`.
      the list of platforms supported by `module`. If the list is empty,
  ...
  if tld.is_eager:
    ...
    try:
      ...
      return xla_call_module_eager_fallback(
          ...
          function_list=function_list, name=name, ctx=_ctx)
    except _core._SymbolicException:
      pass  # Add nodes to the TensorFlow graph.
    except (TypeError, ValueError):
      _result = _dispatch.dispatch(
            xla_call_module, (), dict(args=args, version=version,
                                      ...
                                      platforms=platforms,
                                      function_list=function_list, name=name)
          )
      if _result is not _dispatch.OpDispatcher.NOT_SUPPORTED:
        return _result
      raise
...
def xla_call_module_eager_fallback(args, version, module, Sout, Tout, dim_args_spec, platforms, function_list, name, ctx):
  version = _execute.make_int(version, "version")
  module = _execute.make_str(module, "module")
  if not isinstance(Sout, (list, tuple)):
    raise TypeError(
