...
class Dbx(DB):
  ...
  def update_dbx(self, dict_methods):
    # set non-private method overrides
    _dict_methods = dict_methods if not hasattr(dict_methods,'__dict__') else dict_methods.__dict__
    self.__dict__.update( { k:v for k,v in _dict_methods.items() if not k.startswith('_') } )
