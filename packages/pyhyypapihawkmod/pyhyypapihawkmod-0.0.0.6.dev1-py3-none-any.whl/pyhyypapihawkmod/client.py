...
class HyypClient:
    ...
    def debug_dump(self):
        ...
        HyypAlarmInfos(self).status()

        filepath = os.path.expanduser("~/Desktop/idslog.log")
        f = open(filepath,'w')
        sync_info = self.get_sync_info()
        state_info = self.get_state_info()
        f.write('####SYNC INFO####')
        f.write('\r\n')
        f.write(json.dumps(sync_info))
        f.write('\r\n')
        f.write('####///SYNC INFO####')     
        f.write('\r\n')
        f.write('####STATE INFO####')
        f.write('\r\n')
        f.write(json.dumps(state_info))
        f.write('\r\n')
        f.write('####///STATE INFO####')
        f.write('\r\n')
        f.write('####///LOG END####')
        f.close()
        print('\r\n\r\n\r\nDebug dump complete. \r\nCheck your desktop for idslog.log')
