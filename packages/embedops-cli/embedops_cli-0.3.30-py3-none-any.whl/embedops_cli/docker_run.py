...
def docker_run(run_context: LocalRunContext):
    ...
    run_context.variables.update({"EMBEDOPS_VARS": json.dumps(run_context.variables)})
    ...
    if embedops_ssh_dir:
        ...
        dockercmd.extend([f"-v {ssh_dir}:/tmp/.ssh"])
    else:
        # bind-mount host user's ~/.ssh directory
        dockercmd.extend(
            [f"-v {os.path.expanduser('~')}{os.path.sep}.ssh:/home/ci/.ssh"]
        )
