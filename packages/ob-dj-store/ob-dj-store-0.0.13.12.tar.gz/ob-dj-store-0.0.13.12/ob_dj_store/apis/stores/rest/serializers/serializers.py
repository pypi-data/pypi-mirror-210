    ...
    def validate(self, attrs):
        ...
        if "pickup_time" in attrs:
            if attrs["pickup_time"]:
                ...
                if attrs["pickup_time"] < localtime(now()):
                    raise serializers.ValidationError(
...
class SubCategorySerializer(serializers.ModelSerializer):
    ...
    def get_is_available(self, obj) -> bool:
        ...
        if store_id:
            try:
                parent_availability_hours = obj.parent.availability_hours.get(
...
class CategorySerializer(serializers.ModelSerializer):
    ...
    def get_is_available(self, obj) -> bool:
        ...
        if store_id:
            try:
                current_availability_hours = obj.availability_hours.get(
                    weekday=localtime(now()).weekday() + 1,
                    store=store_id,
                )
                return (
