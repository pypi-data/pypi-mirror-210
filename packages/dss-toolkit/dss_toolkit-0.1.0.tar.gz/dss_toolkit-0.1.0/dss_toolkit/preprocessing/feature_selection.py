...
def appy_boruta(df, numeric_features, categorical_features, target):
    ...
    X = _encode_X(df, numeric_features, categorical_features)
    y = df[target]
...
def _encode_X(df, numeric_features, categorical_features):
    X_num = df[numeric_features]
    X_cat_demogs = pd.get_dummies(df[categorical_features], drop_first=True)
    X_cat_product = pd.get_dummies(df[categorical_features], drop_first=False)
