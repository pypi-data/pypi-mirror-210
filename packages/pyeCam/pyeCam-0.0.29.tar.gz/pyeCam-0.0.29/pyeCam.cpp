...
PYBIND11_MODULE(pyeCam, m) {
    py::class_<DeviceManager>(m, "DeviceManager")
        .def(py::init<>())
        .def("get_device_count", &DeviceManager::getDeviceCount, py::arg("deviceCount"), "Get the device count")
        .def("get_device_info", &DeviceManager::getDeviceInfoWrapper, py::arg("deviceIndex"), "Get device information");
}
