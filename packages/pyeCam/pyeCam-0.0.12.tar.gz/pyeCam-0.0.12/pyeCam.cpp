...
typedef struct
{
	...
	{
		printf("isValidIndex - is inside a method /n");
		
		 py::object gDeviceCount;  // Assuming you have defined gDeviceCount somewhere

    
			
			uint32_t device_count;
			printf("isValidIndex - is inside a method - Before if method/n");
		
			if (getDeviceCount(gDeviceCount) == false)
			{ 
...
device_count = gDeviceCount.attr("value").cast<int>();;
...
device_count = gDeviceCount.attr("value").cast<int>();;
	...
	bool getDeviceInfo(uint32_t deviceIndex,DeviceInfo* gDevice) 
	{
	  ...
	  try
	  {
		  ...
		  {
			  printf("Get_deviceinfo_InvalidDeviceIndex \n ");
			  return false;
