...
// Print the error description if the operation failed
if (FAILED(hr))
{
...
py::tuple getDeviceInfoWrapper(uint32_t deviceIndex) {
    ...
    py::tuple result;
    if (success) {
        result = py::make_tuple(success, gDevice.deviceName, gDevice.vid, gDevice.pid, gDevice.devicePath, gDevice.serialNo);
    } else {
