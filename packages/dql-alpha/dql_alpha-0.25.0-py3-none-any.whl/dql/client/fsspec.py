...
from typing import TYPE_CHECKING, Any, ClassVar, Iterator, List, Tuple, Type
...
class FSSpecClient(Client):
    ...
    async def fetch(self, listing, start_prefix="", partial_id=0):
        ...
        async def worker(queue, data_storage):
            worker_ignore_count = 0
            while True:
                dir_id, prefix = await queue.get()
                try:
                    if dir_id == -1:
                        # Done with tasks
                        return worker_ignore_count
                    ...
                    worker_ignore_count += ignore_count
        ...
        try:
            ...
            await queue.join()
            for _ in range(FETCH_WORKERS):
                # Send shutdown task to workers
                queue.put_nowait((-1, ""))
            # Wait for all shutdown tasks to complete
            await queue.join()
            # Gather results
            results = await asyncio.gather(*workers)
            total_ignore_count += sum(results)
