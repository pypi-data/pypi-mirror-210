...
class DistributedHelper:
    ...
    def allreduce_sum(cls, arg: Union[float, np.array]) -> Union[float, np.array]:
        ...
        :param arg: Value to be aggregated, can be a single number or a numpy list.

        :return: If a single number is passed as arg, a single number will be returned.
        Else, a list will be returned. The value will be the variable aggregated across
        all of the machines. If the value is a list, all values in dimension 0 are added.
        ...
        if torch.numel(result_tensor) == 1:
