...
class ExploreManager:
    ...
    def plot_correlation(self,method='pearson',figsize=(1000,1000),colorscale='agsunset',title=None):
        """
        ...
        fig = ff.create_annotated_heatmap(z=self.df.corr().values, x=self.df.corr().columns.tolist(), y=self.df.corr().columns.tolist(), annotation_text=self.df.corr().round(2).values, showscale=True)
