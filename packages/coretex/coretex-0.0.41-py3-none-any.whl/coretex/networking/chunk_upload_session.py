...
class ChunkUploadSession:
    ...
    def __start(self) -> str:
        ...
        response = NetworkManager.instance().genericJSONRequest("upload/start", RequestType.post, parameters)
    ...
    def __uploadChunk(self, uploadId: str, start: int, end: int) -> None:
        ...
        response = NetworkManager.instance().genericUpload("upload/chunk", files, parameters)
        if response.hasFailed():
