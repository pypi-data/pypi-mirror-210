        ...
        if placeholders is not None:
            ...
            for placeholder in placeholders:
                ...
                if placeholder_type == 'pattern' or placeholder_type == 'line_pattern' or placeholder_type == 'word_pattern':
                    ...
                    if self.mu_pdf_doc is None or self.force_ocr is True:
                            ...
                            for j in range(n_boxes):
                                if 'conf' not in keys or int(float(d['conf'][j])) > self.PYTESSERACT_CONFIDENCE:
                                    if re.match(pattern, d['text'][j]):
                                        ...
                                        if self.re_rendered_for_ocr:
                                            pixel_recalculation_factor = self.DPI / self.MINIMUM_OCR_RESOLUTION
                                            (x, y, w, h) = (
                                                int(pixel_recalculation_factor * x),
                                                int(pixel_recalculation_factor * y),
                                                int(pixel_recalculation_factor * w),
                                                int(pixel_recalculation_factor * h))
                                        text_pattern_mask = {"page": i + 1, "x": x - xoffset, "y": y - yoffset,
                            ...
                            if placeholder_type == 'word_pattern':
                                ...
                                for word in words:
                                    if search_pattern.match(word[4]):
                                        (x, y, w, h) = (word[0] * self.DPI / 72, word[1] * self.DPI / 72,
                                                        (word[2] - word[0]) * self.DPI / 72,
                                                        (word[3] - word[1]) * self.DPI / 72)
                                        text_pattern_mask = {"page": i + 1, "x": x - xoffset, "y": y - yoffset,
                                                             "height": h + 2 * yoffset, "width": w + 2 * xoffset}
                                        self.placeholders.append(text_pattern_mask)
                                ...
                                for block in tdict['blocks']:
                                    if block['type'] == 0:
                                        for line in block['lines']:
                                            if len(line['spans']) != 0 and search_pattern.match(
                                                    line['spans'][0]['text']):
                                                (x, y, w, h) = (
                                                    line['bbox'][0] * self.DPI / 72, line['bbox'][1] * self.DPI / 72,
                                                    (line['bbox'][2] - line['bbox'][0]) * self.DPI / 72,
                                                    (line['bbox'][3] - line['bbox'][1]) * self.DPI / 72)
                                                text_pattern_mask = {"page": i + 1, "x": x - xoffset, "y": y - yoffset,
                                                                     "height": h + 2 * yoffset,
                                                                     "width": w + 2 * xoffset}
                                                self.placeholders.append(text_pattern_mask)
                    ...
                    x, y, h, w = 0, 0, 0, 0
                    if unit == 'px':
                        x, y, h, w = (int(placeholder['x']), int(placeholder['y']), int(placeholder['height']),
                                      int(placeholder['width']))
                    elif unit == 'mm':
                        constant = self.DPI / 25.4
                        x, y, h, w = (int(float(placeholder['x']) * constant), int(float(placeholder['y']) * constant),
                                      int(float(placeholder['height']) * constant),
                                      int(float(placeholder['width']) * constant))
                    elif unit == 'cm':
                        constant = self.DPI / 2.54
                        x, y, h, w = (
                            int(float(placeholder['x']) * constant), int(float(placeholder['y']) * constant),
                            int(float(placeholder['height']) * constant), int(float(placeholder['width']) * constant))
                    ...
                    if location == 'top':
                        ...
                        height = int(image_height * percent / 100)
                        width = image_width
                        placeholder_coordinates = {"page": page, "x": 0, "y": image_height - height, "height": height,
                                                   "width": width}
                    elif location == 'left':
                        height = image_height
                        ...
                        height = image_height
                        width = int(image_width * percent / 100)
                        placeholder_coordinates = {"page": page, "x": image_width - width, "y": 0, "height": height,
                                                   "width": width}
                    self.placeholders.append(placeholder_coordinates)
        ...
        for placeholder in placeholders:
            if placeholder['page'] == 'all':
                for i in range(len(images_with_placeholders)):
                    ...
                    end_point = (
                        start_point[0] + placeholder['width'] + 10, start_point[1] + placeholder['height'] + 10)
                    try:
                        images_with_placeholders[i] = cv2.rectangle(images_with_placeholders[i], start_point, end_point,
                                                                    (255, 0, 0), -1)
                ...
                end_point = (
                    int(start_point[0] + placeholder['width'] + 10), int(start_point[1] + placeholder['height'] + 10))
                try:
                    images_with_placeholders[pagenumber] = cv2.rectangle(images_with_placeholders[pagenumber],
                                                                         start_point, end_point, (255, 0, 0), -1)
                except IndexError as err:
        ...
        if self.extension == '.pdf':
            ...
            if img is None:
                raise AssertionError(
                    "No OpenCV Image could be created for file {} . Maybe the file is corrupt?".format(self.image))
