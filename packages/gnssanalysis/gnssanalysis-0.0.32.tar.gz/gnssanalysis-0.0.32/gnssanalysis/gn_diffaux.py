...
def difftrace(trace1_path, trace2_path, tol, std_coeff, log_lvl, plot):
    ...
    if (states1 is None) and (states2 is None):
        ...
        _gn_aux.throw_if_nans(states1, blocks_to_ignore = ['STATES/SPP'], columns_to_ignore = ['ADJ'])
        _gn_aux.throw_if_nans(states2, blocks_to_ignore = ['STATES/SPP'], columns_to_ignore = ['ADJ'])
        ...
        sats_with_nans = sats.notna() & diffstd_states.DIFF.isna().values # if there are SVs with nans at this stage, this means that files' indices are different 
