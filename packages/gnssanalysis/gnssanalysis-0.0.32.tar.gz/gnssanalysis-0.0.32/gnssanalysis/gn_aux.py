...
def throw_if_nans(df: _pd.DataFrame, blocks_to_ignore: list = [], columns_to_ignore: list = []):
    """Ignoring NaNs in STATES/SPP adjustments could be done with blocks_to_ignore = ['STATES/SPP'] and columns_to_ignore = ['ADJ']"""
    nans_mask = df.isnull()
    BLK_idx = df.index.get_level_values("BLK")
    nans_mask.loc[BLK_idx.isin(blocks_to_ignore), columns_to_ignore] = False

    nans_mask_1d = nans_mask.any(1)
    if nans_mask_1d.any():
        raise ValueError(f"Found nan values at:\n{df[nans_mask_1d.values]}")