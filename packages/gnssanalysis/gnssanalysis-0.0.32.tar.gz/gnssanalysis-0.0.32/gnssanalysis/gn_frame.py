...
def get_frame_of_day(
    ...
    discon_valid = discon_df[
        (discon_df.MODEL == "P")
        & (discon_df.START <= date_J2000)
    ...
    soln_series = soln_series[soln_series.values.astype(bool)] # remove all the stations that do not have a valid solution
    ...
    if list_path_or_df is not None:
        ...
        out_mask = out.CODE_PT.str[:4].isin(core_list)

        if out_mask.values.any():
            out = out[out_mask]
        else:
            logging.error(msg="list stations not present in frame file")
            return None

    out_xyzvel = _pd.Series(data = out.VAL.EST.values,index = _pd.MultiIndex.from_arrays([out.CODE_PT.values, out.TYPE])).unstack(1)

    itrf_reference = out["REF_EPOCH"].values[0] # all values in the frame are relative to frame origin reference time (2010-01-02T00:00:00)
    ...
    if psd_path_or_df is not None:
        if isinstance(psd_path_or_df, _pd.DataFrame):
            ...
            logging.info(msg="reading postseismic from {psd_path_or_df}")
...
def read_frame_snx_all(*file_paths, core_sites=None):
    ...
    for path in file_paths:
        buf.append(_gn_io.sinex._get_snx_vector_gzchunks(filename=path, block_name="SOLUTION/ESTIMATE"),snx_format='raw')
