...
@click.pass_context
def run(ctx, report, open_report, publish, docx, pytest_args):
    ...
    if report:
        if publish:
            ...
            index_fpath = _make(abs_links=True)
    teardown(rootdir)
...
def _make(abs_links=False):
    junit_file, junit_html_target = get_junit_files()
    index, html = xml2html(junit_file, make_matrix=True, abs_links=abs_links)
