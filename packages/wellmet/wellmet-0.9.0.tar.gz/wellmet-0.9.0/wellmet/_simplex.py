...
#č priorita jádra
#č Pokud sousedy sousedů do hloubky n se dá odseparovat - 
#č má se zvolit hyperrovina, které je separuje.
#č Ona má být upřednestěna i hyperrovině, 
#č která by byť pokryvala větší množství simplexů,
#č ale neměla by stejnou hloubku jádra
#
#č Garance:
#č 1. Pokud okoli sousedů sousedů do nějaké hloubky n
#č    je separabilní, tak vrácené vektory budou je separovat
#č 2. Vracené vektory jsou takzvaně finalized, tj. 
#č    jíž nelze ke jíž pokrytym daným vektorem simplexům 
#č    přídat žádného dalšího souseda.
#č    Nelze najít takovou hyperrovinu, která by separovala tyto
#č    simlexy a nějaké další sousedicí.
#č    Žádná množina separovaných simplexu jednoho vektoru 
#č    nemůže být podmnožinou nějakého jiného vektoru.

#č Negarance:
#č Algoritmus může vrátit několik ekvivalentních vektorů
#č se stejnou množinou separovaných simplexů.
...
class _Sense:
    ...
    def _init(self):
        ...
        self.separation_normal = SeparationAxis(points, failsi, simplices)
        self._local_scope = dict()
        self._to_parse = set()
        self._parsed = set()
        self._point_scope = self.separation_normal.point_scope = set()
        self.simplex_scope.add(simplex_id)
        self.point_scope
    ...
    def perform_sensitivity_analysis(self):
        ...
        for simplex_id, is_mixed in zip(range(len(mixed_mask)), mixed_mask):
            ...
            if simplex_id in vectors:
                normal = vectors[simplex_id]
            else:
                vector = sx.get_simplex_normal(indices)[:-1]
                normal = self.separation_normal
                normal.set_normal(vector)
                
            id_before = id(normal)
            scope, normal = self.process_simplex(simplex_id, depth, normal)
            id_after = id(normal)
                
            assert normal.finalized
            ...
            if (id_before != id_after) or (new_depth > depth):
                for key, key_depth in scope.items():
            ...
            global_gradient += normal.normal * p_mixed
            sensitivities += np.square(normal.normal) * p_mixed    
...
#    @staticmethod
#   def get_supports(point_set, failsi, scalars):
#       max_red = -np.inf
#       min_green = np.inf
...
#               green_supp = point
#        
#        return max_red, min_green, red_supp, green_supp
#        
#               
#        
...
#        return max_red < min_green
    ...
    def process_simplex(self, simplex_id, depth, normal):
        # self for this class, sensitivity-related things
        # sx for general Triangulation class related attributes
        sx = self
        ...
        for __i in range(depth):
            ...
            for id in parsed: # meant to be parsed in the cycle's end
                ...
                #point_scope.update(simplices[id])
                local_scope[id] = 0
        ...
        while len(to_parse): #č máme co dělat?
            ...
            for id in parsed: # meant to be parsed in the cycle's end
                local_scope[id] = -1
                non_separable_point = normal.add_neighbor_simplex(id)
                if non_separable_point is not None:
                    point_scope = set() #č předáme ho do vlastníctví nové normály
                    for key in local_scope.keys():
                        point_scope.update(simplices[key])
                    result = self.get_separation_axis(list(point_scope))
                    if not result.success: #č konec, zde končíme
                        ...
                        local_scope[id] = -2 #č přepíšeme, aby bylo možné je rozlišit
                        ...
                        for key in parsed:
                            if key in local_scope:
                                ...
                                #č byly tam -1 a jedna -2
                                local_scope[key] += 1
                        point_scope.remove(non_separable_point)
                        return self._continue_search(normal, point_scope, non_separable_point)
                    
                    #č hned odřízneme poslední složku - 
                    #č bude tam posunutí b aka offcet aka bias
                    vector = result.x[:-1]
                    
                    #č pokračujeme, nic se neděje
                    normal = SeparationAxis(vector, simplices, 
                                        set(local_scope.keys()), point_scope)
    ...
    def _continue_search(self, normal, point_scope, non_separable_point):
        # self for this class, sensitivity-related things
        # sx for general Triangulation class related attributes
        sx = self
        ...
        for key in parsed:
            ...
            to_parse.update(neighbors_masked[id])
            local_scope[id] = -1
            if not normal.add_simplex_by_id(id):
                point_scope = set() #č předáme ho do vlastníctví nové normály
                for key in local_scope.keys():
                    point_scope.update(simplices[key])
                result = self.get_separation_axis(list(point_scope))
                if not result.success: #č konec, zde končíme
                    local_scope[id] = -3 #č přepíšeme, aby bylo možné je rozlišit
                    if result.status != 2: # Problem appears to be infeasible.
                        print("Sense: linprog ended with status %s" % result.status)
                    return self._continue_search(local_scope, normal)
        ...
        #č vrcholiky bych tady chtěl
        #č Když už jsme tu, 
        #č tak asi přece na neseparability naražet budeme
        point_scope = self._point_scope
        point_scope.clear()
...
##č Dá se něco separovat v určitém místě hozenou hyperrovinou?    
##č Ono to by se mělo být vnímano nikoliv jen jako vector,
##č ale jako normála s nějakým zádaným posunutím b
##č (i když to posunutí b neobjevuje se explicitně)
#class SeparationAxis:
#    #č uživatel třídy musí nastavit ED a failsi.
#    #č Kdyby zůstaly instance z ruzných generací,
#    #č ono by to nemělo dopadnout natolik špatně,
#    #č neboť starší hodnoty ED a failsi by zůstaly v platnosti.
#    
#    # User must to set up ED and failsi!
#    #ED
#    #failsi
#
#    __slots__ = ('normal', 
#                'nsim', 'simplices',
#                'simplex_scope', 'point_scope',
#                'max_red', 'min_green', 'red_supp', 'green_supp',
#                'finite')
#    
#    def __init__(self, normal, simplices, separable_simplex_set, point_set):
...
#        
#        #č ono to musí být množinami, 
#        #č jenže je vytvořit necháme uživateli třídy
#        #č Stejně jako uživatel třídy zodpovidá 
#        #č za jejich správnost a konzistentnost
#        self.simplex_scope = separable_simplex_set
#        self.point_scope = point_set
#        
#        
#        max_red = -np.inf
#        min_green = np.inf
...
#                break
#            
#        #č takže i simplex je separabilní
#        self.simplex_scope.add(simplex_id)
#        return None
#        
#    
#    def __repr__(self):
#        return "SeparationAxis(%s)" % repr(self.normal)
#        
#    def __str__(self):
...
#č Tohle celé je extrémně vázano na _Sense.process_simplex()
#č Někdy chce aby ono imitovalo vektor
#č Jinak simplex a point scope jsou vždy shodujou 
#č a jsou provazané s process_simplex
class SeparationAxis:
    finalized = False
    
    def __init__(self, ED, failsi, simplices):
        self.ED = ED
        self.failsi = failsi
        self.simplices = simplices
        
    def set_normal(self, normal, separable_simplex_set, point_set):
        self.normal = normal
        self._setup_supports()
        
    def set_normal_from_simplex(self, normal, simplex_id):
        self.normal = normal
        self.simplex_scope.clear()
        self.simplex_scope.add(simplex_id)
        self.point_scope.clear()
        self.point_scope.update(self.simplices[simplex_id])
        
        self._setup_supports()
        
        
    def _setup_supports(self):
    ...
    #č zkusme takhle. V případě, že tamten nový vzorek jde odseparovat,
    #č tak vratíme None, jinak číslo toho vzorku pro další kód
    def is_neighbor_separable(self, simplex_id):
        if simplex_id in self.simplex_scope:
            return True
        ...
        for point in self.simplices[simplex_id]:
            if point not in self.point_scope:
                ...
                if self.failsi[point]:
                    if value > self.min_green:
                        self.non_separable_point = point
                        return False
                    elif value > self.max_red:
                        self.max_red = value
                        self.red_supp = point
    ...
    def get_finalized(self):
        return FinalizedAxis(self.normal, frozenset(self.simplex_scope))
        


class FinalizedAxis:
    finalized = True
    __slots__ = ('normal', 'separated_simplices', 'sensitivities')
    
    def __init__(self, normal, separated_simplices):
        length = np.sqrt(np.inner(normal, normal))
        # scale to unit length
        np.divide(normal, length, out=normal) 
        self.normal = normal
        
        self.sensitivities = np.square(normal)
        
        self.separated_simplices = separated_simplices
        
        
        
    def is_neighbor_separable(self, simplex_id):
        return simplex_id in self.separated_simplices
            
    
    def __repr__(self):
        return "FinalizedAxis(%s, %s)" % \
                (repr(self.normal), repr(self.separated_simplices))
...
class _Sense:
    ...
    def perform_sensitivity_analysis(self):
        ...
        for simplex_id, is_mixed in zip(range(len(mixed_mask)), mixed_mask):
            ...
            # let's keep a place for a piece of black magic
            # we'll keep previous vector value
            # no one aware about
            #else:
            ...
            scope, normal = self.process_simplex(simplex_id, depth, normal, vector)
            id_after = id(normal)
    ...
    def process_simplex(self, simplex_id, depth, finalized_normal, normal):
        # self for this class, sensitivity-related things
        # sx for general Triangulation class related attributes
        sx = self
        ...
        factory = self.normal_factory
        factory.reset(finalized_nornal, normal)
        
        
        for __i in range(depth):
            factory.add_ensured_layer(to_add, to_parse)
                
            to_parse, to_add = to_add, to_parse
            to_parse.clear()
...
class SeparationAxis:
    ...
    def __init__(self, sx, simplex_scope):
        self.sx = sx
        self.ED = sx.tri.points
        self.failsi = sx.sample_box.failsi
        self.simplices = sx.tri.simplices
        
        #č simplex_scope potřebujeme pouze v jednom místě
        self.simplex_scope = simplex_scope
    ...
    def reset(self, simplex_id, finalized_nornal):
        ...
        if finalized_nornal is None:
            ...
            # in case of separation axis
            # i.e. we should ignore it anyway
            self.normal = self.sx.get_simplex_normal(indices)[:-1]
    ...
    def add_ensured_layer(self, to_add, to_parse):
        #č pokud je hloubka > 0, tak je jásně, 
        #č že se může jednat pouze o finalized vektor
        assert to_add <= self.finalized_nornal.separated_simplices
    ...
    def set_normal_from_LP(self, normal,)
        self.normal = normal
        self.simplex_scope.clear()
        self.simplex_scope.add(simplex_id)
    ...
    def _is_non_finalized_separable(self, simplex_id):
        ...
        # sucсess!               
        #č musíme se přípravit k dalšímu životu
        assert self.finalized_nornal is None
...
#       return self.finalized_nornal is not None
#        
#    def get_finalized(self):
#       return FinalizedAxis(self.normal, frozenset(self.simplex_scope))
...
#        return max_red < min_green
    ...
    def process_simplex(self, simplex_id, depth, normal):
        ...
        for __i in range(depth):
            add_ensured_layer(self, to_add, to_parse)
            for key in local_scope.keys():
                local_scope[key] += 1
