...
### Metrics ###
def generateMetrics(env: Env, selected_nodes: Dict[str, NodeMetadata]):
    for node_name, node_metadata in selected_nodes.items():
        print("Generating metrics for model: " + node_name)
        query = input("Write a base requirement for the metric: ")

        # Get the path to the .yml file
        yaml_file_path = os.path.join(env.base_path, os.path.dirname(node_metadata.original_file_path), "tql_generated_metric_" + node_metadata.name + '.yml')
...
def promptMetrics(node: NodeMetadata, column_lis: List[str]):
    prompt = """You are an expert SQL analyst with a large knowledge of the DBT platform that takes natural language input and outputs DBT metrics in YAML format.
