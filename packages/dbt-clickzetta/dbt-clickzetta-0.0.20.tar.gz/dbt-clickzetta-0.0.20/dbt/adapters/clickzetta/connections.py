...
class ClickZettaConnectionManager(SQLConnectionManager):
    ...
    def execute(
        ...
        logger.info(f"auto_begin: {auto_begin}")
        logger.info(f"schema: {cursor.description}")
        logger.info(f"sql: {sql}")
        logger.info(f"data: {cursor._query_data.data}")
        logger.info(f"ssss: {len(cursor._query_result.schema)}")
