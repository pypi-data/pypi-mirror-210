...
def _collect_data(
    ...
    if excluded and all(isinstance(value, str) for value in excluded):
    ...
    for source in (exchanges or EXCHANGE_MAP.keys()):
        if source not in EXCHANGE_MAP.keys():
            continue
        # end if

        exchange = EXCHANGE_MAP[source]
    ...
    for source, exchange in markets.items():
        threading.Thread(
            ...
            kwargs=dict(
                ...
                data=data, quotes=quotes, excluded=excluded[source]
            )
        ).start()
    # end for
