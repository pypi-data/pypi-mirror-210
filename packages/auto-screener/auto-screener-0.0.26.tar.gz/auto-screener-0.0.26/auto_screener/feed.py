...
from cryptofeed.feed import Feed, LOG
from cryptofeed.types import OrderBook
from cryptofeed.exchanges import EXCHANGE_MAP
from cryptofeed.defines import L2_BOOK
from cryptofeed.connection_handler import ConnectionHandler as ConnectionHandlerBase
...
class ConnectionHandler(ConnectionHandlerBase):
    """A class to represent the connection handler for the feed exchange objects."""

    feed: Feed

    async def _create_connection(self) -> None:
        """Creates the connection for the feed object."""
        ...
        except (ConnectionClosedError, ConnectionAbortedError) as e:
            LOG.warn(str(e))
            LOG.warn(f"Stopping feed: {self.feed.id}.")

            self.feed.stop()

            if self.feed in self.feed.handler.feeds:
                self.feed.handler.feeds.remove(self.feed)
            # end if

            LOG.warn(f"Feed: {self.feed.id} has stopped.")
        # end try
    # end _create_connection
        ...
        for callbacks in self.callbacks.values():
            for callback in callbacks:
                if hasattr(callback, 'start'):
                    ...
                    LOG.info(
                        f'{self.id}: starting backend task '
                        f'{cb_name} with multiprocessing='
                        f'{self.config.backend_multiprocessing}',
                    )

                    callback.start(
                        loop, multiprocess=self.config.backend_multiprocessing
                    )
                # end if
            # end for
        # end for
