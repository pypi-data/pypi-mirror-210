def adwite_icon(master=None):
    from tempfile import mkstemp

    _adwite_light = b""
    _adwite_dark = b""

    _adwite_logo_light = b""
    _adwite_logo_dark = b""

    _, _adwite_light_temp = mkstemp()
    _, _adwite_dark_temp = mkstemp()

    _, _adwite_logo_light_temp = mkstemp()
    _, _adwite_logo_dark_temp = mkstemp()

    with open(_adwite_light_temp, "wb") as _adwite_light_file:
        from base64 import b64decode

        _adwite_light_file.write(b64decode(_adwite_light))

    with open(_adwite_dark_temp, "wb") as _adwite_dark_file:
        from base64 import b64decode

        _adwite_dark_file.write(b64decode(_adwite_dark))

    with open(_adwite_logo_light_temp, "wb") as _adwite_logo_light_file:
        from base64 import b64decode

        _adwite_logo_light_file.write(b64decode(_adwite_logo_light))

    with open(_adwite_logo_dark_temp, "wb") as _adwite_logo_dark_file:
        from base64 import b64decode

        _adwite_logo_dark_file.write(b64decode(_adwite_logo_dark))

    adwite_light_icon = (_adwite_light_temp).replace("\\", "//")
    adwite_dark_icon = (_adwite_dark_temp).replace("\\", "//")

    adwite_logo_light_icon = (_adwite_logo_light_temp).replace("\\", "//")
    adwite_logo_dark_icon = (_adwite_logo_dark_temp).replace("\\", "//")

    if master is None:
        from tkinter import _default_root
        _m = _default_root
    else:
        _m = master
    from tkinter import PhotoImage
    _m.iconbitmap(adwite_light_icon)


def dark_titlebar(master=None, value=True):
    if master is None:
        from tkinter import _default_root
        _m = _default_root
    else:
        _m = master
    import sys
    if sys.platform == "darwin" and not cls._deactivate_macos_window_header_manipulation:
        if Version(platform.python_version()) < Version("3.10"):
            if Version(tkinter.Tcl().call("info", "patchlevel")) >= Version("8.6.9"):
                os.system("defaults write -g NSRequiresAquaSystemAppearance -bool No")
    if sys.platform == "win32":
        import ctypes

        hwnd = ctypes.windll.user32.GetParent(_m.winfo_id())
        DWMWA_USE_IMMERSIVE_DARK_MODE = 20
        DWMWA_USE_IMMERSIVE_DARK_MODE_BEFORE_20H1 = 19

        # try with DWMWA_USE_IMMERSIVE_DARK_MODE
        if ctypes.windll.dwmapi.DwmSetWindowAttribute(hwnd, DWMWA_USE_IMMERSIVE_DARK_MODE,
                                                      ctypes.byref(ctypes.c_int(value)),
                                                      ctypes.sizeof(ctypes.c_int(value))) != 0:
            # try with DWMWA_USE_IMMERSIVE_DARK_MODE_BEFORE_20h1
            ctypes.windll.dwmapi.DwmSetWindowAttribute(hwnd, DWMWA_USE_IMMERSIVE_DARK_MODE_BEFORE_20H1,
                                                       ctypes.byref(ctypes.c_int(value)),
                                                       ctypes.sizeof(ctypes.c_int(value)))

from tkfly.metro import Frame

class TitleBar(Frame):
    def __init__(self, master=None, *args, **kwargs):
        super().__init__(master)
        self.configure(theme="light")
        if master is None:
            from tkinter import _default_root
            self._m = _default_root
        else:
            self._m = master
        self._init_widget()

        from tkmetro import Button, Label

        self.title = Label(self, height=40, text="MetroControl.Mult.TitleBar")
        self.title.configure(text_anchor="center", background=(255, 255, 255, 255))
        self.title.pack(fill="x", expand=True, side="left")

        self.close_button = Button(self, width=40, height=40, text="✕")
        self.close_button.configure(background=(255, 255, 255, 255))
        self.close_button.bind("<<Click>>", lambda _: self._m.quit())
        self.close_button.pack(side="right", padx=5, pady=5)

        from tkinter import Tk
        from win32gui import GetParent, SetWindowPos, UpdateWindow, SetWindowLong, GetWindowLong, ReleaseCapture, \
            SendMessage
        from win32con import NULL, SWP_NOSIZE, SWP_NOMOVE, SWP_NOZORDER, SWP_DRAWFRAME, GWL_STYLE, WS_CAPTION, \
            WM_SYSCOMMAND, SC_MOVE, HTCAPTION

        def titlebar(window):
            hWnd = GetParent(window.winfo_id())
            SetWindowLong(hWnd, GWL_STYLE, GetWindowLong(hWnd, GWL_STYLE) & ~WS_CAPTION)
            SetWindowPos(hWnd, NULL, 0, 0, 0, 0, SWP_NOSIZE | SWP_NOMOVE | SWP_NOZORDER | SWP_DRAWFRAME)
            UpdateWindow(hWnd)

        def windowMove(widget, window):
            def move():
                ReleaseCapture()
                SendMessage(GetParent(window.winfo_id()), WM_SYSCOMMAND, SC_MOVE + HTCAPTION, 0)

            widget.bind("<<Move>>", lambda event: move())

        windowMove(self.title, self._m)
        self._m.after(100, lambda: titlebar(self._m))

    def _init_widget(self):
        pass

    def show(self):
        self.pack(fill="x", side="top")
