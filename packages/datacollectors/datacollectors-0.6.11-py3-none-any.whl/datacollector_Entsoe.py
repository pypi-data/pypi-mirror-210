...
class Entsoe:
    ...
    def __init__(self, token, date_chunksize=20, batch_size=369, max_workers=24):
        self._token = token
    ...
    def _get_dayahead_prices(self, areas, start_date, end_date):
        ...
        for area in areas:
            ...
            for segment in segments:
                code_start_date = max(min(start_date, segment['to_date']), segment['from_date'])
                code_end_date = min(min(end_date, segment['to_date']), segment['to_date'])
                zone = segment['Code']
                # print(code_start_date, code_end_date, code_end_date-code_start_date)
                if code_end_date - code_start_date:
                    tasks += self._dayahead_prices_tasks(area, zone, code_start_date, code_end_date)
        indata = self._dayahead_prices_executor(tasks)
        return indata

    def _dayahead_prices_tasks(self, area, zone, start_date, end_date):
        document_type = 'A44'
    ...
    def _dayahead_prices_executor(self, tasks):
        indata_list = []
        batch_size = 60
        batch_duration = 10
