        ...
        async def _read_output_lines() -> None:
            ...
            while True:
                ...
                if current_line_length == 0:
                    ...
                    # of memory issues.
                    if (
                        line.startswith("/usr/local/bin/run_ingest.sh:")
                        and "Killed" in line
                    ):
                        ctx.get_report().report_error(
                            "The ingestion process was killed, likely because it ran out of memory. You can resolve this issue by allocating more memory to the datahub-actions container."
                        )
        ...
        async def _report_progress() -> None:
            while True:
                if ingest_process.returncode is not None:
                    logger.info(
                        f"Detected subprocess return code exec_id={exec_id} - stopping logs reporting"
                    )
                    break
