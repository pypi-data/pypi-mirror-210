				...
				double * logLIKELIHOOD, double * Beta, double *wald,double *intercept, int *n, int *kdim,int *VB)
{
	...
	if(verbose >1) printf("start EBLasso-NEG with a: %f, \tb: %f\n",a_gamma[0], b_gamma[0]);
	int basisMax			= 1e6/N;
	if (basisMax>M_full)	basisMax = M_full;
	if(verbose >1) printf("M_full: %d; basisMax: %d\n",M_full,basisMax);
	double vk				= 1e-30;
	double vk0				= 1e-30;
	double temp				= 0;
	int i,j,l;
	...
	Mu2						= (double * ) calloc(basisMax,sizeof(double));
	SIGMA2					= (double * ) calloc(basisMax*basisMax,sizeof(double));
	H2						= (double * ) calloc(basisMax*basisMax,sizeof(double));
	Alpha					= (double * ) calloc(basisMax,sizeof(double));
	PHI2					= (double * ) calloc(N*basisMax,sizeof(double));
	...
	while (iter<iter_max && err>err_max)
	{
		...
		err					= fabs(vk - vk0)/m[0];
		if(verbose >2) printf("Iteration number: %d, err: %f\n",iter,err);
	}
	...
	wald[0]					= 0;
	int index = 0;
	if(verbose >1) printf("EBLASSO-NEG Finished, number of basis: %d\n",M);
	for(i=0;i<M;i++)
    {
                   ...
                   int *iteration, int *n, int *kdim, int *m,double * LOGlikelihood,int basisMax, int verbose)
{
  ...
  double *basisCache;
  basisCache         = (double *) calloc(N*K,sizeof(double));
  for(i=0;i<K;i++)
  {
  ...
  tmp					= (double *) calloc(basisMax,sizeof(double));
  tmpp				= (double *) calloc(basisMax,sizeof(double));
  ...
  while(LAST_ITERATION!=1)
  {
    ...
    else if(selectedAction==ACTION_ADD)
    {
      ...
      if(index > (basisMax -10) && (N*K) > 1e7) {
        printf("bases: %d, warning: out of Memory, alloc more to Neffect!\n",index);
      }//return;
      for(i=0;i<index;i++)
      {
        tmp[i]				= 0;
