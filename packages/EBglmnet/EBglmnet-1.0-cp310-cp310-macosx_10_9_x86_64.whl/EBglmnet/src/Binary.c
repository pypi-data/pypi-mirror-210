...
void LinearSolver(double * a, double *logout, int N, int M,double *output)
{
	...
	int info			= 0;
	// *************************Call LAPACK library ************************
dgelsy(&N, &M, &nrhs, a, &N, logout, &N,jpvt,&Rcond, &rank, work, &lwork,&info);
	if(info!=0) 	
	{
		printf("Call linear solver degls error!\n");
		return;
	}
	int i;
				...
				double *BASIS, double *Targets, double *Scales, int * initial, int N, int *m, int K)
{
	...
	if(IniLogic==0)            // is empty
    {
		//printf("\t Inside Initialization, M: %d, K: %d\n",M, K);
        double *TargetPseudo,proj_ini,proj;
        int loc1 = 0;
        TargetPseudo		= (double *) calloc(N,sizeof(double));
		...
		double *PHIqr;
		PHIqr					= (double *) calloc(N*M,sizeof(double));
   	for(i=0;i<N;i++)		
		{
			PHI2[i]         = 1;
			PHIqr[i]		= 1;
    ...
    for(i=0;i<N;i++)
    {
        ...
        PHI2[N+i]		= PHI[i];
		  ...
		  double *logout;
      logout                      = (double *) calloc(N,sizeof(double));
...
void MatrixInverse(double * a,int N)
{
	...
	if(info!=0) 	
	{
		printf("Call 1st function. dpotrf error, Ill-conditioned Hessian!\n");
		return;
	}
	dpotri(&uplo,&N,a,&N,&info );
		if(info!=0) 	
	{
		printf("Call 2nd function dpotri error!\n");
		return;
	}
								...
								double *Targets, double *Alpha,int N, int M)
{
    ...
    errorLog					= (double *) calloc(itsMax,sizeof(double));
    ...
    DeltaMu						= (double *) calloc(M,sizeof(double));
    Mu_new2						= (double *) calloc(M,sizeof(double));
    double step					= 1.0;
				...
				int *n, int *m, int* kdim)
{
    ...
    temp1				= (double *) calloc(M*N,sizeof(double));
