			...
			double *Targets, double *Scales, int * initial, int N, int *m, int K, double *beta)
{
	...
	if(IniLogic==0)            // is empty
    {
        ...
        for(i=0;i<K;i++)
        {
         	proj			= 0;
			readPtr1 = &BASIS[i*N];
			proj = ddot(&N, readPtr1, &inci,Targets, &incj);
            proj			= proj/Scales[i];
		...
		p = ddot(&N, PHI, &inci,PHI, &incj);
		q = ddot(&N, PHI, &inci,Targets, &incj);
		p						= p*beta[0];
		q						= q*beta[0];
		Alpha[0]				= p*p/(q*q-p);
...
{
  double	zTargets;
  double *z2					= (double *) calloc(M,sizeof(double));
  double *cache1				= (double *) calloc(N,sizeof(double));
  double *cache2				= (double *) calloc(N*M,sizeof(double));
				 ...
				 double *gamma,int *n, int *m, int* kdim, int *iteration,int *i_iter)
{
	...
	double * PHIt		= (double *) calloc(M,sizeof(double));

	transa = 'T';
	a_blas = 1;
	b_blas = 0;
	//lda 	= N;
	dgemv(&transa, &N, &M,&a_blas, PHI, &N, Targets, &inci, &b_blas,PHIt, &incj );

	transa = 'N';
	//lda = M;
	dgemv(&transa, &M, &M,&a_blas, SIGMA, &M, PHIt, &inci, &b_blas,Mu, &incj );
	b_blas = beta[0];
	dscal(&M,&b_blas,Mu,&inci); //dscal(n, a, x, incx)
    ...
    //Main loop
    double *BPvector;
    BPvector			= (double *) calloc(M,sizeof(double));
				 ...
				 double *gamma,int *n, int *m, int* kdim, int *iteration,int *i_iter)
{
    ...
    for(i=0; i<M_full; i++)
    {
		...
		b_blas 		= 0;
		dgemv(&transa, &M, &M,&a_blas, SIGMA, &M, readPtr1, &M_full, &b_blas,BPvector, &incj );
		
		
		
        tempSum					= 0;
		readPtr1 	= &BASIS_PHI[i];
		tempSum		= ddot(&M, BPvector, &inci,readPtr1, &M_full);
		
		S_in[i]					= beta[0] - beta[0]*tempSum*beta[0];
		tempBPMu				= 0;
		readPtr1 	= &BASIS_PHI[i];
		tempBPMu 	= ddot(&M, Mu, &inci,readPtr1, &M_full);		
		Q_in[i]					= beta[0]*(BASIS_Targets[i] - tempBPMu);
...
{
	double *BASIS_Phi		= (double *) calloc(M_full,sizeof(double));
	double *BASIS_B_Phi		= (double *) calloc(M_full,sizeof(double));
	double *mCi				= (double *) calloc(M_full,sizeof(double));
	double *z				= (double *) calloc(N,sizeof(double));
	//int kk					= K;
	int i,j,h;
	int index				= M + 1;
	double*   	tmp			= (double *) calloc(M,sizeof(double));
  	double*		tmpp		= (double *) calloc(M,sizeof(double));
	...
	for(i=0;i<M;i++)
    {
        tmp[i]					= 0;
		readPtr1 	= &PHI[i*N];
		tmp[i]  = ddot(&N, readPtr1, &inci,Phi,&incj);
    }
	b_blas = beta[0];
	dscal(&M,&b_blas,tmp,&inci); 		//dscal(n, a, x, incx) x = a*x
    ...
    for(i=0;i<M;i++)
    {
        tmpp[i]					= 0;
		readPtr1 	= &SIGMA[i*M];
		tmpp[i]  = ddot(&M, readPtr1, &inci,tmp,&incj);	
    }//tmpp is tmp in matlab.

	Alpha[M]				= newAlpha;                 //new element

	readPtr1 	= &PHI[M*N];
	dcopy(&N,Phi,&inci,readPtr1,&incj);  //dcopy(n, x, incx, y, incy) ---> y = x

	s_ii					= 1.0/(newAlpha + S_in[nu]);   
	
	mu_i					= s_ii*Q_in[nu];
	b_blas = -mu_i;
	daxpy(&M, &b_blas,tmpp, &inci,Mu, &incj); 
	
    Mu[M]					= mu_i;							//new element
	
	double * s_i			= (double *) calloc(M,sizeof(double));
	dcopy(&M,tmpp,&inci,s_i,&incj);  //dcopy(n, x, incx, y, incy) ---> y = x
	b_blas = -s_ii;
	dscal(&M,&b_blas,s_i,&inci); 		//dscal(n, a, x, incx) x = a*x
	...
	dcopy(&M,s_i,&inci,readPtr1,&incj);  //dcopy(n, x, incx, y, incy) ---> y = x
	readPtr1 = &SIGMANEW[M];
	dcopy(&M,s_i,&inci,readPtr1,&index);  //dcopy(n, x, incx, y, incy) ---> y = x
			...
			double*Q_in, int nu, double*SIGMANEW, int M_full,int N, int K, int M)
{
	...
	for(i=0;i<M_full;i++)
	{
		temp				= 0;
		readPtr1 			= &BASIS_PHI[i];
		temp 				= ddot(&M, readPtr1, &M_full,tmpp,&incj); 
...
int ActionDel(double*PHI, double*Alpha, double*SIGMA, double*SIGMANEW, double**BASIS_PHI,
		double*Mu, double*S_in, double*Q_in, double *beta, int jj, int N, int M, int M_full)
 {
	...
	dcopy(&N,readPtr2,&inci,readPtr1,&incj);  //dcopy(n, x, incx, y, incy) ---> y = x
	...
	Mu[jj]					= Mu[index];        
	//------------------------------------------------------------------------------
	double *tempSIGMA = (double *) calloc((M*M),sizeof(double));
	for(i=0;i<M;i++)
	{
	...
	dcopy(&index,readPtr2,&inci,readPtr1,&incj);  //dcopy(n, x, incx, y, incy) ---> y = x
	...
	dcopy(&index,readPtr2,&M,readPtr1,&index);  //dcopy(n, x, incx, y, incy) ---> y = x
	}
...
int ActionDelGmNeg(double*PHI, double*Alpha, double*SIGMA, double*SIGMANEW, double*BASIS_PHI,
		double*Mu, double*S_in, double*Q_in, double *beta, int jj, int N, int M, int M_full)
 {
	...
	dcopy(&M_full,readPtr2,&inci,readPtr1,&incj);  //dcopy(n, x, incx, y, incy) ---> y = x
			...
			double *Mu, double *Alpha, double *beta, int N, int M)
{
	...
	ldc 	= M;
	b_blas 	= 0;	
	dgemm(&transa, &transb,&M, &M, &ldk,&a_blas, PHI, &lda, PHI, &ldb, &b_blas, H, &ldc );
	//dgemm(transa, transb, m, n, k, alpha, a, lda, b, ldb, beta, c, ldc)
	//	C := alpha*op(A)*op(B) + beta*C,
	b_blas 	= beta[0];
	MM	 	= M*M;
	dscal(&MM,&b_blas,H,&inci); 		//dscal(n, a, x, incx) x = a*x
	
	for(i=0;i<M;i++)			H[i*M+i]		= H[i*M+i] + Alpha[i];
		
	dcopy(&MM,H,&inci,SIGMA,&incj);  //dcopy(n, x, incx, y, incy) ---> y = x
