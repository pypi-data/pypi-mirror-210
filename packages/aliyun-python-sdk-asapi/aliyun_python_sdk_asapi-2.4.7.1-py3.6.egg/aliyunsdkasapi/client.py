...
class AcsClient:
    ...
    def appendUserAgent(key, value):
        global userAgents
        userAgents[key] = value

    def resolveAgent(self,requestAgent):
        finalDict = dict(requestAgent)
        finalDict.update(userAgents)
        agents = ''
    ...
    def setSdkSource(self, sdkSource):
        self._sdk_source = sdkSource
    ...
    def _check_before_request(self, request):
        if not request.endpoint or len(request.endpoint) == 0:
            raise exceptions.ClientException(
            error_code.SDK_INVALID_PARAMS,'You should set asapi endpoint.')
        if self._sdk_source is None:
            raise exceptions.ClientException(
            error_code.SDK_INVALID_PARAMS,"You should call 'ASClient->setSdkSource' method to identify yourself for asapi.")
        if self._ak is not None and len(self._ak) == 0:
            raise exceptions.ClientException(
            error_code.SDK_INVALID_PARAMS,"You should set AccessKeyId in request parameters.")
        if self._secret is not None and len(self._secret) == 0:
            raise exceptions.ClientException(
            error_code.SDK_INVALID_PARAMS,"You should set AccessKeySecret in request parameters.")
        if not request.get_product():
            raise exceptions.ClientException(
            error_code.SDK_INVALID_PARAMS,'You should set Product in request parameters')
        if not request.get_version():
            raise exceptions.ClientException(
            error_code.SDK_INVALID_PARAMS,'You should set Version in request parameters')
        if not request.get_action_name():
            raise exceptions.ClientException(
            error_code.SDK_INVALID_PARAMS,'You should set Action in request parameters')
        if not self._region_id:
            raise exceptions.ClientException(
            error_code.SDK_INVALID_PARAMS,'You should set RegionId in request parameters')
    ...
    def _make_http_response(self, endpoint, request, read_timeout, connect_timeout,
        ...
        self._check_before_request(request)
        ip = '';
        try:
            ...
            logger.warning('Failed to get ip:%s', str(e))
        finally:
            s.close()
        request.add_header('x-acs-caller-ip', ip)
        request.add_header('x-acs-caller-sdk-source', self._sdk_source)
        request.add_header('x-acs-caller-sdk-language', 'python')
        import aliyunsdkasapi
        request.add_header('x-acs-caller-sdk-version', aliyunsdkasapi.__version__)
        if self.sign_with_asapi == 1 and request.get_method() == 'POST':
            body_params['Product'] = request.get_product()
            body_params['Version'] = request.get_version()
            body_params['Action'] = request.get_action_name()
            body_params['Format'] = request.get_accept_format()
        if body_params:
        ...
        # for key, value in request.get_query_params().items():
        for key in list(request.get_query_params().keys()):
            if request.get_query_params()[key] is None:
                request.get_query_params().pop(key)
            # request.get_query_params()[key] = 'null'

        if self.sign_with_asapi == 1 and request.get_method() == 'POST':
            request.add_header('x-ascm-sign-version', '1.0')
        ...
        if body_params:
            ...
            if self.sign_with_asapi == 1:
                # response.set_content(request.get_body_params(), "utf-8", format_type.APPLICATION_JSON)
                response.set_content_type(format_type.APPLICATION_JSON)
                response.set_body(request.get_body_params())
            else:
    ...
    def do_request(self, acs_request):
        self.sign_with_asapi = 1
        status, headers, body, exception = self._implementation_of_do_action(acs_request)
        return body
