...
class FastdupVisualizer:
    ...
    def component_gallery(self, save_path: str = None, label_col: str = None, draw_bbox: bool = False,
        ...
        label_col = self._compute_label_col(label_col, load_crops)
        if draw_bbox and external_df is None:
            external_df = self._controller.connected_components(load_crops=load_crops)
            assert isinstance(external_df, tuple), f"Wrong return values from connected components {external_df}"
            external_df = external_df[0]
            external_df = external_df.dropna()
            external_df = load_and_merge_stats(external_df, metric, self._controller.work_dir, kwargs)
            ...
            external_df = pd.DataFrame({'files': files.values, 'component_id': files.index.values, 'files_ids': files.values if crop_files is None else crop_files.values,
