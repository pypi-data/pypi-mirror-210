...
if __name__ == "__main__":
    ...
    if False:
        ...
        fd.run(overwrite=True, model_path='dinov2s', verbose=False)
        fd.run(overwrite=True, verbose=True)
    elif False:
        import fastdup
        os.chdir("../unittests")
        ...
        ret = fd.run(annotations=coco_annotations, overwrite=True, num_images=1000, cc_threshold=0.8, threshold=0.8, print_summary=False)
        assert ret == 0
        ret = fd.vis.component_gallery(metric='size',slice='diff',draw_bbox=True)
        assert ret == 0
