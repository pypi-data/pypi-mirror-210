...
#         return "<Job Group Object> {} | Headcount: {}".format(self.name, self.df.shape[0])
    ...
    def overall_gap_ttest(self, segment=None):
        df_temp = self.df.copy()
        df_temp = df_temp.merge(self.predictions,right_on=self.eeid, left_on=self.column_map_inv[self.eeid], how='right')
        df_temp["Residual"] = np.log(df_temp["Actual"])-np.log(df_temp["Prediction"])
        
        if segment is None or segment not in df_temp.columns:
            df_temp['OVR_TTEST'] = "OVR"
            segment = 'OVR_TTEST'
        ...
        for feature in df_temp[segment].unique():
            ...
            for diversity in self.div_vars:
                ...
                div = self.column_map_inv[self.div_vars[diversity]]
