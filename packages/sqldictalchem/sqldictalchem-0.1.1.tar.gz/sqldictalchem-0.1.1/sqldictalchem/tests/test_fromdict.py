...
from sqldictalchem import (
    SQLAlchemError,
)

from sqldictalchem.tests import (
    TestCase,
    Named,
    NamedWithOtherPk,
    OneToManyChild,
...
class TestFromdict(TestCase):
    ...
    def test_sqldictalchem_include_doesnt_override_pk(self):
        h = Named('a name')
        try:
            h.fromdict({'id': 1}, include=['id'])
            assert False
        except SQLAlchemError:
            assert True



    def test_sqldictalchem_include(self):
        m = WithHybrid(2)
        m.fromdict({'value': 7})
        assert m.value == 2
        setattr(m, 'sqldictalchem_include', ['value'])
        m.fromdict({'value': 7})
        assert m.value == 7

    def test_sqldictalchem_asdict_include_overrides(self):
        m = WithHybrid(2)
        m.fromdict({'value': 7})
        assert m.value == 2
        setattr(m, 'sqldictalchem_include', ['value'])
        m.fromdict({'value': 7})
        setattr(m, 'sqldictalchem_fromdict_include', [])
        m.fromdict({'value': 2})
        assert m.value == 7
