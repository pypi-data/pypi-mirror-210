...
from sqldictalchem import constants
from sqldictalchem import errors
...
def asdict(model, exclude=None, exclude_underscore=None, exclude_pk=None,
    ...
    :param exclude: List of properties that should be excluded, will be \
            merged with `model.sqldictalchem_exclude`
    :param exclude_pk: If True any column that refers to the primary key will \
            be excluded.
    :param exclude_underscore: Overides `model.sqldictalchem_exclude_underscore`\
            if set
    :param include: List of properties that should be included. Use this to \
            ...
            with `model.sqldictalchem_asdict_include` or \
            `model.sqldictalchem_include`.
    ...
    :raises: :class:`sqldictalchem.errors.MissingRelationError` \
            if `follow` contains a non-existent relationship.
    :raises: :class:`sqldictalchem.errors.UnsupportedRelationError` If `follow` \
    ...
    if only:
        ...
        include = (include or []) + (getattr(model,
                                             'sqldictalchem_asdict_include',
                                             getattr(model,
                                                     'sqldictalchem_include',
                                                     None)) or [])
...
def fromdict(model, data, exclude=None, exclude_underscore=None,
    ...
    Works almost identically as :meth:`sqldictalchem.utils.asdict`. However, it
    ...
    * Simple 1-m relationships
    ...
    :param exclude_underscore: If True underscore properties will be excluded,\
            if set to None model.sqldictalchem_exclude_underscore will be used.
    :param allow_pk: If True any column that refers to the primary key will \
            be excluded. Defaults model.sqldictalchem_fromdict_allow_pk or \
    ...
    :raises: :class:`sqldictalchem.errors.SQLAlchemError` If a primary key is \
    ...
    if allow_pk is None:
        allow_pk = getattr(model, 'sqldictalchem_fromdict_allow_pk',
    ...
    if only:
        ...
        include = (include or []) + (getattr(model,
                                             'sqldictalchem_fromdict_include',
                                             getattr(model,
                                                     'sqldictalchem_include',
                                                     None)) or [])
    ...
    if len(data_primary_key) and not allow_pk:
        msg = ("Primary keys({0}) cannot be updated by fromdict."
               "Set 'sqldictalchem_fromdict_allow_pk' to True in your Model"
               " or pass 'allow_pk=True'.").format(','.join(data_primary_key))
        raise errors.SQLAlchemError(msg)
...
def make_class_dictable(
    ...
    :param exclude: Will be set as sqldictalchem_exclude on the class
    :param exclude_underscore: Will be set as sqldictalchem_exclude_underscore \
            on the class
    :param fromdict_allow_pk: Will be set as sqldictalchem_fromdict_allow_pk\
            on the class
    :param include: Will be set as sqldictalchem_include on the class.
    :param asdict_include: Will be set as `sqldictalchem_asdict_include` on the \
            class. If not None it will override `sqldictalchem_include`.
    :param fromdict_include: Will be set as `sqldictalchem_fromdict_include` on \
            the class. If not None it will override `sqldictalchem_include`.

    :returns: The class
    """

    setattr(cls, 'sqldictalchem_exclude', exclude)
    setattr(cls, 'sqldictalchem_exclude_underscore', exclude_underscore)
    setattr(cls, 'sqldictalchem_fromdict_allow_pk', fromdict_allow_pk)
    setattr(cls, 'asdict', asdict)
    setattr(cls, 'fromdict', fromdict)
    setattr(cls, '__iter__', iter)
    setattr(cls, 'sqldictalchem_include', include)
    setattr(cls, 'sqldictalchem_asdict_include', asdict_include)
    setattr(cls, 'sqldictalchem_fromdict_include', fromdict_include)
    return cls
