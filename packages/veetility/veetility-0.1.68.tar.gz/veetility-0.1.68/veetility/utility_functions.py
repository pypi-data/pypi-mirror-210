...
class UtilityFunctions():
    ...
    def duplicates_check(self, df, name_of_df, perc_dupes_thresh=3, cols_to_check=None, 
                        cols_to_add=None, return_type='error_message',raise_exceptions=True):
        """Check for duplicates in the dataframe"""

        num_rows = len(df)
        self.logger.logger.info(f'Num of rows in {name_of_df} = {num_rows}')
        paid_or_organic = self.identify_paid_or_organic(df)
        ...
        for i in range(2,len(cols_to_check)+1):
            ...
            self.logger.logger.info(f'{num_duplicates} - {name_of_df} - {cols_to_check[:i]}')
        
        exceed_thresh = df.duplicated(subset=cols_to_check).sum()*100 / num_rows > perc_dupes_thresh

        if raise_exceptions and exceed_thresh:
            raise Exception(f'Number of duplicates in {name_of_df} exceeds {perc_dupes_thresh}%')
        
        elif return_type == 'error_message' and exceed_thresh:
            return f'Number of duplicates in {name_of_df} exceeds {perc_dupes_thresh}%'
