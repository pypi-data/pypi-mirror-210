...
class BlockBuilder:
    ...
    def many_context_block(value, text_type="plain_text", block_id=None):
        ...
        if not isinstance(value, list):
            raise Exception(
                f"Please use an array of text values if you do not want multiple use the simple_context_block {isinstance(value,list)},{type(value)}"
            )
        if not block_id:
            block_id = str(uuid.uuid4())
    ...
    def section_with_button_block(
        ...
        return BlockBuilder.section_with_accessory_block(
            section_text, BlockBuilder.simple_button_block(button_label, button_value, url, style, confirm, action_id), text_type, block_id
        )
