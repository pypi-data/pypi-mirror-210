...
class SlackApp:
    ...
    def open_modal_from_trigger(self, trigger_id, view):
        """
        Opens a modal from a trigger_id
        """
        url = self.SLACK_API_ROOT + self.VIEW_OPEN
        data = {"trigger_id": trigger_id, "view": view}
        res = requests.post(url, data=json.dumps(data), headers=self.JSON_HEADERS)
