...
class SlackApp:
    ...
    def _block_action_consumer(self, payload):
        """a function that consumes block actions and routes them to the correct function"""
    ...
    def _block_suggestion_consumer(self, payload):
        """a function that consumes block suggestions and routes them to the correct function"""
    ...
    def open_modal_from_trigger(self, trigger_id, view, access_token):
        """
        Opens a modal from a trigger_id
        This requires an access token either workspace or user
        """
        url = self.SLACK_API_ROOT + self.VIEWS_OPEN
        data = {"trigger_id": trigger_id, "view": view}
        res = requests.post(url, data=json.dumps(data), headers=self.auth_headers(access_token))
        return self._handle_response(res, blocks=view.get("blocks"))

    def respond_with_modal(self, trigger_id, view):
        """
        response to a user action by opening a modal
        @param trigger_id: the trigger id from the user action
        @param view: the view object to open
        """
