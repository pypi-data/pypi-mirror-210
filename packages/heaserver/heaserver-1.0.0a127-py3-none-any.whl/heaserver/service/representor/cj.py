...
class CJ(Representor):
    ...
    def __format(request: Request, wstl_obj: Dict[str, Any], link_callback: Callable[[int, Link], None] = None) -> Dict[
        ...
        if 'template' not in collection:
            template = _get_template(wstl.get('actions', []), tvars if len(wstl.get('data', [])) <= 1 else {})
            if template:
...
def add_extended_property_values(wstl_action, template_input):
    ...
    if has_extended_property_value('object-url-target-types-include', wstl_action):
        template_input['object-url-target-types-include'] = get_extended_property_value('object-url-target-types-include', wstl_action)
