...
def filter_barcodes(data,experiment):
    ...
    out["id"] = out["Feature"].astype(str) + out["Orientation"] + out["Reference"]
    ids = get_hash_ids(out["id"]) 
    out["id"] = ids
    ...
    mask = pool.starmap_async(get_filter_mask, zip(unique_ids, repeat(ids),
...
def filter_counts(counts, thr=0.05):
    """
...
def get_count_matrix(ids, bowtie_res, experiment):
    ...
    # exclude barcodes which occur only once
    unique_ids = unique_ids[counts>=5]
