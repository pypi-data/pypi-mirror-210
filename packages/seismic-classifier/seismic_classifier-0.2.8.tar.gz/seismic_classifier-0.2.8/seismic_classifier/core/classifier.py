...
from seismic_classifier.ai import database as ai_database
from seismic_classifier.ai import dataset
from seismic_classifier.ai.event_type_lookup import event_type_lookup
...
from seismic_classifier.ai.model import EventClassifier, EventClassifier2
...
class ClassifierResult(object):
    def __init__(self, raw_output, label_mapping, inputs, spectrograms,
                 inventory, event_location=None):
    ...
    def raw_output_strings(self):
        output_strings = []
        for probability in self.probabilities.detach().numpy():
            output_string = ''
            indices = np.argsort(probability)[::-1]
            keys = [key for key in self.label_mapping.keys()]
            for j, i in enumerate(indices):
                output_string += f'{keys[i]}: {probability[i] * 100:2.0f}%'
                if j < 2:
                    # add divider
                    output_string += ' | '
    ...
    def predicted_class_ensemble(self, event_location=None):
        ...
        if event_location is not None:
            if inventory is None:
                logger.error('No inventory. The event location cannot be used')
                return
    ...
    def to_dict(self):
        ...
        for i in range(0, len(raw_outputs)):
            outputs.append({'network': self.inputs[i].stats.network,
                            ...
                            'location': self.inputs[i].stats.location,
    ...
    def __repr__(self):
        ...
        for tr, predicted_class, probability in zip(self.inputs,
...
class Classifier(ProjectManager):
    ...
    def create_spectrogram_training(self, stream: Stream, event_type: str,
                                    ...
                                    event_time: UTCDateTime, magnitude: float,
                                    expect_impulsive=True, override_exist=False,
        ...
        for window_length in self.window_length_seconds:
            for tr in stream.copy():
                ...
                training_db_manager = ai_database.DBManager(
                    self.databases.training_database)
                training_db_manager.add_record(event_id=event_id,
                ...
                for filename, tr in zip(filenames, st_trimmed):
                    ...
                    path = (classifier_dataset_path / self.network / event_type
                            / "time_domain")
                    path.mkdir(parents=True, exist_ok=True)
                    plt.savefig(str(path / f'{filename.stem}_td.png'))

            # Continue to the next iteration if there are no resampled traces

    def write_spectrogram(self, spectrograms, event_type, event_time, magnitude, duration,
        ...
        for i, spectrogram in enumerate(spectrograms):
            ...
            filename = f'{event_type}_{event_time}' \
            ...
            data_path = self.paths.training_dataset / self.network / event_type \
                        / 'spectrograms'
            data_path.mkdir(parents=True, exist_ok=True)

            # save the spectrogram image to disk
            path = self.paths.training_dataset / self.network / event_type \
                   / 'spectrograms' / filename
            spectrogram.save(path, format='png')
    ...
    def split_dataset(self, split_test=0.2, split_validation=0.1, use_synthetic=True):
        ...
        training_db_manager = ai_database.DBManager(
            self.databases.training_database)
        session = training_db_manager.Session()
        ...
        if use_synthetic:
            seismic_events = training_db_manager.filter(categories='seismic event',
                                                        duration=wl)
            ...
            training_db_manager = ai_database.DBManager(
                self.databases.training_database)
            seismic_events = training_db_manager.filter(synthetic=False,
                                                             categories='seismic event',
                                                             duration=wl)
        blasts = training_db_manager.filter(categories='blast', duration=wl)
        noises = training_db_manager.filter(categories='noise', duration=wl)

        df = training_db_manager.to_pandas()
    ...
    def label_mapping(self):
        ...
        if self.event_classifier is None:
            unique_labels = training_db_manager.categories
            label_mapping = {}
...
class Classifier2(Classifier):
    ...
    def split_dataset(self, split_test=0.2, split_validation=0.1, use_synthetic=True):
        ...
        seismic_events = training_db_manager.filter(categories='seismic event')
        blasts = training_db_manager.filter(categories='blast')
        noises = training_db_manager.filter(categories='noise')
    ...
    def create_training_dataset(self, data_path, reset_training=False, resume=True,
        ...
        if resume:
            processed_mseed_files = training_db_manager.all_unique('mseed_file')
            mseed_files = [f for f in mseed_files if f.name not in processed_mseed_files]

        if reset_training:
            training_db_manager.clear_database()
    ...
    def create_training_dataset_hsf(self, data_path, reset_training=False, resume=True,
                                    multiprocessing=False, network=None):
        training_db_manager = ai_database.DBManager(
            self.databases.training_database)
        ...
        data_path = Path(data_path)
        hsf_files = [filename for filename in data_path.glob('*.hsf')]
        if resume:
            processed_mseed_files = training_db_manager.all_unique('hsf_file')
            hsf_files = [f for f in hsf_files if f.name not in processed_mseed_files]
        ...
        if multiprocessing:
            ...
            list(tqdm(pool.imap(self.__process__, hsf_files), total=len(hsf_files)))
        else:
            for hsf_file in tqdm(hsf_files):
                self.__process__(hsf_file)
                # try:
                #     self.__process__(hsf_file)
                # except Exception as e:
                #     logger.error(e)
    ...
    def __process__(self, f):
        ...
        if f.suffix == '.mseed':
            ...
            hsf = HSFHandler.read(str(f), self.network)
            fb = hsf.file_bundle
            cat = fb.catalog
            st = fb.stream
            self.add_inventory(fb.inventory, initialize_travel_time=False)

        logger.info(cat[0].event_type)

        # new
        # if np.random.rand() > 0.7:
        #     traces = []
        #     for tr in st:
        #         if 'GP' in tr.stats.channel:
        #             tr = tr.differentiate()
        #             traces.append(tr)
        #
    ...
    def __process_seismic_event__(self, st, cat, f, snr_threshold=7):
        ...
        for arr in tqdm(origin.arrivals):
            pk = arr.pick
            st2 = st.copy().select(station=pk.waveform_id.station_code,
                                   location=pk.waveform_id.location_code)
            for tr in st2:
                ...
                for i in range(3):
                    ...
                    self.create_spectrogram_training(Stream(traces=[tr2]),
                                                     'seismic event',
                                                     cat[0].event_type,
                                                     f.name, endtime, origin_time,
                                                     magnitude,
                                                     simulate_magnitude=False)
    ...
    def __process_blast__(self, st, cat, f):
        ...
        # if self.event_classifier.model:
        #     set_trace()
        #     self.predict(st)
        ...
        for arr in tqdm(origin.arrivals):
            ...
            for tr in st2:
                end_window = pk.time + 0.6 + np.random.rand() * 0.5
                magnitude = self.__get_magnitude__(cat)
                ...
                self.create_spectrogram_training(Stream(traces=[tr]), 'blast',
                                                 cat[0].event_type,
                                                 f.name, end_window, origin.time,
                                                 magnitude, simulate_magnitude=False)
