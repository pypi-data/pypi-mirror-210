...
class BMA220_TAP(BMA220):
    ...
    _tt_z_enabled = RWBit(_CONF, 0)
    _tt_y_enabled = RWBit(_CONF, 1)
    _tt_x_enabled = RWBit(_CONF, 2)
    _tt_int = ROBit(_INTERRUPTS, 4)
    _tt_threshold = RWBits(4, _TT_INFO, 3)
    _tt_duration = RWBits(3, _TT_INFO, 0)
    _tt_filter_enable = RWBit(_TT_INFO, 7)
    _double_tap_enabled = RWBit(0x0A, 4)

    _tt_sign = RWBit(_TT_INFO2, 0)
    _tt_z_first = RWBit(_TT_INFO2, 1)
    _tt_y_first = RWBit(_TT_INFO2, 2)
    _tt_x_first = RWBit(_TT_INFO2, 3)
