...
def architecture() -> Dict[str, Any]:
    ...
    """
    arch = {
        "cpu": pylikwid.getcpuinfo()["short_name"],
    }
    if hasattr(pylikwid, "getgputopology"):
        arch["gpu"] = [
            (str(gpu["ccapMajor"]) + str(gpu["ccapMinor"]))
            for gpu in pylikwid.getgputopology()
        ]
    return arch
...
def topology() -> Dict[str, Dict]:
    ...
    """
    topo = {"cpu": pylikwid.getcputopology()}
    if hasattr(pylikwid, "getgputopology"):
        topo["gpu"] = pylikwid.getgputopology()
    return topo
