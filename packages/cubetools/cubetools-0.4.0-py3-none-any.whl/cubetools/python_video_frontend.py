        ...
        with gr.Blocks(title=title) as self.demo:
            ...
            if local_video:
                with gr.Tab("本地视频"):
                    ...
                    with gr.Row():
                        self.local_video = gr.Video(show_label=False)
                        img_local_video = gr.Image(show_label=False)
                    self.btn_play_local_video = gr.Button(value='播放')
                    self.btn_stop_local_video = gr.Button(value='停止', visible=False)
                    self.error_local_video = gr.Textbox(label='播放状态：', visible=False, interactive=False)

                    self.btn_play_local_video.click(self.play_local_video,
                                                    inputs=[self.local_video],
                    ...
                    self.btn_stop_local_video.click(self.stop_local_video,
                                                    outputs=[playing_local_video, self.local_video,
                                                             self.btn_play_local_video, self.btn_stop_local_video],
                                                    show_progress=False)
                    ...
                    if examples:
                        gr.Examples(label='示例', examples=examples, inputs=[self.local_video])
    ...
    def play_local_video(self, local_video):
        ...
        if img_or_err.startswith('data:image'):
            return img_or_err, url, 1, random.random(), '', \
                   ...
                   self.local_video.update(interactive=False), \
        ...
        return None, url, 0, 0, img_or_err, \
               ...
               self.local_video.update(interactive=True), \
