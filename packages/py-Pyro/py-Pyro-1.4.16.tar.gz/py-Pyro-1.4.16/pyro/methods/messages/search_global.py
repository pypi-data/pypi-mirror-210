...
class SearchGlobal(Scaffold):
    async def search_global(
        ...
        If you want to get the messages count only, see :meth:`~pyro.Client.search_global_count`.
        ...
        Returns:
            ``Generator``: A generator yielding :obj:`~pyro.types.Message` objects.

        Example:
            .. code-block:: python

                # Search for "pyro". Get the first 50 results
                for message in app.search_global("pyro", limit=50):
                    print(message.text)
