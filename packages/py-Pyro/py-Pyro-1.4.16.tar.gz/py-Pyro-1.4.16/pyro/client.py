...
import pyro
from pyro import __version__, __license__
from pyro import raw
from pyro import utils
from pyro.crypto import aes
from pyro.errors import CDNFileHashMismatch
from pyro.errors import (
    SessionPasswordNeeded,
    VolumeLocNotFound, ChannelPrivate,
    AuthBytesInvalid, BadRequest
)
from pyro.handlers.handler import Handler
from pyro.methods import Methods
from pyro.session import Auth, Session
from pyro.storage import Storage, FileStorage, MemoryStorage
from pyro.types import User, TermsOfService
from pyro.utils import ainput
...
class Client(Methods, Scaffold):
    ...
    Parameters:
        session_name (``str``):
            ...
            :meth:`~pyro.Client.export_session_string` before stopping the client to get a session string you can
    ...
    def load_config(self):
        ...
        if self.bot_token:
            ...
            self.bot_token = parser.get("pyro", "bot_token", fallback=None)

        if self.api_id and self.api_hash:
            pass
        else:
            if parser.has_section("pyro"):
                self.api_id = parser.getint("pyro", "api_id")
                self.api_hash = parser.get("pyro", "api_hash")
            else:
                raise AttributeError("No API Key found. More info: https://docs.pyro.org/intro/setup")
    ...
    async def get_file(
        ...
        try:
            ...
            if not isinstance(e, pyro.StopTransmission):
                log.error(e, exc_info=True)
