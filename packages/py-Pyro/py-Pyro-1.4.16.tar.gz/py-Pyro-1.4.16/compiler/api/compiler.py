...
DESTINATION_PATH = Path("pyro/raw")
NOTICE_PATH = "NOTICE"
...
def get_docstring_arg_type(t: str, is_list: bool = False, is_pyro_type: bool = False):
    if t in CORE_TYPES:
        if t == "long":
            return "``int`` ``64-bit``"
        elif "int" in t:
        ...
        return "Any object from :obj:`~pyro.raw.types`"
    elif t == "!X":
        return "Any method from :obj:`~pyro.raw.functions`"
    elif t.lower().startswith("vector"):
        return "List of " + get_docstring_arg_type(t.split("<", 1)[1][:-1], True)
    else:
        return f":obj:`{t} <pyro.raw.base.{t}>`"
...
def start(format: bool = False):
    ...
    for qualtype in types_to_constructors:
        ...
        items = "\n            ".join([f"- :obj:`{c} <pyro.raw.types.{c}>`" for c in constructors])
        ...
        for i, arg in enumerate(sorted_args):
            ...
            docstring_args.append(
                "{}{}: {}".format(
                    ...
                    get_docstring_arg_type(arg_type, is_pyro_type=c.namespace == "pyro")
                )
            )

        if c.section == "types":
            docstring += f"This object is a constructor of the base type :obj:`~pyro.raw.base.{c.qualtype}`.\n\n"
        else:
    ...
    with open(DESTINATION_PATH / "all.py", "w", encoding="utf-8") as f:
        ...
        for c in combinators:
            f.write(f'\n    {c.id}: "pyro.raw.{c.section}.{c.qualname}",')

        f.write('\n    0xbc799737: "pyro.raw.core.BoolFalse",')
        f.write('\n    0x997275b5: "pyro.raw.core.BoolTrue",')
        f.write('\n    0x1cb5c415: "pyro.raw.core.Vector",')
        f.write('\n    0x73f1f8dc: "pyro.raw.core.MsgContainer",')
        f.write('\n    0xae500895: "pyro.raw.core.FutureSalts",')
        f.write('\n    0x0949d9dc: "pyro.raw.core.FutureSalt",')
        f.write('\n    0x3072cfa1: "pyro.raw.core.GzipPacked",')
        f.write('\n    0x5bb8e511: "pyro.raw.core.Message",')
