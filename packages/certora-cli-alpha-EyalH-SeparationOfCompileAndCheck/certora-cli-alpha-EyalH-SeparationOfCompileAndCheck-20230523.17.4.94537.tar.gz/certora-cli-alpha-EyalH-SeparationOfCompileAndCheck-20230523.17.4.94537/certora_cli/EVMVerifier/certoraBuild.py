...
class CertoraBuildGenerator:
    ...
    def build(self, context: CertoraContext) -> None:
        ...
        for i, build_arg_contract_file in enumerate(sorted(self.input_config.files)):
            ...
            shutil.copytree(get_certora_sources_dir(), copy_temp, ignore=ignore_patterns, dirs_exist_ok=True)
            shutil.copytree(copy_temp, pre_backup_dir)
            ...
            if compiler_lang == CompilerLangSol():
                ...
                copy_temp = tempfile.mkdtemp()
                shutil.copytree(get_certora_sources_dir(), copy_temp, ignore=ignore_patterns, dirs_exist_ok=True)
                shutil.copytree(copy_temp, post_backup_dir)
                shutil.rmtree(copy_temp)
    ...
    def collect_sources(self, context: CertoraContext) -> Set[Path]:
        ...
        if context.mode == Mode.BYTECODE:
            bytecode_attr = context.bytecode if is_new_api() else context.bytecode_jsons
            for bytecode_resource in bytecode_attr + [context.bytecode_spec]:
