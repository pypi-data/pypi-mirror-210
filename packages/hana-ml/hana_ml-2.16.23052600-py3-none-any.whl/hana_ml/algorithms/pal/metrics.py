...
def confusion_matrix(data, key, label_true=None, label_pred=None,
        ...
        try:
            ...
            with conn.connection.cursor() as cur:
                for sql in (cm_insert_sql, cr_insert_sql):
                    execute_logged(cur, sql, conn.sql_tracer, conn)
    ...
    try:
        ...
        logger.exception("Fit failure, the error message: %s", str(db_er.args[1]), exc_info=True)
        try_drop(conn, tables)
        raise
    cm_df = conn.table(cm_tbl)
...
def auc(data, positive_label=None, output_threshold=None):
    ...
    DataFrame
        ...
        False positive rate and true positive rate (ROC), structured as follows:
            ...
            - THRESHOLD, type DOUBLE, representing the corresponding threshold value,
              available only when ``output_threshold`` is set to True.
...
def accuracy_score(data, label_true, label_pred):
    ...
    try:
        with conn.connection.cursor() as cur:
            execute_logged(cur, accuracy_select, conn.sql_tracer, conn)
...
def rmse(data, label_true, label_pred):
    """
    Computes RMSE for regression results.
    ...
    """
    result = data.connection_context.sql("""
    SELECT ({0}-{1}) * ({0}-{1}) AS SQ_VAL FROM ({2})
    """.format(quotename(label_pred), quotename(label_true), data.select_statement))
    return math.sqrt(result.mean())

def mae(data, label_true, label_pred):
    """
    Computes MAE for regression results.
    ...
    """
    result = data.connection_context.sql("""
    SELECT ABS({0}-{1}) AS SQ_VAL FROM ({2})
    """.format(quotename(label_pred), quotename(label_true), data.select_statement))
    return result.mean()
...
def r2_score(data, label_true, label_pred):
    ...
    try:
        with conn.connection.cursor() as cur:
            execute_logged(cur, r2_select, conn.sql_tracer, conn)
...
def binary_classification_debriefing(data, label_true, label_pred, auc_data=None, positive_label=1, negative_label=0):
    """
    ...
    positive_label : str or int, optional
        ...
        Defaults to 1.
    negative_label : str or int, optional
    ...
    tp_sql = """select count(*) from ({0}) where "ACTUAL"='{1}' and "PREDICTED"='{1}'""".format(data_.select_statement, positive_label)
    fp_sql = """select count(*) from ({0}) where "ACTUAL"='{1}' and "PREDICTED"='{2}'""".format(data_.select_statement, negative_label, positive_label)
    tn_sql = """select count(*) from ({0}) where "ACTUAL"='{1}' and "PREDICTED"='{1}'""".format(data_.select_statement, negative_label)
    fn_sql = """select count(*) from ({0}) where "ACTUAL"='{1}' and "PREDICTED"='{2}'""".format(data_.select_statement, positive_label, negative_label)
    ...
    try:
        with conn.connection.cursor() as cur:
            execute_logged(cur, tp_sql, conn.sql_tracer, conn)
            tp = cur.fetchone()[0]
            execute_logged(cur, fp_sql, conn.sql_tracer, conn)
            fp = cur.fetchone()[0]
            execute_logged(cur, tn_sql, conn.sql_tracer, conn)
            tn = cur.fetchone()[0]
            execute_logged(cur, fn_sql, conn.sql_tracer, conn)
            fn = cur.fetchone()[0]
    except dbapi.Error as db_err:
        logger.exception(str(db_err))
        raise
    ...
    return {
        ...
        'FPR': fp / (fp + fn) if fp + fn != 0 else None,
        'FNR': fn / (fn + tp) if fn + tp != 0 else None,
        'F1': 2 * tp / (2 * tp + fp + fn) if 2 * tp + fp +fn !=0 else None,
        'MCC': (tp * tn - fp * fn) / math.sqrt((tp + fp) * (tp + fn) * (tn + fp) * (tn + fn)) if (tp + fp) * (tp + fn) * (tn + fp) * (tn + fn) != 0 else None,
        'KI': ki,
        'KAPPA': (obs_acc - exp_acc) / (1 - exp_acc) if 1 - exp_acc != 0 else None
    }
