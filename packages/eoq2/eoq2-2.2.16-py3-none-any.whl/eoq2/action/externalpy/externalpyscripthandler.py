...
class ExternalPyScriptHandler(CallHandler):
    ...
    def __ScanForActions(self):
        ...
        for root, dirs, files in os.walk(self._basedir, topdown=True):
            for file in files:
                if(file.endswith('.%s'%('py')) or file.endswith('.%s'%('pyc'))):
        ...
        for pythonfile in pythonfiles:
            pathname, filename = os.path.split(pythonfile)
            if(os.path.isfile(os.path.join(pathname,'__init__.py')) or os.path.isfile(os.path.join(pathname,'__init__.pyc')) or os.path.basename(pathname) == '__pycache__'):
                continue #skip any submodules from beeing loaded as an action
            if filename[-2:] == "py":
                actionfunctionname = filename[:-3] #without .py
            elif filename[-3:] == "pyc":
                actionfunctionname = filename[:-4] #without .pyc
