...
class S3FS(FS):
    ...
    :param bool strict: When ``True`` (default) S3FS will follow the
        ...
        downloads.
    :param bool make_dirs: When ``True`` (default) S3FS will follow the
        PyFilesystem specification exactly and add an empty file to emulate directories. 
        Set to ``False`` to disable all directory creations which may speed up uploads /
        downloads. Can be ``True`` only if ``strict`` is False too. 
    ...
    def __init__(
        ...
        if self.strict and not self.make_dirs:
            raise Exception("In strict mode, directory creation is mandatory, Set either `make_dirs` to True or `strict` to False")
