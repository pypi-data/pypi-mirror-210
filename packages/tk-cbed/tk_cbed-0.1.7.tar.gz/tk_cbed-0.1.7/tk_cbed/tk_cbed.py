...
def gen_dataset(x, batch_size):
    sh = x.shape
    dataset = tf.data.Dataset.from_generator(
                                Gen(x),
                                output_signature=(tf.TensorSpec(shape=(sh[1], sh[2], sh[3]), dtype=tf.float32)))
    dataset = dataset.repeat(1).batch(batch_size, drop_remainder=False)
    return dataset

def predict(model, x, output_shape, batch_size=16, verbose=2):
    # Determine the batch size and the number of batches
    ...
    if batch_size == x.shape[0]:
        ...
        if verbose == 2:
            with tqdm(total=num_batches, desc='batch number') as pbar:
                for ib, x_ib in enumerate(ds):
                    ib_0, ib_e = ib*batch_size, (ib+1)*batch_size
                    y[ib_0:ib_e, ...] = model.predict_on_batch(x_ib)
                    pbar.update(1)
        else:
            for ib, x_ib in enumerate(ds):
                ib_0, ib_e = ib*batch_size, (ib+1)*batch_size
...
class C_CBED(tf.keras.Model):
    ...
    def predict(self, x, batch_size=16, verbose=2):
        ...
        # Model prediction
        y = predict(self.base_model, x, (x.shape[0], 2), batch_size, verbose)
...
class C_CBED_NNI(tf.keras.Model):
    ...
    def predict(self, x, batch_size=16, verbose=2):
        ...
        # Model prediction
        y = predict(self.base_model, x, x.shape, batch_size, verbose)

        if self.crop_shape:
            return y
        else:
            # Removing extra row or column if added
            y = remove_extra_rows_or_columns(y, x_i_sh_e)

            # Adjusting output dimensions to match input dimensions
            return adjust_output_dimensions(y, x_i_sh)
