...
def save_multiprocess_vis(dem_path, vis, default, custom_dir=None, save_float=True, save_8bit=False,
    ...
    big rasters, 10000x10000 or bigger. Be careful if visualization="sky-view factor default" it looks up in default if calculate
    ...
    if mp.process.current_process().name == "MainProcess":
        ...
        # get output visualization raster paths for float and 8bit
        if vis != "sky-view factor default":  # single visualization function
            ...
            if save_8bit:
                create_blank_raster(in_data_set=data_set, out_raster_path=vis_8bit_path, nr_bands=nr_bands_8bit,
                                    e_type=1)
        else:  # visualization == "sky-view factor default", we can compute svf, asvf, pos_opns simultaneously
        ...
        else:  # visualization == "sky-view factor default"
            if save_svf:
                if save_float:
...
def get_block_offset(default, vis, res):
    """Returns offset for selected visualization (visualization). """
    if vis.lower() == "hillshade":
        ...
        raise Exception("rvt.multiproc.calculate_block_visualization: Wrong visualization parameter, visualization options:"
...
def calc_add_vis_block(save_float, save_8bit, blocks_to_save_float, blocks_to_save_8bit, block_array, default, x, y,
    ...
    if save_8bit:
        if vis == "multiple directions hillshade":  # 8bit multihillshade (calculating hillshade in 3 directions)
            vis_block_array_8bit = default.float_to_8bit(float_arr=block_array, visualization=vis, x_res=x_res, y_res=y_res)
            del block_array
            if right_offset == 0 and bottom_offset == 0:
            ...
            vis_block_array_8bit = default.float_to_8bit(float_arr=vis_block_array, visualization=vis)
...
def calc_add_svf_asvf_opns_block(save_float, save_8bit, blocks_to_save_svf_float, blocks_to_save_svf_8bit,
    ...
    if save_8bit:
        if save_svf:
            svf_block_arr_8bit = default.float_to_8bit(float_arr=svf_block_arr, visualization="sky-view factor")
            svf_block_8bit = RasterVisBlock(svf_block_arr_8bit, x, y)
            blocks_to_save_svf_8bit.append(svf_block_8bit)
        if save_asvf:
            asvf_block_arr_8bit = default.float_to_8bit(float_arr=asvf_block_arr, visualization="anisotropic sky-view factor")
            asvf_block_8bit = RasterVisBlock(asvf_block_arr_8bit, x, y)
            blocks_to_save_asvf_8bit.append(asvf_block_8bit)
        if save_pos_opns:
            pos_opns_block_arr_8bit = default.float_to_8bit(float_arr=pos_opns_block_arr, visualization="openness - positive")
