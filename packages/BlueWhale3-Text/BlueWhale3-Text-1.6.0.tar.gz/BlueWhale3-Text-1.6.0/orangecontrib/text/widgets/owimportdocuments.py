...
class OWImportDocuments(widget.OWWidget):
    ...
    class Outputs:
        data = Output("Corpus", Corpus, default=True, label=i18n.t("text.common.corpus"))
    ...
    def __init__(self):
        ...
        layout = QGridLayout()
        layout.setSpacing(4)
        gui.widgetBox(self.controlArea, orientation=layout, box=__("box.source"))
        source_box = gui.radioButtons(None, self, "source", box=True,
                                      callback=self.start, addToLayout=False)
        rb_button = gui.appendRadioButton(source_box, __("box.folder"),
                                          addToLayout=False)
        ...
        box = gui.hBox(self.controlArea, __("box.options"))
        gui.checkBox(box, self, "lemma_cb", __("checkbox.lemma"),
                     callback=self.commit)
        gui.checkBox(box, self, "pos_cb", __("checkbox.pos_tags"),
                     callback=self.commit)
        gui.checkBox(box, self, "ner_cb", __("checkbox.ner"),
                     callback=self.commit)
        self.controlArea.layout().addWidget(box)

        box = gui.vBox(self.controlArea, __("box.info"))
