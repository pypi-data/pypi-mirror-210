...
def __(key):
    return i18n.t('text.owkeywords.' + key)
...
class SelectionMethods:
    ...
    ITEMS = __("item.none"), __("item.all"), __("item.manual"), __("item.top_words")
...
class OWKeywords(OWWidget, ConcurrentWidgetMixin):
    ...
    class Outputs:
        words = Output("Words", Corpus, label=i18n.t("text.common.words"))

    class Warning(OWWidget.Warning):
        no_words_column = Msg(__("msg.no_words_column"))
    ...
    def _setup_gui(self):
        grid = QGridLayout()
        box = gui.widgetBox(self.controlArea, __("box.score_methods"), grid)
        ...
        box = gui.vBox(self.controlArea, __("box.agg"))
        gui.comboBox(
            box, self, "agg_method", items=AggregationMethods.ITEMS,
            callback=self.update_scores
        )

        box = gui.vBox(self.buttonsArea, __("box.select_word"))
        grid = QGridLayout()
        ...
        self.__filter_line_edit = QLineEdit(
            ...
            placeholderText=__("placeholder.filter")
        )
    ...
    def send_report(self):
        if not self.corpus:
            return
        self.report_data(__("report.corpus"), self.corpus)
        if self.words is not None:
            self.report_paragraph(__("report.words"), ", ".join(self.words))
        self.report_table(__("report.keywords"), self.view, num_format="{:.3f}")
