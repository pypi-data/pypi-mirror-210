...
class OWTopicModeling(OWWidget, ConcurrentWidgetMixin):
    ...
    class Outputs:
        corpus = Output("Corpus", Table, default=True, label=i18n.t("text.common.corpus"))
    ...
    def __init__(self):
        ...
        for i, (method, attr_name) in enumerate(self.methods):
            widget = method(self, title=__("box.option"))
        ...
        box = gui.vBox(self.controlArea, __("box.topic_evaluation"))
        gui.label(box, self, __("label.perplexity"))
        gui.label(box, self, __("label.coherence"))
...
class TopicViewer(QTreeWidget):
    ...
    columns = [__("label.topic"), __("label.topic_key")]
