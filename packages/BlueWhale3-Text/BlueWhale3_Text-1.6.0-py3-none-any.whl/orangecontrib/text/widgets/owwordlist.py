...
def __(key):
    return i18n.t('text.owwordlist.' + key)
...
class UpdateRules:
    ...
    ITEMS = [__("item.intersection"), __("item.union"), __("item.only_input"), __("item.ignore_input")]
...
class OWWordList(OWWidget):
    ...
    class Outputs:
        selected_words = Output("Selected Words", Table, label=i18n.t("text.common.select_word"))
        words = Output("Words", Table, label=i18n.t("text.common.words"))

    class Warning(OWWidget.Warning):
        no_string_vars = Msg(__("msg.no_string_vars"))
    ...
    def __init__(self):
        ...
        self.__library_box: QGroupBox = gui.vBox(None, __("box.library"))
        self.__input_box: QGroupBox = gui.vBox(None, __("box.input"))
    ...
    def _setup_library_box(self):
        ...
        action = QAction(__("btn.more"), self, toolTip=__("tip.more"))

        new_from_file = QAction(__("btn.import_file"), self)
        new_from_file.triggered.connect(self.__on_import_word_list)

        save_to_file = QAction(__("btn.save_file"), self)
    ...
    def __on_save_word_list(self):
        ...
        filename, _ = QFileDialog.getSaveFileName(
            self, __("btn.save_word"), filename,
            __("btn.open_type")
        )
        if filename:
    ...
    def _setup_input_box(self):
        gui.comboBox(
            self.__input_box, self, "words_var", label=__("label.word_variable"),
        ...
        gui.radioButtons(
            ...
            label=__("label.update"), orientation=Qt.Vertical,
    ...
    def _setup_words_box(self):
        ...
        action = QAction("+", self.words_view, toolTip=__("tip.add_word"))
        action.triggered.connect(self.__on_add_word)
        actions_widget.addAction(action)
        self.__add_word_action = action

        action = QAction("\N{MINUS SIGN}", self, toolTip=__("tip.remove_word"))
        ...
        action = QAction(__("btn.sort"), self)
        action.setToolTip(__("tip.sort"))
    ...
    def send_report(self):
        ...
        settings = [(__("report.library"), library)]
        if self.__input_words:
            self.report_data(__("report.input_words"), self.__input_words)
            settings.append((__("report.word_var"), self.words_var))
            rule = UpdateRules.ITEMS[self.update_rule_index]
            settings.append((__("report.update"), rule))
        self.report_items(__("report.settings"), settings)
        self.report_paragraph(__("report.words"), ", ".join(self.words_model[:]))
