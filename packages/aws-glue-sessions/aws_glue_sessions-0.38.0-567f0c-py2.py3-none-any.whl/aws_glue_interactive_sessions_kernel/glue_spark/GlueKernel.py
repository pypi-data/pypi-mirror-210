...
class GlueKernel(IPythonKernel):
    ...
    def display_result(self, statement_output):
        if 'Data' in statement_output:
            ...
            if MimeTypes.TextPlain.name in statement_output_data:
                ...
                if mime_type == MimeTypes.S3URI.value:
                    ...
                    if result.get(MimeTypes.ImagePng.value):
                ...
                if 'StdOut' in statement_output:
                    self._send_output(statement_output_data['StdOut'])
                if 'StdErr' in statement_output:
                    self._send_output(statement_output_data['StdErr'])
    ...
    def get_results(self, s3_path):
        ...
        try:
            ...
            bucket, key = self.split_s3_path(s3_path)
