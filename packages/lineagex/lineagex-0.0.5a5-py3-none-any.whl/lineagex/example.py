...
class example:
    def __init__(self, case: str = ""):
        ...
        if case == "github_example":
            ...
            t_noconn = lineagex(sql=os.path.join(cwd, 'examples', 'mimic-iv'), target_schema="mimiciv_derived", search_path_schema="mimiciv_icu, mimiciv_hosp")
            print("{} finished, please check in the folder for output.json and index.html".format(case))
        elif case == "mimic-iii":
            t_noconn = lineagex(sql=os.path.join(cwd, 'examples', 'mimic-iii'), target_schema="mimiciv_derived", search_path_schema="mimiciii_clinical, public")
            print("{} finished, please check in the folder for output.json and index.html".format(case))
