...
class MysqlPlatForm(object):
    ...
    def sync_autotest_script(self, testname, caseid, priority, caseurl):
        if DataProcess.isNotNull(get_system_key(Constant.GIT_PROJECTNAME)):
            ...
            if DataProcess.isNotNull(scripturl):
                ...
                _sql = f"INSERT INTO `traffic_test`.`test_autotest_script`(`gitname`, `suitname`, `tagversion`, `testid`, `priority`, `testname`, `scriptclass`, `scriptmethod`, `scriptname`, `scriptpackage`, `status`, `updatetime`, `caseurl`, `scripturl`) " \
                       f"VALUES ('{gitname}', '', 'run', '{caseid}', '{priority}','{testname}', '{scriptclass}', '{scriptmethod}', '{scriptname}', '', '0', '{_date}', '{caseurl}', '{scripturl}')"
                _mysql.execute(_sql)
                _mysql.conn.commit()
                _mysql.close()
    ...
    def sync_projectConfig(self, gitName):
        _info = []
        _sql = f"select count(*) as _count from test_autotest_script where gitname='{gitName}'"
        try:
            _config = {'key': 'atf'}
            _mysql = MysqlDB(_config)
            _info = _mysql.execute(_sql).fetchall()
            _sql2 = f"select count(*) as _count from test_autotest_script where gitname='{gitName}' and priority='P0'"
            _info1 = _mysql.execute(_sql2).fetchall()
            _sql = f"update system_global_conf set case_count={_info[0]['_count']}, p_case_count={_info1[0]['_count']} where git_project_name='{gitName}'"
            _mysql = MysqlDB(_config)
            _mysql.execute(_sql)
            _mysql.conn.commit()
            _mysql.close()
    ...
    @classmethod
    def delete_autotest_script(self, gitName):
        _info = []
