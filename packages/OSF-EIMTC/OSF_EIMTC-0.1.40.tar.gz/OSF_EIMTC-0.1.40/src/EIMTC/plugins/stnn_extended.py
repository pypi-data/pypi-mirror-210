from EIMTC.plugins import stnn,clump_flows
import pandas  as pd
import numpy as np
...
class STNN_Extended():
    ...
    def plugins(n_packets=32):
        return clump_flows.Clump_Flow(n_packets),stnn.STNN(n_packets)

    @staticmethod
    def new_stnn_feat(row):
    ...
    def preprocess(dataframe: pd.DataFrame):
        ...
        '''
        # validate
        clump_flows.Clump_Flow.preprocess(dataframe)
        stnn.STNN.preprocess(dataframe)

        dataframe['udps.new_stnn'] = dataframe.apply(STNN_Extended.new_stnn_feat,axis=1)
