...
class QuantitativeReport:
    ...
    def get_fund_industry_change(self):
        ...
        fund_industry_up = fund_industry_up.T.reset_index().T.reset_index().replace({'index': '', 1: '第一名', 2: '第二名', 3: '第三名', 4: '第四名', 5: '第五名'})
        fund_industry_down.columns = [sorted(date_list * 2)[len(sorted(date_list * 2)) - 1 - i] for i, date in enumerate(sorted(date_list * 2))]
        fund_industry_down = fund_industry_down.T.reset_index().T.reset_index().replace({'index': '', 1: '第一名', 2: '第二名', 3: '第三名', 4: '第四名', 5: '第五名'})
    ...
    def get_fund_turnover(self):
        ...
        if len(self.compare_fund_code_list) > 0:
            ...
            compare_turnover['TURNOVER'] = compare_turnover['TURNOVER'] * 2.0
    ...
    def get_quantitative_report(self):
        ...
        # document.add_paragraph(text='数据来源：好买基金研究中心').alignment = WD_PARAGRAPH_ALIGNMENT.CENTER
        # document.add_paragraph(text=' ')
        # document.add_picture('{0}{1}_shift.png'.format(self.file_path, self.target_fund_code), width=Cm(15), height=Cm(8))
        # document.add_paragraph(text='数据来源：好买基金研究中心').alignment = WD_PARAGRAPH_ALIGNMENT.CENTER
        ...
        # document.add_paragraph(text='数据来源：好买基金研究中心').alignment = WD_PARAGRAPH_ALIGNMENT.CENTER
        # document.add_paragraph(text=' ')
        # document.add_picture('{0}{1}_cen.png'.format(self.file_path, self.target_fund_code), width=Cm(15), height=Cm(8))
        # document.add_paragraph(text='数据来源：好买基金研究中心').alignment = WD_PARAGRAPH_ALIGNMENT.CENTER
        ...
        # add_hyperlink(p, 'D:/Git/hbshare/hbshare/fe/xwq/data/fund_report_external/{0}/持仓情况.xlsx'.format(self.target_fund_code), '持仓情况。', '#3FAEEE', True)
        ...
        document.add_picture('{0}{1}_brinson_attribution.png'.format(self.file_path, self.target_fund_code), width=Cm(18), height=Cm(10))
        document.add_paragraph(text='数据来源：好买基金研究中心').alignment = WD_PARAGRAPH_ALIGNMENT.CENTER
        ...
        document.add_paragraph(text=fund_summary)
        fund_summary = '9. 绩效归因：从Brinson绩效归因看，该产品最新一期{0}；从Barra风格归因看，该产品最新一期{1}'.format(fund_brinson_attribution_text.split('从最新一期看，')[1].split('。')[0], fund_barra_text.split('如下，')[1])
        document.add_paragraph(text=fund_summary)
...
if __name__ == "__main__":
    file_path = 'D:/Git/hbshare/hbshare/fe/xwq/data/fund_report_external/'
    for fund_id in ['001476', '001856', '270028']:
        target_fund_code = fund_id
