...
class QuantitativeReport:
    ...
    def get_fund_scale(self):
        ...
        ax2 = ax1.twinx()
        ax1.plot(scale['报告日期'].values, scale['规模（亿元）/份额（亿份）'].values, label='规模', color='#F04950')
        ax1.plot(share['报告日期'].values, share['规模（亿元）/份额（亿份）'].values, label='份额', color='#6268A2')
        ...
        plt.legend(handles=h1 + h2, labels=l1 + l2, loc=8, bbox_to_anchor=(0.5, -0.2), ncol=4)
        ax1.set_xticklabels(labels=fund_scale['报告日期'], rotation=45)
        ax1.set_xlabel('')
        ax2.set_xlabel('')
        ax1.set_ylabel('规模（亿元）/份额（亿份）')
        plt.tight_layout()
    ...
    def get_fund_holder(self):
        ...
        fund_holder = fund_holder.fillna(0.0)
    ...
    def get_fund_manager_method(self):
        ...
        hbs_rank_history = hbs_rank_history[hbs_rank_history['jsrq'].str.slice(4, 6).isin(['06', '12'])]
        hbs_rank_history = hbs_rank_history.sort_values('jsrq').iloc[-6:]
        ...
        hbs_abs_history = hbs_abs_history[hbs_abs_history['jsrq'].str.slice(4, 6).isin(['06', '12'])]
        hbs_abs_history = hbs_abs_history.sort_values('jsrq').iloc[-6:]
        ...
        for i in range(2):
            for j in range(2):
                axij = ax[i][j]
                axij2 = ax[i][j].twinx()
                axij.plot(shift_ts['报告日期'], shift_ts[pic_list[i][j]], label=pic_list[i][j], color='#F04950')
                axij2.plot(shift_abs_ts['报告日期'], shift_abs_ts[pic_list[i][j][:5]], label=pic_list[i][j][:5] + '（右轴）', color='#6268A2')
                h1, l1 = axij.get_legend_handles_labels()
                h2, l2 = axij2.get_legend_handles_labels()
                plt.legend(handles=h1 + h2, labels=l1 + l2, loc=8, bbox_to_anchor=(0.5, -0.2), ncol=2)
                axij.set_xticklabels(labels=shift_ts['报告日期'], rotation=45)
                axij.set_ylim([0.0, 1.0])
                axij2.set_ylim([0.0, 1.0])
                axij.hlines(y=0.5, xmin=0.0, xmax=5.0, linestyles='dashed', color='#959595')
        ...
        for i in range(2):
            for j in range(2):
                axij = ax[i][j]
                axij2 = ax[i][j].twinx()
                axij.plot(cen_ts['报告日期'], cen_ts[pic_list[i][j]], label=pic_list[i][j], color='#F04950')
                axij2.plot(cen_abs_ts['报告日期'], cen_abs_ts[pic_list[i][j][:5]], label=pic_list[i][j][:5] + '（右轴）', color='#6268A2')
                h1, l1 = axij.get_legend_handles_labels()
                h2, l2 = axij2.get_legend_handles_labels()
                plt.legend(handles=h1 + h2, labels=l1 + l2, loc=8, bbox_to_anchor=(0.5, -0.2), ncol=2)
                axij.set_xticklabels(labels=cen_ts['报告日期'], rotation=45)
                axij.set_ylim([0.0, 1.0])
                axij2.set_ylim([0.0, 1.0])
                axij.hlines(y=0.5, xmin=0.0, xmax=5.0, linestyles='dashed', color='#959595')
                axij.vlines(x=change_dates_loc, ymin=0.0, ymax=1.0, linestyles='dashed', color='#959595')
        ...
        plt.legend(loc=8, bbox_to_anchor=(0.5, -0.2), ncol=2)
        ax.set_xticklabels(labels=industry_stock_cen_ts['报告日期'], rotation=45)
        ax.set_ylim([0.0, 1.0])
        ax.hlines(y=0.5, xmin=0.0, xmax=5.0, linestyles='dashed', color='#959595')
    ...
    def get_fund_size(self):
        ...
        fund_size_ts = fund_size_ts[fund_size_ts['REPORT_DATE'].str.slice(4, 6).isin(['06', '12'])]
        fund_size_ts = fund_size_ts[['REPORT_DATE', 'SIZE_TYPE', 'MV_IN_NA']]
        fund_size_ts.columns = ['报告日期', '规模类型', '规模暴露（%）']
        ...
        fig, ax = plt.subplots(figsize=(12, 6))
        sns.barplot(ax=ax, x='报告日期', y='规模暴露（%）', data=fund_size_ts, hue='规模类型', hue_order=['大盘', '中盘', '小盘'], palette=['#C94649', '#8588B7', '#7D7D7E'])
        ax.vlines(x=change_dates_loc, ymin=0.0, ymax=max(fund_size_ts['规模暴露（%）']), linestyles='dashed', color='#959595')
        ax.set_xticklabels(labels=sorted(fund_size_ts['报告日期'].unique().tolist()), rotation=45)
        ...
        if len(self.compare_fund_code_list) > 0:
            ...
            compare_size.columns = ['大盘绝对暴露', '中小盘绝对暴露', '大盘暴露排名', '中小盘暴露排名']
    ...
    def get_fund_style(self):
        ...
        fund_style_ts = fund_style_ts[fund_style_ts['REPORT_DATE'].str.slice(4, 6).isin(['06', '12'])]
        fund_style_ts = fund_style_ts[['REPORT_DATE', 'STYLE_TYPE', 'MV_IN_NA']]
        fund_style_ts.columns = ['报告日期', '风格类型', '风格暴露（%）']
        ...
        fig, ax = plt.subplots(figsize=(12, 6))
        sns.barplot(ax=ax, x='报告日期', y='风格暴露（%）', data=fund_style_ts, hue='风格类型', hue_order=style_list, palette=['#C94649', '#8588B7'])
        ax.vlines(x=change_dates_loc, ymin=0.0, ymax=max(fund_style_ts['风格暴露（%）']), linestyles='dashed', color='#959595')
        ax.set_xticklabels(labels=sorted(fund_style_ts['报告日期'].unique().tolist()), rotation=45)
        plt.legend(loc=8, bbox_to_anchor=(0.5, -0.2), ncol=2)
        plt.gca().yaxis.set_major_formatter(FuncFormatter(to_percent))
        plt.xlabel('')
        plt.ylabel('风格暴露')
    ...
    def get_fund_theme(self):
        ...
        fund_theme_ts['REPORT_DATE'] =fund_theme_ts['REPORT_DATE'].astype(str)
        fund_theme_ts['THEME'] = fund_theme_ts['THEME'].replace({'TMT': 'TMT', 'ZZ': '制造', 'ZQ': '周期', 'DJR': '大金融', 'XF': '消费', 'QT': '其他'})
        fund_theme_ts = fund_theme_ts[fund_theme_ts['IS_ZC'] == '2']
        fund_theme_ts = fund_theme_ts.pivot(index='REPORT_DATE', columns='THEME', values='MV_IN_NA').reset_index().fillna(0.0)
        theme_list = [theme for theme in theme_list + ['其他'] if theme in list(fund_theme_ts.columns)]
        for i in range(1, len(theme_list)):
            fund_theme_ts[theme_list[i]] = fund_theme_ts[theme_list[i]] + fund_theme_ts[theme_list[i - 1]]
        for theme in theme_list:
            fund_theme_ts[theme] = fund_theme_ts[theme] / fund_theme_ts[theme_list[-1]]
        fund_theme_ts = fund_theme_ts[fund_theme_ts['REPORT_DATE'].str.slice(4, 6).isin(['06', '12'])]
        fund_theme_ts = fund_theme_ts.sort_values('REPORT_DATE').iloc[-6:]
        ...
        fig, ax = plt.subplots(figsize=(12, 6))
        color_list = [bar_color_list[0], bar_color_list[1], bar_color_list[7], bar_color_list[8], bar_color_list[14], bar_color_list[15]]
        ...
        plt.legend(handles=h[::-1], labels=l[::-1], loc=8, bbox_to_anchor=(0.5, -0.2), ncol=6)
    ...
    def get_fund_industry(self):
        ...
        for industry in industry_ts_list:
            fund_industry_ts[industry] = fund_industry_ts[industry] / fund_industry_ts[industry_ts_list[-1]]
        fund_industry_ts = fund_industry_ts[fund_industry_ts['REPORT_DATE'].str.slice(4, 6).isin(['06', '12'])]
        fund_industry_ts = fund_industry_ts.sort_values('REPORT_DATE').iloc[-6:]
        ...
        h, l = ax.get_legend_handles_labels()
        plt.legend(handles=h[::-1], labels=l[::-1], loc=8, bbox_to_anchor=(0.5, -0.4), ncol=8)
        plt.gca().yaxis.set_major_formatter(FuncFormatter(to_percent))
        plt.xticks(rotation=45)
        plt.xlabel('')
        plt.ylabel('行业占比')
    ...
    def get_fund_finance(self):
        ...
        for i in range(2):
            for j in range(2):
                ...
                axij.set_xticklabels(labels=fund_finance['报告日期'], rotation=45)
                axij.legend(loc=8, bbox_to_anchor=(0.5, -0.2), ncol=2)
                if pic_list[i][j] == 'ROE' or pic_list[i][j] == '股息率':
                    axij.yaxis.set_major_formatter(FuncFormatter(to_percent_r2))
                axij.set_xlabel('')
                axij.set_ylabel(pic_list[i][j])
                sns.despine(left=False, bottom=False, top=True, right=True)
        plt.tight_layout()
    ...
    def get_fund_stock_compare(self):
        ...
        fund_holding_indu = pd.concat(fund_holding_indu_list).drop_duplicates()
    ...
    def get_fund_performace(self):
        ...
        if len(self.compare_fund_code_list) > 0:
            ...
            ax.yaxis.set_major_formatter(FuncFormatter(to_100percent))
            plt.xticks(rotation=45)
            plt.legend(loc=8, bbox_to_anchor=(0.5, -0.3), ncol=len(self.compare_fund_code_list) + 1)
            plt.xlabel('')
            plt.ylabel('累计收益率')
            plt.tight_layout()
            sns.despine(left=False, bottom=False, top=True, right=True)
    ...
    def get_fund_achievement(self):
        ...
        if len(self.compare_fund_code_list) > 0:
            ...
            for idx, col in enumerate(list(compare_nav.columns)):
                ...
                compare_achievement.loc['年化收益率', idx] = (col_nav_df.iloc[-1] / col_nav_df.iloc[0]) ** (250.0 / len(col_nav_df)) - 1.0
    ...
    def get_fund_brinson_attribution(self):
        fund_brinson_attribution = HBDB().read_fund_brinson_attribution_given_code(self.target_fund_code)
        fund_brinson_attribution = fund_brinson_attribution.rename(columns={'jjdm': 'FUND_CODE', 'tjrq': 'REPORT_DATE', 'asset_allo': '大类资产配置收益', 'sector_allo': '行业配置收益', 'equity_selection': '行业选股收益', 'trading': '交易收益'})
        fund_brinson_attribution['REPORT_DATE'] = fund_brinson_attribution['REPORT_DATE'].apply(lambda x: str(x)[:6] + '30' if str(x)[4:6] == '06' else str(x)[:6] + '31')
        fund_brinson_attribution = fund_brinson_attribution.sort_values('REPORT_DATE')
        fund_brinson_attribution['总超额收益'] = fund_brinson_attribution['portfolio_return'] - fund_brinson_attribution['benchmark_return']
        attr_list = ['大类资产配置收益', '行业配置收益', '行业选股收益', '交易收益']
        fund_brinson_attribution = fund_brinson_attribution[['REPORT_DATE', '总超额收益'] + attr_list]
        fund_brinson_attribution.columns = ['报告日期', '总超额收益'] + attr_list
        fund_brinson_attribution_disp = fund_brinson_attribution.copy(deep=True)
        for i in range(1, len(attr_list)):
            fund_brinson_attribution_disp[attr_list[i]] = fund_brinson_attribution_disp[attr_list[i]] + fund_brinson_attribution_disp[attr_list[i - 1]]
        fund_brinson_attribution_disp = fund_brinson_attribution_disp.iloc[-12:]

        fund_brinson_attribution_disp['报告日期'] = fund_brinson_attribution_disp['报告日期'].astype(str)
        change_dates = [date for date in self.change_dates if date >= min(fund_brinson_attribution_disp['报告日期']) and date <= max(fund_brinson_attribution_disp['报告日期'])]
        change_dates_loc = get_change_dates_loc(change_dates, sorted(fund_brinson_attribution_disp['报告日期'].unique().tolist()))
        fig, ax = plt.subplots(figsize=(12, 6))
        color_list = [bar_color_list[0], bar_color_list[1], bar_color_list[7], bar_color_list[14]]
        for i in range(len(attr_list) - 1, -1, -1):
            sns.barplot(ax=ax, x='报告日期', y=attr_list[i], data=fund_brinson_attribution_disp, label=attr_list[i], color=color_list[i])
        sns.lineplot(ax=ax, x='报告日期', y='总超额收益', data=fund_brinson_attribution_disp, label='总超额收益', color='#F04950')
        ax.vlines(x=change_dates_loc, ymin=min([min(fund_brinson_attribution_disp[col]) for col in attr_list + ['总超额收益']]), ymax=max([max(fund_brinson_attribution_disp[col]) for col in attr_list + ['总超额收益']]), linestyles='dashed', color='#959595')
        ax.yaxis.set_major_formatter(FuncFormatter(to_100percent))
        plt.legend(loc=8, bbox_to_anchor=(0.5, -0.3), ncol=5)
        plt.xlabel('')
        plt.xticks(rotation=45)
        plt.ylabel('Brinson归因')
        plt.tight_layout()
        sns.despine(left=False, bottom=False, top=True, right=True)
        plt.savefig('{0}{1}_brinson_attribution.png'.format(self.file_path, self.target_fund_code))

        max_date = max(fund_brinson_attribution['报告日期'])
        fund_brinson_attribution_latest = fund_brinson_attribution[fund_brinson_attribution['报告日期'] == max_date].set_index('报告日期')
        excess_return = '{0}%'.format(round(fund_brinson_attribution_latest['总超额收益'].values[0] * 100.0, 2))
        fund_brinson_attribution_latest = fund_brinson_attribution_latest.drop('总超额收益', axis=1).T
        fund_brinson_attribution_lateste_sorted = fund_brinson_attribution_latest.sort_values(max_date, ascending=False)
        fund_brinson_attribution_latest[max_date] = fund_brinson_attribution_latest[max_date].apply(lambda x: '{0}%'.format(round(x * 100.0, 2)))
        fund_brinson_attribution_lateste_sorted[max_date] = fund_brinson_attribution_lateste_sorted[max_date].apply(lambda x: '{0}%'.format(round(x * 100.0, 2)))
        brinson_attribution_ret = '，'.join(fund_brinson_attribution_latest[max_date].unique().tolist())
        max_attribution = fund_brinson_attribution_lateste_sorted.index[0]

        fund_brinson_attribution_text = '该产品Brinson归因结果如下，从最新一期看，总超额收益为{0}，大类资产配置收益、行业配置收益、行业选股收益、交易收益分别为{1}，其中{2}贡献最大。'.format(excess_return, brinson_attribution_ret, max_attribution)
        return fund_brinson_attribution_text
    ...
    def get_quantitative_report(self):
        ...
        run_head_1_2 = head_1_2.add_run('2. 规模变动')
        ...
        document.add_picture('{0}{1}_scale.png'.format(self.file_path, self.target_fund_code), width=Cm(15), height=Cm(8))
        document.add_paragraph(text='数据来源：好买基金研究中心').alignment = WD_PARAGRAPH_ALIGNMENT.CENTER
        ...
        run_head_1_3 = head_1_3.add_run('3. 持有人结构')
        ...
        document.add_picture('{0}{1}_holder.png'.format(self.file_path, self.target_fund_code), width=Cm(15), height=Cm(8))
        document.add_paragraph(text='数据来源：好买基金研究中心').alignment = WD_PARAGRAPH_ALIGNMENT.CENTER
        if len(self.compare_fund_code_list) > 0:
        ...
        head_2_1_1 = document.add_heading(text='', level=3)
        run_head_2_1_1 = head_2_1_1.add_run('1.1 换手率水平')
        run_head_2_1_1.font.color.rgb = RGBColor(201, 70, 73)
        ...
        document.add_paragraph(text='数据来源：好买基金研究中心').alignment = WD_PARAGRAPH_ALIGNMENT.CENTER
        document.add_paragraph(text=' ')
        document.add_picture('{0}{1}_shift.png'.format(self.file_path, self.target_fund_code), width=Cm(15), height=Cm(15))
        document.add_paragraph(text='数据来源：好买基金研究中心').alignment = WD_PARAGRAPH_ALIGNMENT.CENTER
        ...
        head_2_1_2 = document.add_heading(text='', level=3)
        run_head_2_1_2 = head_2_1_2.add_run('1.2 集中度水平')
        run_head_2_1_2.font.color.rgb = RGBColor(201, 70, 73)
        ...
        document.add_paragraph(text='数据来源：好买基金研究中心').alignment = WD_PARAGRAPH_ALIGNMENT.CENTER
        document.add_paragraph(text=' ')
        document.add_picture('{0}{1}_cen.png'.format(self.file_path, self.target_fund_code), width=Cm(15), height=Cm(15))
        document.add_paragraph(text='数据来源：好买基金研究中心').alignment = WD_PARAGRAPH_ALIGNMENT.CENTER
        ...
        head_2_1_3 = document.add_heading(text='', level=3)
        run_head_2_1_3 = head_2_1_3.add_run('1.3 选股方法')
        run_head_2_1_3.font.color.rgb = RGBColor(201, 70, 73)
        ...
        document.add_paragraph(text='数据来源：好买基金研究中心').alignment = WD_PARAGRAPH_ALIGNMENT.CENTER
        document.add_paragraph(text=' ')
        document.add_picture('{0}{1}_indu_stock_cen.png'.format(self.file_path, self.target_fund_code), width=Cm(15), height=Cm(8))
        document.add_paragraph(text='数据来源：好买基金研究中心').alignment = WD_PARAGRAPH_ALIGNMENT.CENTER
        ...
        head_2_1_4 = document.add_heading(text='', level=3)
        run_head_2_1_4 = head_2_1_4.add_run('1.4 持仓周期')
        run_head_2_1_4.font.color.rgb = RGBColor(201, 70, 73)
        ...
        head_2_1_5 = document.add_heading(text='', level=3)
        run_head_2_1_5 = head_2_1_5.add_run('1.5 左侧特征')
        run_head_2_1_5.font.color.rgb = RGBColor(201, 70, 73)
        ...
        head_2_1_6 = document.add_heading(text='', level=3)
        run_head_2_1_6 = head_2_1_6.add_run('1.6 新股/次新股偏好')
        run_head_2_1_6.font.color.rgb = RGBColor(201, 70, 73)
        ...
        run_head_2_2 = head_2_2.add_run('2. 基金定量分析')
        run_head_2_2.font.color.rgb = RGBColor(201, 70, 73)
        head_2_2_1 = document.add_heading(text='', level=3)
        run_head_2_2_1 = head_2_2_1.add_run('2.1 仓位分析')
        run_head_2_2_1.font.color.rgb = RGBColor(201, 70, 73)
        fund_position_text, compare_position = self.get_fund_position()
        document.add_paragraph(text=fund_position_text)
        document.add_picture('{0}{1}_position.png'.format(self.file_path, self.target_fund_code), width=Cm(15), height=Cm(8))
        document.add_paragraph(text='数据来源：好买基金研究中心').alignment = WD_PARAGRAPH_ALIGNMENT.CENTER
        if len(self.compare_fund_code_list) > 0:
        ...
        head_2_2_2 = document.add_heading(text='', level=3)
        run_head_2_2_2 = head_2_2_2.add_run('2.2 规模分析')
        run_head_2_2_2.font.color.rgb = RGBColor(201, 70, 73)
        ...
        document.add_paragraph(text='数据来源：好买基金研究中心').alignment = WD_PARAGRAPH_ALIGNMENT.CENTER
        document.add_paragraph(text=' ')
        document.add_picture('{0}{1}_size.png'.format(self.file_path, self.target_fund_code), width=Cm(15), height=Cm(8))
        document.add_paragraph(text='数据来源：好买基金研究中心').alignment = WD_PARAGRAPH_ALIGNMENT.CENTER
        ...
        head_2_2_3 = document.add_heading(text='', level=3)
        run_head_2_2_3 = head_2_2_3.add_run('2.3 风格分析')
        run_head_2_2_3.font.color.rgb = RGBColor(201, 70, 73)
        ...
        document.add_paragraph(text='数据来源：好买基金研究中心').alignment = WD_PARAGRAPH_ALIGNMENT.CENTER
        document.add_paragraph(text=' ')
        document.add_picture('{0}{1}_style.png'.format(self.file_path, self.target_fund_code), width=Cm(15), height=Cm(8))
        document.add_paragraph(text='数据来源：好买基金研究中心').alignment = WD_PARAGRAPH_ALIGNMENT.CENTER
        ...
        head_2_2_4 = document.add_heading(text='', level=3)
        run_head_2_2_4 = head_2_2_4.add_run('2.4 主题分析')
        run_head_2_2_4.font.color.rgb = RGBColor(201, 70, 73)
        ...
        head_2_2_5 = document.add_heading(text='', level=3)
        run_head_2_2_5 = head_2_2_5.add_run('2.5 行业分析')
        run_head_2_2_5.font.color.rgb = RGBColor(201, 70, 73)
        ...
        rowc, colc = fund_industry_up.shape[0], fund_industry_up.shape[1]
        table = document.add_table(rows=rowc, cols=colc, style='Medium Shading 1 Accent 2')
        for i in range(rowc):
            for j in range(colc):
        ...
        rowc, colc = fund_industry_down.shape[0], fund_industry_down.shape[1]
        table = document.add_table(rows=rowc, cols=colc, style='Medium Shading 1 Accent 2')
        for i in range(rowc):
            for j in range(colc):
        ...
        head_2_2_6 = document.add_heading(text='', level=3)
        run_head_2_2_6 = head_2_2_6.add_run('2.6 持股分析')
        run_head_2_2_6.font.color.rgb = RGBColor(201, 70, 73)
        fund_cr_text = self.get_fund_cr()
        document.add_paragraph(text=fund_cr_text)
        document.add_picture('{0}{1}_cr5.png'.format(self.file_path, self.target_fund_code), width=Cm(15), height=Cm(8))
        document.add_paragraph(text='数据来源：好买基金研究中心').alignment = WD_PARAGRAPH_ALIGNMENT.CENTER
        document.add_picture('{0}{1}_cr10.png'.format(self.file_path, self.target_fund_code), width=Cm(15), height=Cm(8))
        document.add_paragraph(text='数据来源：好买基金研究中心').alignment = WD_PARAGRAPH_ALIGNMENT.CENTER
        fund_finance_text, compare_finance = self.get_fund_finance()
        document.add_paragraph(text=fund_finance_text)
        document.add_picture('{0}{1}_finance.png'.format(self.file_path, self.target_fund_code), width=Cm(15), height=Cm(15))
        ...
        document.add_picture('{0}{1}_dist.png'.format(self.file_path, self.target_fund_code), width=Cm(15), height=Cm(8))
        document.add_paragraph(text='数据来源：好买基金研究中心').alignment = WD_PARAGRAPH_ALIGNMENT.CENTER
        ...
        if len(self.compare_fund_code_list) > 0:
            ...
            add_hyperlink(p, 'D:/Git/hbshare/hbshare/fe/xwq/data/fund_report/{0}/持仓情况.xlsx'.format(self.target_fund_code), '持仓情况。', '#3FAEEE', True)

        head_2_2_7 = document.add_heading(text='', level=3)
        run_head_2_2_7 = head_2_2_7.add_run('2.7 换手率分析')
        run_head_2_2_7.font.color.rgb = RGBColor(201, 70, 73)
        fund_turnover_text, compare_turnover = self.get_fund_turnover()
        document.add_paragraph(text=fund_turnover_text)
        document.add_picture('{0}{1}_turnover.png'.format(self.file_path, self.target_fund_code), width=Cm(15), height=Cm(8))
        document.add_paragraph(text='数据来源：好买基金研究中心').alignment = WD_PARAGRAPH_ALIGNMENT.CENTER
        if len(self.compare_fund_code_list) > 0:
            document.add_paragraph(text='与同类型基金换手率比较情况如下：')
        ...
        document.add_picture('{0}{1}_return.png'.format(self.file_path, self.target_fund_code), width=Cm(15), height=Cm(8))
        document.add_paragraph(text='数据来源：好买基金研究中心').alignment = WD_PARAGRAPH_ALIGNMENT.CENTER
        document.add_picture('{0}{1}_drawdown.png'.format(self.file_path, self.target_fund_code), width=Cm(15), height=Cm(8))
        document.add_paragraph(text='数据来源：好买基金研究中心').alignment = WD_PARAGRAPH_ALIGNMENT.CENTER
        ...
        if len(self.compare_fund_code_list) > 0:
            ...
            document.add_picture('{0}{1}_compare_return.png'.format(self.file_path, self.target_fund_code), width=Cm(15), height=Cm(8))
            document.add_paragraph(text='数据来源：好买基金研究中心').alignment = WD_PARAGRAPH_ALIGNMENT.CENTER
        ...
        fund_brinson_attribution_text = self.get_fund_brinson_attribution()
        document.add_paragraph(text=fund_brinson_attribution_text, style=None)
        document.add_picture('{0}{1}_brinson_attribution.png'.format(self.file_path, self.target_fund_code), width=Cm(15), height=Cm(8))
        document.add_paragraph(text='数据来源：好买基金研究中心').alignment = WD_PARAGRAPH_ALIGNMENT.CENTER
        fund_barra_text = self.get_fund_barra_attribution()
        document.add_paragraph(text=fund_barra_text, style=None)
        document.add_picture('{0}{1}_barra_exposure.png'.format(self.file_path, self.target_fund_code), width=Cm(15), height=Cm(8))
        document.add_paragraph(text='数据来源：好买基金研究中心').alignment = WD_PARAGRAPH_ALIGNMENT.CENTER
        document.add_picture('{0}{1}_barra_return.png'.format(self.file_path, self.target_fund_code), width=Cm(15), height=Cm(8))
        document.add_paragraph(text='数据来源：好买基金研究中心').alignment = WD_PARAGRAPH_ALIGNMENT.CENTER
        ...
        document.add_paragraph(text=fund_summary)
        fund_summary = '3. 投资理念：该产品{0}，{1}，{2}，{3}（{4}），{5}，{6}。'.format(fund_manager_method_shift_text.split('该产品')[1].split('，规模')[0],
        ...
        document.add_paragraph(text=fund_summary)
        fund_summary = '4. 仓位：近三年股票平均仓位为{0}。'.format(fund_position_text.split('其中股票资产近三年平均配置比例为')[1].split('，最新一期')[0])
        document.add_paragraph(text=fund_summary)
        fund_summary = '5. 投资风格：从产品规模看，该产品属于{0}从产品风格看，该产品属于{1}'.format(fund_size_text.split('属于')[1], fund_style_text.split('属于')[1])
        document.add_paragraph(text=fund_summary)
        fund_summary = '6. 主题配置：该产品属于{0}'.format(fund_theme_text.split('属于')[1])
        document.add_paragraph(text=fund_summary)
        fund_summary = '7. 行业配置：该产品属于{0}'.format(fund_industry_text.split('属于')[1])
        document.add_paragraph(text=fund_summary)
        fund_summary = '8. 持股：该产品{0}，{1}。'.format(fund_cr_text.split('该产品持股集中度情况如下，')[1].split('。')[0],
                                                    fund_holding_disp_text.split('从概率密度分布图可以看出，该产品')[1].split('，近六期')[0])
        document.add_paragraph(text=fund_summary)
        fund_summary = '9. 换手率：该产品{0}'.format(fund_turnover_text.split('该产品换手率情况如下，')[1])
        document.add_paragraph(text=fund_summary)
        fund_summary = '10. 绩效归因：从Brinson绩效归因看，该产品最新一期{0}；从Barra风格归因看，该产品最新一期{1}'.format(fund_brinson_attribution_text.split('从最新一期看，')[1].split('。')[0], fund_barra_text.split('如下，')[1])
        document.add_paragraph(text=fund_summary)
...
def get_stock_industry():
    stock_industry = HBDB().read_stock_industry()
    stock_industry.to_hdf('D:/Git/hbshare/hbshare/fe/xwq/data/fund_report/stock_industry.hdf', key='table', mode='w')
    stock_industry = pd.read_hdf('D:/Git/hbshare/hbshare/fe/xwq/data/fund_report/stock_industry.hdf', key='table')
...
def get_contribution_pricemean(fund_code, stock_name_dict, fund_holding, stock_daily_k):
    # 获取期间收益数据
    stock_daily_k = stock_daily_k[stock_daily_k['TICKER_SYMBOL'].isin(list(fund_holding.columns))]
    stock_daily_k = stock_daily_k.pivot(index='TRADE_DATE', columns='TICKER_SYMBOL', values='CLOSE_PRICE')
    stock_daily_k = stock_daily_k.sort_index()
    stock_daily_k = stock_daily_k.replace(0.0, np.nan).fillna(method='ffill')
    stock_daily_k_mean = stock_daily_k.unstack().reset_index()
    stock_daily_k_mean.columns = ['TICKER_SYMBOL', 'TRADE_DATE', 'CLOSE_PRICE']
    stock_daily_k_mean['REPORT_DATE'] = stock_daily_k_mean['TRADE_DATE'].apply(lambda x: x[:4] + '0331' if x[4:6] <= '03' else x[:4] + '0630' if x[4:6] > '03' and x[4:6] <= '06' else x[:4] + '0930' if x[4:6] > '06' and x[4:6] <= '09' else x[:4] + '1231')
    stock_daily_k_mean = stock_daily_k_mean[['REPORT_DATE', 'TICKER_SYMBOL', 'CLOSE_PRICE']].groupby(['REPORT_DATE', 'TICKER_SYMBOL']).mean().reset_index().rename(columns={'CLOSE_PRICE': 'CLOSE_PRICE_MEAN'})
    stock_daily_k_mean = stock_daily_k_mean.pivot(index='REPORT_DATE', columns='TICKER_SYMBOL', values='CLOSE_PRICE_MEAN')
    stock_daily_k_comp = stock_daily_k_mean.pct_change().shift(-1).dropna(how='all')

    # 计算个股贡献
    stock_contribution = fund_holding * stock_daily_k_comp
    stock_contribution = stock_contribution.dropna(how='all')
    stock_contribution.rename(columns=stock_name_dict).to_excel('D:/Git/hbshare/hbshare/fe/xwq/data/fund_report/{0}/stock_contribution.xlsx'.format(fund_code))
    stock_contribution = stock_contribution.unstack().reset_index()
    stock_contribution.columns = ['TICKER_SYMBOL', 'REPORT_DATE', 'RET']
    stock_industry = get_stock_industry()
    stock_industry = stock_industry[stock_industry['INDUSTRY_TYPE'] == 1]
    stock_industry = stock_industry[stock_industry['IS_NEW'] == 1]
    stock_contribution = stock_contribution.merge(stock_industry[['TICKER_SYMBOL', 'INDUSTRY_NAME']], on=['TICKER_SYMBOL'], how='left')
    industry_contribution = stock_contribution[['REPORT_DATE', 'INDUSTRY_NAME', 'RET']].groupby(['REPORT_DATE', 'INDUSTRY_NAME']).sum().reset_index()
    industry_contribution = industry_contribution.pivot(index='REPORT_DATE', columns='INDUSTRY_NAME', values='RET')
    industry_contribution = industry_contribution.dropna(how='all')
    industry_contribution.to_excel('D:/Git/hbshare/hbshare/fe/xwq/data/fund_report/{0}/industry_contribution.xlsx'.format(fund_code))
    return

def get_contribution_positionmean(fund_code, stock_name_dict, fund_holding, stock_daily_k):
    # 获取期间收益数据
    stock_daily_k = stock_daily_k[stock_daily_k['TICKER_SYMBOL'].isin(list(fund_holding.columns))]
    stock_daily_k = stock_daily_k.pivot(index='TRADE_DATE', columns='TICKER_SYMBOL', values='CLOSE_PRICE')
    stock_daily_k = stock_daily_k.sort_index()
    stock_daily_k = stock_daily_k.replace(0.0, np.nan).fillna(method='ffill')
    stock_daily_k_end = stock_daily_k.reset_index()
    stock_daily_k_end['YEAR_MONTH'] = stock_daily_k_end['TRADE_DATE'].apply(lambda x: str(x)[:6])
    stock_daily_k_end['MONTH'] = stock_daily_k_end['TRADE_DATE'].apply(lambda x: str(x)[4:6])
    stock_daily_k_end = stock_daily_k_end.sort_values('TRADE_DATE').drop_duplicates('YEAR_MONTH', keep='last')
    stock_daily_k_end = stock_daily_k_end[stock_daily_k_end['MONTH'].isin(['03', '06', '09', '12'])]
    stock_daily_k_end['REPORT_DATE'] = stock_daily_k_end['YEAR_MONTH'].apply(lambda x: x + '31' if x[4:] =='03' or x[4:] =='12' else x + '30')
    stock_daily_k_end = stock_daily_k_end.drop(['TRADE_DATE', 'YEAR_MONTH', 'MONTH'], axis=1).set_index('REPORT_DATE')
    stock_daily_k_comp = stock_daily_k_end.pct_change().dropna(how='all')

    # 计算个股贡献
    fund_holding = fund_holding.rolling(2).mean()
    stock_contribution = fund_holding * stock_daily_k_comp
    stock_contribution = stock_contribution.dropna(how='all')
    stock_contribution.rename(columns=stock_name_dict).to_excel('D:/Git/hbshare/hbshare/fe/xwq/data/fund_report/{0}/stock_contribution.xlsx'.format(fund_code))
    stock_contribution = stock_contribution.unstack().reset_index()
...
def get_stock_contribution(fund_code, start_date='19000101', end_date='20990101'):
    # 获取基金持仓数据
    fund_holding = HBDB().read_fund_holding_given_codes([fund_code])
    fund_holding = fund_holding.rename(columns={'jjdm': 'FUND_CODE', 'jsrq': 'REPORT_DATE', 'zqdm': 'TICKER_SYMBOL', 'zqmc': 'SEC_SHORT_NAME', 'ccsz': 'HOLDING_MARKET_VALUE', 'ccsl': 'HOLDING_AMOUNT', 'zjbl': 'MV_IN_NA'})
    fund_holding['REPORT_DATE'] = fund_holding['REPORT_DATE'].astype(str)
    fund_holding = fund_holding.sort_values('REPORT_DATE', ascending=False)
    ...
    stock_name_dict = stock_name.set_index('TICKER_SYMBOL')['SEC_SHORT_NAME'].to_dict()
    fund_holding = fund_holding[(fund_holding['REPORT_DATE'] >= start_date) & (fund_holding['REPORT_DATE'] <= end_date)]
    fund_holding = fund_holding.pivot(index='REPORT_DATE', columns='TICKER_SYMBOL', values='MV_IN_NA')

    # 补全持仓
    fund_holding.loc[fund_holding.index.str.slice(4, 6).isin(['06', '12'])] = fund_holding.loc[fund_holding.index.str.slice(4, 6).isin(['06', '12'])].fillna(0.00)
    fund_holding_min = pd.DataFrame(fund_holding.min(axis=1), columns=['MV_IN_NA_MIN'])
    fund_holding = fund_holding.interpolate()
    fund_holding = fund_holding.merge(fund_holding_min, left_index=True, right_index=True, how='left')
    def replace_value(ser):
        ser[ser >= ser[-1]] = ser[-1]
        return ser
    fund_holding.loc[fund_holding.index.str.slice(4, 6).isin(['03', '09'])] = fund_holding.loc[fund_holding.index.str.slice(4, 6).isin(['03', '09'])].apply(lambda x: replace_value(x), axis=1)
    fund_holding = fund_holding.iloc[:-1, :].drop('MV_IN_NA_MIN', axis=1)

    # 日历数据
    calendar_df, report_df, trade_df, report_trade_df, calendar_trade_df = get_date('19000101', datetime.today().strftime('%Y%m%d'))

    # 个股行情数据
    stock_daily_k_path = 'D:/Git/hbshare/hbshare/fe/xwq/data/fund_report/stock_daily_k.hdf'
    if os.path.isfile(stock_daily_k_path):
        ...
        start_date = '20150101'
    trade_df = trade_df[(trade_df['TRADE_DATE'] > start_date) & (trade_df['TRADE_DATE'] < datetime.today().strftime('%Y%m%d'))]
    data = []
    ...
    stock_daily_k = pd.concat([existed_stock_daily_k] + data, ignore_index=True)
    stock_daily_k = stock_daily_k.sort_values(['TRADE_DATE', 'TICKER_SYMBOL']).drop_duplicates(['TRADE_DATE', 'TICKER_SYMBOL'])
    stock_daily_k.to_hdf(stock_daily_k_path, key='table', mode='w')
    stock_daily_k = pd.read_hdf(stock_daily_k_path, key='table')

    # 平均价格涨跌幅乘仓位
    get_contribution_pricemean(fund_code, stock_name_dict, fund_holding, stock_daily_k)
    # 平均仓位乘价格涨跌幅
    get_contribution_positionmean(fund_code, stock_name_dict, fund_holding, stock_daily_k)
    return

if __name__ == "__main__":
    file_path = 'D:/Git/hbshare/hbshare/fe/xwq/data/fund_report/'
    for target_fund_code in ['002910', '008901', '007449']:
        # target_fund_code = '002910'  # 001476 270028 377240 008657
        compare_fund_code_list = []
        QuantitativeReport(file_path, target_fund_code, compare_fund_code_list).get_quantitative_report()

    # get_stock_contribution('002910')