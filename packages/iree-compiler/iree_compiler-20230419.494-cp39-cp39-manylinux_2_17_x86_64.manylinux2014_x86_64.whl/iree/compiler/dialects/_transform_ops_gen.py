...
class NamedSequenceOp(_ods_ir.OpView):
  ...
  def __init__(self, sym_name, function_type, *, sym_visibility=None, arg_attrs=None, res_attrs=None, loc=None, ip=None):
    ...
    not _ods_ir.AttrBuilder.contains('anonymous_433')) else
      _ods_ir.AttrBuilder.get('anonymous_433')(function_type, context=_ods_context))
...
class PDLMatchOp(_ods_ir.OpView):
  ...
  def __init__(self, matched, root, pattern_name, *, loc=None, ip=None):
    ...
    issubclass(type(pattern_name), _ods_ir.Attribute) or
    not _ods_ir.AttrBuilder.contains('SymbolRefAttr')) else
...
@_ods_cext.register_operation(_Dialect)
@_ods_extend_opview_class(_ods_ext_module)
class SplitHandlesOp(_ods_ir.OpView):
  OPERATION_NAME = "transform.split_handles"

  _ODS_REGIONS = (0, True)

  def __init__(self, results_, handle, num_result_handles, *, loc=None, ip=None):
    operands = []
    results = []
    attributes = {}
    regions = None
    ...
    _ods_context = _ods_get_default_loc_context(loc)
    attributes["num_result_handles"] = (num_result_handles if (
    issubclass(type(num_result_handles), _ods_ir.Attribute) or
    not _ods_ir.AttrBuilder.contains('I64Attr')) else
      _ods_ir.AttrBuilder.get('I64Attr')(num_result_handles, context=_ods_context))
    results.extend(results_)
    _ods_successors = None
    super().__init__(self.build_generic(
      attributes=attributes, results=results, operands=operands,
  ...
  @builtins.property
  def num_result_handles(self):
    return _ods_ir.IntegerAttr(self.operation.attributes["num_result_handles"])

  @num_result_handles.setter
  def num_result_handles(self, value):
    if value is None:
      raise ValueError("'None' not allowed as value for mandatory attributes")
    self.operation.attributes["num_result_handles"] = value
