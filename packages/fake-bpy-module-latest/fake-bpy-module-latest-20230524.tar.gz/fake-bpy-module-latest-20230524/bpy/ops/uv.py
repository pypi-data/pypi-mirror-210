...
def pack_islands(override_context: typing.
                 ...
                 *,
                 ...
                 pin_method: typing.Optional[typing.Any] = 'DEFAULT',
    ...
    :param rotate_method: Rotation Method * ``NONE`` No rotation -- No rotation is applied to the islands. * ``AXIS_ALIGNED`` Axis-aligned -- Rotated to a minimal rectangle, either vertical or horizontal. * ``CARDINAL`` Cardinal -- Only 90 degree rotations are allowed. * ``ANY`` Any -- Any angle is allowed for rotation.
    ...
    :param pin_method: Pinned Islands * ``DEFAULT`` Default -- Pin information is not used. * ``IGNORED`` Ignored -- Pinned islands are not packed. * ``SCALE`` Locked scale -- Pinned islands won't rescale. * ``ROTATION`` Locked rotation -- Pinned islands won't rotate. * ``ROTATION_SCALE`` Locked rotation and scale -- Pinned islands will translate only. * ``LOCKED`` Locked position -- Pinned islands are locked in place.
