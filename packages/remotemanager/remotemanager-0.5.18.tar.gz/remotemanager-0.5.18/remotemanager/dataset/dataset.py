...
\n"""
    ...
    def fetch_results(
        ...
        for runner in self.runners:
            if result_mtimes[runner.resultfile.remote]:
                pulled = runner.resultfile.local

                result = self.serialiser.load(pulled)

                timestamp = int(os.path.getmtime(pulled))

                if timestamp < runner.last_updated and not runner.is_finished:
                    self._logger.warning(f"{runner}: " f"calculation not completed yet")
                    continue
