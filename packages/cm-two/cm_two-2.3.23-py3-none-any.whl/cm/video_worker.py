...
class PhotoBoothApp:
    def __init__(self, vs, outputPath, root, canvas,
    ...
    def videoLoop(self):
        ...
        try:
            ...
            while not self.stopEvent.is_set():
                ...
                #time.sleep(1)
                #    continue
                #if self.framenot self.frame.all():
                #    pass
                try:
                ...
                image = self.add_corners(image, 15)
                image = ImageTk.PhotoImage(image)
                tag = str(uuid.uuid4())
                if self.can_show:
                    img = self.canvas.create_image(self.place_x, self.place_y,
                                                   image=image, tag=tag)
                    #self.canvas.tag_bind(img, '<Double-1>', self.img_callback)
...
def start_video_stream(root, canvas, xpos, ypos, v_width, v_height,
    ...
    print(f"rtsp://{cam_login}:{cam_pass}@{cam_ip}:{cam_port}/Streaming/Channels/101")
    vs = VideoStream(
        f"rtsp://{cam_login}:{cam_pass}@{cam_ip}:{cam_port}/Streaming/Channels/101").start()
    inst = PhotoBoothApp(vs, "output", root=root, canvas=canvas, width=v_width,
