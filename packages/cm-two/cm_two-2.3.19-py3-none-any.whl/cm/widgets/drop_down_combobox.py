...
class AutocompleteCombobox(Combobox):
    ...
    def autocomplete(self, event, delta=0):
        #print(event.keysym)
        #print("OLD USERDATA", self.user_data)
        if event.keysym == "??" or (platform.system() == "Linux" and event.keysym.startswith("U04")):
            #print("TERM", platform.system(), event.keysym.startswith(self.sys_keysym))
            self.user_data += self.get()[-1:]
        else:
            #print("NOT TERM")
            self.user_data += event.keysym
        #print("NEW USERDATA", self.user_data)
