...
def typed_dataclass(cls: dataclass) -> dataclass:
    ...
    after object initialization. Raises TypeCheckError from the typeguard
    library if a field has the wrong type.
    """

    if is_dataclass(cls):
        raise ValueError(
            "@typed_dataclass must be defined after @dataclasses.dataclass"
        )
