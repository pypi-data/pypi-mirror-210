...
import deepomatic.oef.configs.image.segmentation.mask_rcnn as image_segmentation
...
configs = {
    ...
    'image_segmentation': concatenate_configs([image_segmentation]),
}
...
class ModelFamily:
    ...
    def add_model(self, key, display_name, default_args, switch_args, pretrained_parameters):
        self._add_model_('pretraining_none', key, display_name, default_args, switch_args)
        for pretraining_key, path in pretrained_parameters.items():
            ...
            self._add_model_with_pretrained_weights_(
                ...
                key, display_name, default_args, switch_args,
                path)

    def _add_model_with_pretrained_weights_(self, pretraining_type, key, display_name, default_args, switch_args, pretrained_weights):
        default_args = dict_inject(copy.deepcopy(default_args), {
            'trainer': {
                'pretrained_parameters': pretrained_weights
            },
        })
        self._add_model_(pretraining_type, key, display_name, default_args, switch_args)

    def _add_model_(self, pretraining_type, key, display_name, default_args, switch_args):
        model_key = '{}.{}.{}'.format(self._family_name, pretraining_type, key)
        self._models[model_key] = ModelArguments(display_name, default_args, switch_args)
...
def add_models_to_family(family, model_config):
    ...
    default_args = {}
    # Classification config is already handling injection of the learning rate and optimizer.
    # FIXME: moving the common_default_args definition for each task separately could
    # make things more explicit and transparent.
    if family.name != "image_classification":
    ...
    for variant in model_config.variants:
        ...
        switch_args = variant.switch_args
        family.add_model(
            variant.alias,
