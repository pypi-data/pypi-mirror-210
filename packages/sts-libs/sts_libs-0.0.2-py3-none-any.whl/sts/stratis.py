...
class Stratis(Wrapper):
    ...
    def __init__(self, disable_check=True):
        ...
        if linux.dist_ver() < 8:
            sts_print("FATAL: Stratis is not supported on RHEL < 8.")
        ...
        if linux.service_status(get_stratis_service(), verbose=False) != 0:
            if not linux.service_restart(get_stratis_service()):
                sts_print(f"FAIL: Could not start {get_stratis_service()} service")
            else:
                sts_print(f"INFO: Service {get_stratis_service()} restarted.")
