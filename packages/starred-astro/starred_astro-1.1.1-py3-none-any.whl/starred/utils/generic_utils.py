...
def save_npy(array, path):
    """
    Saves ``.npy`` file of an array to a specified ``path``.
...
# def rebin_3D(arr, new_shape, axis=0): #TODO : this is not working yet... keep the old implementation for the moment
#   jnp.apply_along_axis(rebin, axis, arr, new_shape)
