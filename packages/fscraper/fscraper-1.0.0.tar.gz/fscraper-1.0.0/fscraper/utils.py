...
def calculate_beta(code: str, market: str='^N225', period: str='1y'):
    ...
    df = pd.concat([df1['close'], df2['close']], axis=1, join='outer', keys= [code, market])
...
def calculate_macd(close: pd.Series, short_periods: int=12, long_periods: int=26, signal_periods: int=9):
    ...
    short_ema = close.ewm(span=short_periods, adjust=False, min_periods=short_periods).mean()
    # Get the 26-day EMA of the closing price
    long_ema = close.ewm(span=long_periods, adjust=False, min_periods=long_periods).mean()
    ...
    macd_signal = macd.ewm(span=signal_periods, adjust=False, min_periods=signal_periods).mean()
