...
def compress_tsc(signal: np.ndarray, pers_diag: Optional[pd.DataFrame] = None,
                 persistence_cutoff: Optional[int] = None,
                 num_indices_to_keep: Optional[Union[int, float, str]] = None) -> np.ndarray:
    """
        ...
        if num_indices_to_keep == "all":
            ...
            if 0 < num_indices_to_keep < 1:
                ...
                if num_indices_to_keep < 2:
                    warnings.warn("Decimal `num_indices_to_keep` results in less than 2 points, "
                                  "defaulting to return just the 2 edges", stacklevel=2)
...
def reconstruct_tsc(signal: np.ndarray, x_values: Optional[Union[list, np.ndarray]] = None) -> np.ndarray:
    """
...
def tsc_pipeline(signal: np.ndarray, n_keep: Union[float, int, str],
                 pers_diag: Optional[pd.DataFrame] = None) -> np.ndarray:
    """
    ...
    n = signal.shape[0]
    compression = compress_tsc(np.c_[np.arange(n), signal], pers_diag=pers_diag,
