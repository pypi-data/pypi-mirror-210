...
prs = Presentation('template169.pptx')
...
def merge_data(df_fund,df_tradeweek,df_benchmark,date,fund_nv,benchmark_close_price):
    ...
    ##排序+缺失值+归一化
    df = df.sort_values(by=date,ascending=True).fillna(method='ffill')
...
def details_cpt():        
    ...
    if strategy == '指数增强策略':
        ...
        intro.columns[0].width = Cm(2)
        intro.columns[1].width = Cm(12)    
