...
import build.util
import pkginfo
from modelbit.api import MbApi, ObjectApi, PackageApi, PackageDescResponse
from modelbit.cli.describe import describeFile, toYaml
...
class PackageBuilder:
  ...
  def uploadPackage(self,
    ...
    ObjectApi(None, self.api.api).uploadRuntimeObject(content, contentHash,
                                                      f"packages/{os.path.basename(wheelPath)}")
    objDesc = describeFile(content)
    yamlContent = toYaml(contentHash, len(content), objDesc)
    self.api.uploadPackage(pkgInfo.name, pkgInfo.version, yamlContent, allowClobberVersions)
    return pkgInfo
